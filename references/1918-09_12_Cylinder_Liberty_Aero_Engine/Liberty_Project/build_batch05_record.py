"""Append batch 05 to the preserved batch-04 SLA. Run inside Scribus 1.6.x.
...
OLD=json.loads((ROOT/'data/baseline_v04_release.json').read_text())
D=json.loads((ROOT/'data/batch05_transcription.json').read_text())
A=json.loads((ROOT/'data/batch05_artwork.json').read_text())
def main():
 ...
 s.openDoc(str(ROOT/OLD['sla']));assert s.pageCount()==80
 s.setInfo('Ministry of Munitions; digital reconstruction','The Liberty 12-Cylinder Aero Engine Handbook (1918) — '+R['release_id'],'Batches 01–05: source scans 0001–0100, cover through printed page 98. Editable transcription and original-pixel figures. Provisional trim and typography.')
 s.setRedraw(False)
 for p in D['pages']:
  ...
  for legend in p['legends']:
   for j,t in enumerate(legend['lines']):
    h.rich(t,legend['x'],(legend['y']+j*legend['step'])*h.SX,legend['width'],legend['size'])
 ...
 s.setRedraw(True)
 oldframes=json.loads((ROOT/'data/baseline_v04_native_frames.json').read_text());frames=oldframes+h.frames
 ...
 pdf=s.PDFfile();pdf.file=str(ROOT/R['pdf']);pdf.pages=list(range(1,101));pdf.version=15;pdf.compress=True;pdf.compressmtd=2;pdf.quality=0;pdf.downsample=0;pdf.resolution=600;pdf.outdst=0;pdf.bookmarks=True;pdf.save();del pdf

...
import numpy as np
import fitz
R=Path(__file__).resolve().parents[1];Q=R.parent/'qa/batch05';Q.mkdir(exist_ok=True)
release=json.loads((R/'data/release.json').read_text());old=json.loads((R/'data/baseline_v04_release.json').read_text())
...
oldmanifest=json.loads((R/'data/baseline_v04_release_manifest.json').read_text())['files'];protected=[]
...
(R/'data/baseline_v04_preservation.json').write_text(json.dumps(report,indent=2))
...
with ThreadPoolExecutor(max_workers=4) as ex:renders=list(ex.map(render,range(81,101)))
...
for k in range(5):
 ...
 for j,n in enumerate(range(81+k*4,85+k*4)):

...
import numpy as np
import fitz
R=Path(__file__).resolve().parents[1];Q=R.parent/'qa/batch08';Q.mkdir(exist_ok=True)
release=json.loads((R/'data/release.json').read_text());old=json.loads((R/'data/baseline_v07_release.json').read_text())
...
oldmanifest=json.loads((R/'data/baseline_v07_release_manifest.json').read_text())['files'];protected=[]
...
(R/'data/baseline_v07_preservation.json').write_text(json.dumps(report,indent=2))
...
with ThreadPoolExecutor(max_workers=4) as ex:renders=list(ex.map(render,range(141,161)))
...
for k in range(5):
 ...
 for j,n in enumerate(range(141+k*4,145+k*4)):

"""OCR review material for pinned scans 0101–0120; leaves approved text alone."""
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor
from PIL import Image,ImageDraw
import json,subprocess,csv,io
ROOT=Path(__file__).resolve().parents[1];QA=ROOT.parent/'qa/batch06';QA.mkdir(parents=True,exist_ok=True)
def one(n):
...
with ThreadPoolExecutor(max_workers=4) as ex:pages=list(ex.map(one,range(101,121)))
(ROOT/'data/batch06_ocr_draft.json').write_text(json.dumps(pages,indent=2))
...
for k in range(5):
 ...
 for j in range(4):
  n=101+k*4+j;im=Image.open(QA/f'{n:04}_view.jpg');im.thumbnail((390,700));c.paste(im,(j*400+(400-im.width)//2,25));d.text((j*400+8,5),f'Scan {n:04} / printed {n-2}',fill='black')

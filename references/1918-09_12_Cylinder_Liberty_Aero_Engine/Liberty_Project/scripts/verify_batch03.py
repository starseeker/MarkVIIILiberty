...
import numpy as np
import fitz
R=Path(__file__).resolve().parents[1];Q=R.parent/'qa/batch03';Q.mkdir(exist_ok=True)
release=json.loads((R/'data/release.json').read_text());old=json.loads((R/'data/batch02_release.json').read_text())
...
oldmanifest=json.loads((R/'data/batch02_release_manifest.json').read_text())['files'];protected=[]
...
(R/'data/batch02_preservation.json').write_text(json.dumps(report,indent=2))
...
with ThreadPoolExecutor(max_workers=4) as ex:renders=list(ex.map(render,range(41,61)))
...
for k in range(5):
 ...
 for j,n in enumerate(range(41+k*4,45+k*4)):

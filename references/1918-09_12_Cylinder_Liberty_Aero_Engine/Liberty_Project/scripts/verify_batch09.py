...
import numpy as np
import fitz
R=Path(__file__).resolve().parents[1];Q=R.parent/'qa/batch09';Q.mkdir(exist_ok=True)
release=json.loads((R/'data/release.json').read_text());old=json.loads((R/'data/baseline_v08_release.json').read_text())
...
oldmanifest=json.loads((R/'data/baseline_v08_release_manifest.json').read_text())['files'];protected=[]
...
(R/'data/baseline_v08_preservation.json').write_text(json.dumps(report,indent=2))
...
with ThreadPoolExecutor(max_workers=4) as ex:renders=list(ex.map(render,range(161,163)))
...
# The final closing surfaces are genuinely unprinted, with no hidden native objects.
assert len(doc)==162
blank_checks=[]
for n in [161,162]:
 p=doc[n-1];pix=p.get_pixmap(matrix=fitz.Matrix(1,1))
 ok=not p.get_text() and not p.get_image_info() and not p.get_drawings() and set(pix.samples)=={255}
 assert ok,n
 objects=[e for e in b.findall('.//PAGEOBJECT') if e.get('OwnPage')==str(n-1)]
 assert not objects,n
 blank_checks.append(dict(page=n,all_white=True,native_objects=0))
classification=json.loads((R/'data/batch09_classification.json').read_text())
assert [x['leaf'] for x in classification if x['include_in_manual']]==[161,162]
assert [x['leaf'] for x in classification if not x['include_in_manual']]==[163,164,165,166,167]
assert len(sources)==167
(R/'data/completion_validation.json').write_text(json.dumps(dict(staged_sources=167,manual_pages=162,closing_pages=blank_checks,excluded_scanner_sources=[163,164,165,166,167],all_staged_sources_accounted_for=True),indent=2))
sheet=Image.new('RGB',(800,750),'#e9e9e9');draw=ImageDraw.Draw(sheet)
for j,n in enumerate([161,162]):
...
buf=io.BytesIO();sheet.save(buf,format='PNG');(Q/'final_closing_pages.png').write_bytes(buf.getvalue())

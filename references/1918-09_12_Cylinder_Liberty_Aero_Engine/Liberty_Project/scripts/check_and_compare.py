...
# All seven final source exposures are documented, including five exclusions.
out=fitz.open();mapping=[]
classification=json.loads((ROOT/'data/batch09_classification.json').read_text())
for index,meta in enumerate(classification):
 n=meta['leaf'];included=meta['include_in_manual']
 sheet=out.new_page(width=880,height=810);right=457
 sheet.insert_text((26,25),f'LIBERTY ENGINE HANDBOOK | {R["release_id"]} | scan {n:04}',fontsize=9,fontname='helv')
 sheet.insert_text((26,45),meta['label'].upper(),fontsize=11,fontname='hebo')
 sheet.insert_text((26,68),'SOURCE SCAN',fontsize=8,fontname='hebo')
 sheet.insert_text((right,68),'SCRIBUS RECONSTRUCTION' if included else 'SOURCE DISPOSITION',fontsize=8,fontname='hebo')
 im=Image.open(ROOT/'sources'/f'liberty12cylinde00grea_{n:04}.jp2').convert('RGB');im.thumbnail((1450,2500));b=io.BytesIO();im.save(b,format='JPEG',quality=92)
 sheet.insert_image(fitz.Rect(26,85,422,776),stream=b.getvalue(),keep_proportion=True)
 if included:
  # Empty PDF pages cannot be imported with show_pdf_page; the white panel is exact.
  assert not src[n-1].get_text() and not src[n-1].get_image_info() and not src[n-1].get_drawings()
  sheet.draw_rect(fitz.Rect(right,85,853,776),color=(.84,.84,.84),width=.4)
  note='Blank manual page; later '+('barcode label' if n==161 else 'ownership marks')+' omitted.'
  sheet.insert_text((right,793),note,fontsize=8,fontname='helv',color=(.25,.25,.25))
 else:
  text='Excluded from the reconstructed manual.\n\n'+meta['observation']+'\n\nThe original JP2 remains in the project. No book page or printed folio has been assigned.'
  remaining=sheet.insert_textbox(fitz.Rect(right+25,130,828,410),text,fontsize=12,fontname='helv',lineheight=1.4)
  assert remaining>=0
 mapping.append(dict(comparison_page=index+1,source_leaf=n,master_page=meta['master_page'],label=meta['label'],printed_folio=None,blank=included,included_in_manual=included,classification=meta['classification']))
...
review=ROOT.parent/'qa/batch09';review.mkdir(exist_ok=True)
for n in [160,161,162]:
 pix=src[n-1].get_pixmap(matrix=fitz.Matrix(2,2));b=pix.tobytes('png');Image.open(io.BytesIO(b)).load();(review/f'final_{n}.png').write_bytes(b)
proof=fitz.open(ROOT/R['comparison'])
for i,p in enumerate(proof,161):
 b=p.get_pixmap(matrix=fitz.Matrix(1.4,1.4)).tobytes('png');Image.open(io.BytesIO(b)).load();(review/f'comparison_{i}.png').write_bytes(b)
print('162 manual pages verified; exact native/PDF character inventories match. Seven-sheet final source comparison written.')

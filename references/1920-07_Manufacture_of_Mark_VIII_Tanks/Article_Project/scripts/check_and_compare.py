"""Validate text/assets and create a seven-page source/reconstruction proof."""
...
def main():
 ...
 p=fitz.open(ROOT/r['pdf']);assert len(p)==7
 p.set_page_labels([dict(startpage=0,prefix='',style='D',firstpagenum=27)])
 p.set_toc([[1,'Manufacture of Mark VIII Tanks at Rock Island Arsenal',1]]+[[2,'Original page '+str(27+i),i+1] for i in range(7)])
 ...
 for i,page in enumerate(p):
  expected=''.join(f['text'] for f in frames if f['page']==i+27);actual=page.get_text()
  ...
  checks.append(dict(page=i+27,missing_characters=dict(diff),extra_characters=dict(extra),out_of_bounds=oob))
 ...
 result=dict(pages=checks,images=links,pdf_fonts_embedded=all(x[1]!='n/a' for i in range(7) for x in p.get_page_fonts(i)))
 ...
 for i in range(7):
  ...
  page.insert_text((28,24),'SOURCE SCAN'+(' - supplied composite' if i==1 else ''),fontsize=10,fontname='hebo')
  page.insert_text((656,24),'SCRIBUS RECONSTRUCTION - original page '+str(27+i),fontsize=10,fontname='hebo')
  source=ROOT/'sources'/('MarkVIII_manufacture'+('002_composite' if i==1 else '%03d'%(i+1))+'.jpg')

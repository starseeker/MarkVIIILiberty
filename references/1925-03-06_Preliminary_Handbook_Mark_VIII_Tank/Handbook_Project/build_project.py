#!/usr/bin/env python3
"""Append printed pages 241–251 to the preserved 240-page Scribus checkpoint.
...
def body(n):
 for r in BODY.get(str(n),[]):
  ...
  if r.get('smallcaps_prefix'):
   ...
   s.selectText(0,len(r['smallcaps_prefix']),obj);s.setCharacterStyle('Batch13 Note small capitals',obj);s.selectText(0,0,obj)
...
from layout_batch13 import extra
...
def main():
 ...
 BODY=json.loads((ROOT/'data/body_batch13.json').read_text());assets=json.loads((ROOT/'data/assets.json').read_text())
 previous=json.loads((ROOT/'data/batch12_native_validation.json').read_text())['frames']
 s.openDoc(str(ROOT/'Handbook_Checkpoint_001-240_v12.sla'))
 if s.pageCount()!=240:raise RuntimeError('Baseline must contain exactly 240 pages.')
 s.setInfo('Ordnance Department; digital reconstruction','Preliminary Handbook of the Mark VIII Tank — cumulative pages 1–251','1918 handbook, reprinted March 6, 1925. Review checkpoint 13; provisional C059 and 5.5 x 8.5 inch canvas. See PROJECT_STATUS.json.')
 s.createCharStyle(name='Batch13 Note small capitals',font='C059 Roman',fontsize=9.7,scaleh=.925,features='smallcaps',fillcolor='Black',language='en_US')
 s.setRedraw(False)
 ...
 s.gotoPage(1);s.saveDocAs(str(ROOT/'Handbook_Master.sla'))
 (ROOT/'build-status.txt').write_text(f'PASS: 251 cumulative pages; {len(validation)} native text frames, including {len(b.frames)} new frames; no overflow or text mismatches.\n')
 s.setRedraw(True)

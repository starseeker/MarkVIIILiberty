#!/usr/bin/env python3
"""Append printed pages 181–200 to the preserved 180-page Scribus checkpoint.
...
def body(n):
 for r in BODY.get(str(n),[]):
  ...
  if r.get('smallcaps_prefix'):
   ...
   s.selectText(0,len(r['smallcaps_prefix']),obj);s.setCharacterStyle('Batch10 Note small capitals',obj);s.selectText(0,0,obj)
...
from layout_batch10 import extra
...
def main():
 ...
 BODY=json.loads((ROOT/'data/body_batch10.json').read_text());assets=json.loads((ROOT/'data/assets.json').read_text())
 previous=json.loads((ROOT/'data/batch09_native_validation.json').read_text())['frames']
 s.openDoc(str(ROOT/'Handbook_Checkpoint_001-180_v9.sla'))
 if s.pageCount()!=180:raise RuntimeError('Baseline must contain exactly 180 pages.')
 s.setInfo('Ordnance Department; digital reconstruction','Preliminary Handbook of the Mark VIII Tank — cumulative pages 1–200','1918 handbook, reprinted March 6, 1925. Review checkpoint 10; provisional C059 and 5.5 x 8.5 inch canvas. See PROJECT_STATUS.json.')
 s.createCharStyle(name='Batch10 Note small capitals',font='C059 Roman',fontsize=9.7,scaleh=.925,features='smallcaps',fillcolor='Black',language='en_US')
 s.setRedraw(False)
 for n in range(181,201):
  s.newPage(-1);s.gotoPage(n)
  if n==184:b.center('(184)',17+2390*.225,8.3)
  else:b.folio(n)
  body(n)
  for a in assets:
   if a['page']==n:art(a)
 ...
 s.gotoPage(1);s.saveDocAs(str(ROOT/'Handbook_Rebuilt_001-200_v10.sla'))
 (ROOT/'build-status.txt').write_text(f'PASS: 200 cumulative pages; {len(validation)} native text frames, including {len(b.frames)} new frames; no overflow or text mismatches.\n')
 s.setRedraw(True)

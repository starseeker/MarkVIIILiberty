...
and a targeted cross-book ambiguity audit are complete. A cleanup and selective
vector pass covers every plate. The book now uses the recommended native vector
and hybrid artwork, plus the approved aggressive Plate 12 cleanup. Independent proofreading
...
is the audit and figure-restoration work of **September 12, 2026**.
...
The native document contains **25,682 editable text frames** and **125,890 native
...
printer imprints are also editable. Figure artwork uses seven native SVG groups
and 24 linked raster images; internal lettering remains traced outlines or pixels.
...
## Clean figure and vector masters

A subsequent restoration pass covers all **34 plates in 31 assets**. It removes
surrounding page color, reduces uneven illumination and show-through, and improves
usable contrast without changing the accepted geometry or source pixel dimensions.
Protected photographs and shaded parts retain continuous gray tones. Fine line
regions use local paper normalization; no generative reconstruction is used.

The preferred masters include **five vector SVGs** (Plates 6, 7, 21, 24
and 29), **two hybrid SVGs** (Plates 10–11 together, and Plate 16), and **24 raster
assets**. Hybrid files contain embedded grayscale regions where photographs or
artistic shading require them. The SVGs trace original lettering and ink in eight
gray levels; they are neither semantic CAD geometry nor editable type. All 31
assets also have lossless grayscale PNG and PDF versions.

Plate 12 receives an additional correction from a **registered mirror of its
reverse, page 283**. The first local cleanup had not used that reference. A
projective and smooth local registration aligns the faint tank/wiring ghost, then
a fitted attenuation field reduces its contribution. Only the reverse reference
is resampled. The selected blank-patch checks show further improvement while the
checked dark rod/label/edge pixels remain. A subsequent user-approved aggressive keep mask removes unassociated mottling
while preserving reviewed text, mechanical details and line corridors. Plate 12
remains raster-preferred. The earlier conservative master and comparison are retained.
Several clusters overlap reverse-side inked areas and may reflect stronger local
ink penetration; they are not classified as unrelated stains. The foldout
also favors raster because of its dense tiny lettering; its flat geometry is
unchanged. No paired subtraction has been applied to the other plates.

`Figure_Restoration_Comparisons.pdf` contains the pilots, enlarged checks and all
31 before/after comparisons. `Plate12_Mirror_Study.pdf` documents the paired trial.
`Restored_Figures.pdf` presents the preferred masters in plate order. Methods,
format choices and limitations are in `FIGURE_RESTORATION.md` and `restoration/`.
The book now imports the **five vector and two hybrid SVG masters as native
Scribus groups**, and links the other **24 clean PNGs**. Scribus can import SVG
through File → Import → Get Vector File; no prior rasterization is required.
Hybrid tone images are linked lossless PNGs in `restoration/native/`, because the
tested Scribus importer did not retain embedded data URIs. Standalone hybrid SVGs
still embed their images. The book PDF preserves vectors and uses lossless image
compression without downsampling. Original source and geometry assets are preserved.

Plate 12 still has a slight varying slant (about 1.8 degrees clockwise at the
upper rule, about 1.2 degrees in the middle and nearly level at the bottom).
This release preserves its approved geometry rather than rotating the entire plate.

Validation compares this release with the preceding clean-raster checkpoint.
All 314 PDF pages retain their text and original-folio order. All 32,625 native
objects outside the seven replaced artwork frames are unchanged; the seven groups
retain the original frame positions, sizes and rotations. All source dimensions
are retained. The 283 non-figure pages render identically. Figure pages now use
native vectors or lossless raster export, with the approved new Plate 12 pixels. These checks establish preservation, not complete historical
accuracy or recovery of lost detail. Some residual blemishes and sampling limits
remain; an independent transcription proofread is still needed.
...
| `assets/` | Preserved geometry-corrected figure references |
| `restoration/` | Clean PNGs, SVG/PDF masters, linked hybrid tone PNGs, masks, models and validation |
| `FIGURE_RESTORATION.md` | Figure cleanup methods, limits and per-plate decisions |
| `Plate12_Mirror_Study.pdf` | Mirrored reverse-page comparison and preservation checks |
...
Keep `restoration/` beside the document, along with the preserved `assets/` folder. A rebuild is unnecessary for normal editing.
...
Figure restoration entry points are `paired_bleedthrough.py`, `restore_figures.py`,
`export_restored_figures.py` and `make_restoration_proofs.py`. Follow
`FIGURE_RESTORATION.md` for reproduction and review requirements.

"""Replace accepted figure artwork without altering page order or native text.
Run inside Scribus after rectifying the accepted text/line models.
"""
...
def main():
 numbers=list(map(int,json.loads((ROOT/'calibration/text_line_controls.json').read_text())))
 prior=json.loads((ROOT/'validation.json').read_text())
 transforms=json.loads((ROOT/'calibration/figure_transforms.json').read_text())
 (ROOT/'build-status.txt').unlink(missing_ok=True);(ROOT/'build-error.txt').unlink(missing_ok=True)
 s.openDoc(str(ROOT/'SNL_G13_Pilot.sla'));assert s.pageCount()==len(b.LABELS)
 s.setRedraw(False)
 for n in numbers:
  s.gotoPage(b.LABELS.index(str(n))+1)
  s.deleteObject(f'p{n}-plate-artwork')
  b.figure_artwork(n,transforms)
  print(f'Updated artwork on printed page {n}',flush=True)
 ...
 s.setInfo('Ordnance Department; digital reconstruction',
  'S. N. L. No. G-13 — complete draft; text-guided figure refinement',

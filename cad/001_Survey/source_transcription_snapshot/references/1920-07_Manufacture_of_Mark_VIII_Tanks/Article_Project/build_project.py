"""Run in Scribus 1.6.x: Script > Execute Script. Creates a cover and seven article pages.
...
C=json.loads((ROOT/'data/cover.json').read_text())
...
def text(t,x,y,w,size=10.5,font='Roman',align=0,name=None,scale=100):
 global serial
 serial+=1;page_id='cover' if s.currentPage()==1 else s.currentPage()+25
 name=name or '%s-text-%04d'%('cover' if page_id=='cover' else 'p%d'%page_id,serial)
 ...
 frames.append(dict(name=o,page=page_id,text=t,x=x,baseline=y,width=w,size=size,font=font,scale=scale,alignment=align))
 return o
...
def cover_page():
 # The three original publication lines remain separate from the added citation.
 center(C['source_lines'][0],359,36)
 center(C['source_lines'][1],408,18)
 center(C['source_lines'][2],455,11.3)
 rule(274,500,338,500,.4)
 center(C['article_lines'][0],551,20.5)
 center(C['article_lines'][1],577,20.5)
 center(C['article_lines'][2],618,12)
 center(C['article_lines'][3],650,11)
 center(C['editorial_note'][0],790,8,'Italic')
 center(C['editorial_note'][1],802,8,'Italic')
def title_page():
...
def main():
 ...
 s.newDocument((W,H),(45,45,40,34),s.PORTRAIT,26,s.UNIT_POINTS,s.PAGE_2,0,8)
 s.setInfo('Harry B. Jordan','Manufacture of Mark VIII Tanks at Rock Island Arsenal — '+R['release_id'],'Army Ordnance, Volume I, No. 1, July–August 1920, pp. 27–33. Editable reconstruction with an adapted volume title page and added article citation.')
 s.setRedraw(False)
 ...
 # Add cover text last to preserve article object names and style definitions.
 s.gotoPage(1);cover_page()
 s.setRedraw(True)
 ...
 pdf=s.PDFfile();pdf.file=str(ROOT/R['pdf']);pdf.pages=list(range(1,9));pdf.version=15;pdf.compress=True;pdf.compressmtd=2;pdf.quality=0;pdf.downsample=0;pdf.resolution=600;pdf.outdst=0;pdf.bookmarks=True;pdf.fonts=[];pdf.subsetList=list(FONT.values());pdf.save();del pdf
 (ROOT/'data/native_frames.json').write_text(json.dumps(frames,ensure_ascii=False,indent=2))

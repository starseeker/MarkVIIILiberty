...
import numpy as np
import fitz
R=Path(__file__).resolve().parents[1];Q=R.parent/'qa/batch02';Q.mkdir(exist_ok=True)
release=json.loads((R/'data/release.json').read_text());old=json.loads((R/'data/batch01_release.json').read_text())
...
oldmanifest=json.loads((R/'data/batch01_release_manifest.json').read_text())['files'];protected=[]
...
(R/'data/batch01_preservation.json').write_text(json.dumps(report,indent=2))
...
for a in json.loads((R/'data/artwork.json').read_text()):
 p=R/'assets'/a['asset'];im=Image.open(p);im.load();original=Image.open(R/'assets'/f'figure_{a["figure"]:03}_original.png');original.load()
...
with ThreadPoolExecutor(max_workers=4) as ex:renders=list(ex.map(render,range(21,41)))
...
for k in range(5):
 ...
 for j,n in enumerate(range(21+k*4,25+k*4)):
  im=Image.open(Q/f'poppler_{n:02}.png').convert('RGB');im.thumbnail((390,700));x=j*400+5;sheet.paste(im,(x,28));draw.text((x,9),f'Scan {n:04} / printed p{n-2}',fill='black')

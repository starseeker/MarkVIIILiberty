...
def text(t,x,y,w,size=BODY,font='Roman',align=0,scale=100,name=None):
 ...
 if key not in styles:
  sty='Liberty b01 '+str(len(styles)+1);lead=size*1.25
...
def main():
 ...
 s.newDocument((W,H),(25,25,25,25),s.PORTRAIT,1,s.UNIT_POINTS,s.PAGE_1,0,1)
 s.setInfo('Ministry of Munitions; digital reconstruction','The Liberty 12-Cylinder Aero Engine Handbook (1918) — '+R['release_id'],'Batch 01: source scans 0001–0020, cover through printed page 18. Editable transcription and original-pixel figures. Provisional trim and typography.')
 s.setRedraw(False)
 for p in D['pages']:
  n=p['leaf']
  if n>1:s.newPage(-1)
  s.gotoPage(n)
  if n>=5:center(str(n-2),28 if n in [11,12] else 53,11)
  special(n)
  for b in p['blocks']:
   for j,l in enumerate(b['lines']):
    ind=l['indent']*SX;txt=l['text'];y=(b['y']+j*b['step'])*SX
    width=BW-ind
    # Firing-order rows leave room for the shared brace and propeller label.
    if n==11 and j in [30,31]:width=143
    if n==11 and j in [32,33]:width=262
    al=4 if n not in [10,11,12] and len(txt)>46 and '{' not in txt else 0
    rich(txt,LEFT+ind,y,width,BODY,al)
  for a in [a for a in A if a['leaf']==n]:
   x0,y0,x1,y1=a['crop'];cx={4:946.5,12:952.5,13:917.5,14:1011.5,15:878.5,16:1014.5,17:913.5,18:1007.5,19:922.5,20:1005}[n]
   o=s.createImage(W/2+(x0-cx)*SX,y0*SX,(x1-x0)*SX,(y1-y0)*SX,'figure-'+str(a['figure']))
   s.loadImage(str(ROOT/'assets'/a['asset']),o);s.setScaleImageToFrame(True,True,o);s.setTextFlowMode(o,0)
  for cap in CAP.get(n,[]):
   fig,yp,*ls=cap;center('Fig. '+str(fig)+'.',yp*SX,10.8)
   for k,t in enumerate(ls):center(t,(yp+86+k*86)*SX,10.8)
 s.setRedraw(True)
 ...
 pdf=s.PDFfile();pdf.file=str(ROOT/R['pdf']);pdf.pages=list(range(1,21));pdf.version=15;pdf.compress=True;pdf.compressmtd=2;pdf.quality=0;pdf.downsample=0;pdf.resolution=600;pdf.bookmarks=True;pdf.save();del pdf
 (ROOT/'data/native_validation.json').write_text(json.dumps(dict(scribus_version=s.scribus_version,pages=s.pageCount(),editable_text_frames=len(frames),rules=len(rules),overflow_before=err,overflow_after=err2,frames_below_85_percent=fits),indent=2))

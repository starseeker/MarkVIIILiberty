"""Append batch 07 to the preserved batch-06 SLA. Run inside Scribus 1.6.x.
...
OLD=json.loads((ROOT/'data/baseline_v06_release.json').read_text())
D=json.loads((ROOT/'data/batch07_transcription.json').read_text())
A=json.loads((ROOT/'data/batch07_artwork.json').read_text())
def main():
 ...
 import runpy
 runpy.run_path(str(ROOT/'scripts/prepare_batch07_seed.py'))
 s.openDoc(str(ROOT/'_batch07_seed.sla'));assert s.pageCount()==140
 s.setInfo('Ministry of Munitions; digital reconstruction','The Liberty 12-Cylinder Aero Engine Handbook (1918) — '+R['release_id'],'Batches 01–07: source scans 0001–0140, cover through printed page 136, including page-123 foldout and two blank leaves. Editable transcription and original-pixel figures. Provisional trim and typography.')
 s.setRedraw(False)
 for p in D['pages']:
  ...
  for v in p['positioned_text']:h.text(v['text'],v['x'],v['y']*h.SX,v['width'],v['size'],v.get('font','Roman'),align=v['align'])
 ...
 s.setRedraw(True)
 oldframes=json.loads((ROOT/'data/baseline_v06_native_frames.json').read_text());frames=oldframes+h.frames
 ...
 pdf=s.PDFfile();pdf.file=str(ROOT/R['pdf']);pdf.pages=list(range(1,141));pdf.version=15;pdf.compress=True;pdf.compressmtd=2;pdf.quality=0;pdf.downsample=0;pdf.resolution=600;pdf.outdst=0;pdf.bookmarks=True;pdf.save();del pdf

"""Append batch 06 to the preserved batch-05 SLA. Run inside Scribus 1.6.x.
...
OLD=json.loads((ROOT/'data/baseline_v05_release.json').read_text())
D=json.loads((ROOT/'data/batch06_transcription.json').read_text())
A=json.loads((ROOT/'data/batch06_artwork.json').read_text())
def main():
 ...
 s.openDoc(str(ROOT/OLD['sla']));assert s.pageCount()==100
 s.setInfo('Ministry of Munitions; digital reconstruction','The Liberty 12-Cylinder Aero Engine Handbook (1918) — '+R['release_id'],'Batches 01–06: source scans 0001–0120, cover through printed page 118. Editable transcription and original-pixel figures. Provisional trim and typography.')
 s.setRedraw(False)
 for p in D['pages']:
  ...
  for b in p['blocks']:
   for j,l in enumerate(b['lines']):
    ...
    start=len(h.frames)
    align=l.get('align',4 if len(t)>(28 if n==100 and ind>=40 else 46) and '{' not in t and (j<len(b['lines'])-1 or not t.endswith(('.', '—'))) else 0)
  ...
  for a in [a for a in A if a['leaf']==n]:
   ...
   o=s.createImage(h.W/2+(x0-cx)*scale,y0*h.SX,(x1-x0)*scale,(y1-y0)*scale,'figure-'+str(a['figure']))
  ...
  for v in p['positioned_text']:h.text(v['text'],v['x'],v['y']*h.SX,v['width'],v['size'],align=v['align'])
 ...
 s.setRedraw(True)
 oldframes=json.loads((ROOT/'data/baseline_v05_native_frames.json').read_text());frames=oldframes+h.frames
 ...
 pdf=s.PDFfile();pdf.file=str(ROOT/R['pdf']);pdf.pages=list(range(1,121));pdf.version=15;pdf.compress=True;pdf.compressmtd=2;pdf.quality=0;pdf.downsample=0;pdf.resolution=600;pdf.outdst=0;pdf.bookmarks=True;pdf.save();del pdf

"""Append batch 04 to the preserved batch-03 SLA. Run inside Scribus 1.6.x.
...
OLD=json.loads((ROOT/'data/baseline_v03_release.json').read_text())
D=json.loads((ROOT/'data/batch04_transcription.json').read_text())
A=json.loads((ROOT/'data/batch04_artwork.json').read_text())
def main():
 ...
 s.openDoc(str(ROOT/OLD['sla']));assert s.pageCount()==60
 s.setInfo('Ministry of Munitions; digital reconstruction','The Liberty 12-Cylinder Aero Engine Handbook (1918) — '+R['release_id'],'Batches 01–04: source scans 0001–0080, cover through printed page 78. Editable transcription and original-pixel figures. Provisional trim and typography.')
 s.setRedraw(False)
 for p in D['pages']:
  ...
  for b in p['blocks']:
   for j,l in enumerate(b['lines']):
    ...
    h.rich(t,h.LEFT+ind,y,h.BW-ind,h.BODY,4 if len(t)>(28 if n in [73,75] and ind>=20 else 46) and '{' not in t and (j<len(b['lines'])-1 or not t.endswith(('.', '—'))) else 0)
 ...
 s.setRedraw(True)
 oldframes=json.loads((ROOT/'data/baseline_v03_native_frames.json').read_text());frames=oldframes+h.frames
 ...
 pdf=s.PDFfile();pdf.file=str(ROOT/R['pdf']);pdf.pages=list(range(1,81));pdf.version=15;pdf.compress=True;pdf.compressmtd=2;pdf.quality=0;pdf.downsample=0;pdf.resolution=600;pdf.outdst=0;pdf.bookmarks=True;pdf.save();del pdf

...
def plug_envelope(shaft,plug,pose,a):
    """Check the inserted plain thread envelope; a pipe plug has no bolt seat.

    The common Q52C reconstruction has a 12 mm shank and .2 mm radial
    clearance. Its little square head does not establish a bearing shoulder.
    These native checks validate placement only, never sealing or retention.
    """
    import Part
    local=[]
    for original in [shaft,plug]:
        shape=original.copy();shape.Placement=pose.inverse().multiply(shape.Placement)
        local.append(shape)
    hole_radius=a['roller_oil_bore']/2;shank_radius=hole_radius-.2;end=a['end']
    def cylinders(shape,radius):
        return [f for f in shape.Faces if isinstance(f.Surface,Part.Cylinder)
                ...
                and abs(abs(f.Surface.Axis.y)-1)<1e-6
                and math.hypot(f.Surface.Center.x,f.Surface.Center.z)<1e-5]
    holes=cylinders(local[0],hole_radius);shanks=cylinders(local[1],shank_radius)
    if not holes or not shanks:raise ValueError('Drive oil-plug cylindrical envelope missing')
    low=min(f.BoundBox.YMin for f in shanks);high=max(f.BoundBox.YMax for f in shanks)
    length=sum(f.Area for f in shanks)/(2*math.pi*shank_radius)
    if abs(low-(end-12))>1e-5 or abs(high-end)>1e-5 or abs(length-12)>1e-5:
        raise ValueError('Drive oil-plug insertion does not follow shaft end')
    if min(f.BoundBox.YMin for f in holes)>low+1e-5 or abs(max(f.BoundBox.YMax for f in holes)-end)>1e-5:
        raise ValueError('Drive oil-plug lacks receiving bore depth')
    from .roller_validation import bearing_face
    gap,area=bearing_face(shaft,plug)
    return dict(insertion_mm=length,radial_envelope_gap_mm=hole_radius-shank_radius,
                head_contact_area_mm2=area,minimum_gap_mm=gap,
                shoulder_seat_qualified=False,thread_retention_qualified=False,sealing_qualified=False)
...
def validate(data,items,out):
    ...
    for hand in ['Port','Starboard']:
        ...
        plugs.append(dict(shaft=shaft+'Shaft',plug=shaft+'OilPlug',
                          **plug_envelope(by_id[shaft+'Shaft']['shape'],by_id[shaft+'OilPlug']['shape'],pose,a)))
        for n in range(6):
    ...
    report=dict(applicable=True,source_composition=source,bearing_faces=seats,receiving_hull_bores=bores,
        clearances=gaps,oil_leads=oil,oil_plug_envelopes=plugs,controlled_dimensions=dimensions,rivet_stock=stocks,

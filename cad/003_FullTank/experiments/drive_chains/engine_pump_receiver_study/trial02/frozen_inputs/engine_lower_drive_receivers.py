...
def revise(original, c, unit, apex, driver_controls, progress=None):
    ...
    lug_low = lo+c['lug_lower_end_inset']
    ...
    # Oil enters the radial housing ports from above the top of the receiving
    # lug. Leave an estimated open annular recess around this exposed collar.
    s = s.cut(Part.makeCylinder(radius+c['oil_entry_radial_relief'], top-lug_high+1,
                                V(apex, 0, lug_high), Z))
    ...
    # The cast screw-support cone was added after the initial pump cavity.
    # Machine through that stock too; an older candidate obstructed this bore.
    s = s.cut(Part.makeCylinder(c['pump_open_radius'], px1-px0+2,
                                V(px0-1, 0, pz), X))
    audit('receiver_machined', s)
    tolerance_before = s.getTolerance(1)
    cleanup_tolerance = None
    refinement = 'retained valid shape before optional cleanup'
    try:
        cleaned = s.copy().removeSplitter()
        cleanup_tolerance = cleaned.getTolerance(1)
        if (cleaned.isValid() and len(cleaned.Solids) == 1 and
                cleanup_tolerance <= tolerance_before+1e-10):
            s = cleaned
            refinement = 'unified faces without increasing maximum kernel tolerance'
        else:
            refinement = 'rejected optional cleanup: invalid topology or increased kernel tolerance'
    except Part.OCCError:
        refinement = 'optional cleanup raised OCCError; valid original retained'
    ...
    datums = dict(lug_axis=[apex, 0, 0], lug_span=[lug_low, lug_high],
                  ...
                  refinement=refinement, tolerance_before_cleanup_mm=tolerance_before,
                  attempted_cleanup_tolerance_mm=cleanup_tolerance)
    return s, datums

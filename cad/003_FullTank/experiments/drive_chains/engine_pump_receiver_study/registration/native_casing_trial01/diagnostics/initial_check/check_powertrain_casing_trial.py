...
joint_checks=[]
def bore_and_seats(name,tool,receivers,fastener):
    ...
    passed=abs(remaining)<1e-5 and max(distances)<1e-6 and abs(overlap)<1e-5
    ck(name+' bore and bearing contact',passed,remaining_mm3=remaining,gaps_mm=distances,overlap_mm3=overlap)
...
for hand,cy in r['centers'].items():
    ...
    for s in details['cap_stations']['rivets']:
        ...
        if s.get('packing'):receivers.append(shape(hand+'CasingCap_'+s['packing']))
        bore_and_seats(name,translated(cutter(s,cap['rivet_diameter']+cap['hole_diameter_clearance']),cy),receivers,shape(name))

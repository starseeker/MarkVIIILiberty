...
ck('All saved definition BReps and source metadata unchanged',
   len(definitions) == len(manifests['source']['definitions']) and
   all(v['exact_brep'] and v['metadata_equal'] for v in definitions),
   definitions=len(definitions), failures=[v for v in definitions if not v['exact_brep'] or not v['metadata_equal']])
...
floor = ['EngineContext_hull_floor_5', 'EngineContext_hull_floor_6', 'hull_floor_7']
floor_rows = []

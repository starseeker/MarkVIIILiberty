"""Export and strictly compare three support definitions and all 61 occurrences."""
import argparse
...
definitions = {v: definition(v) for v in r['replacement_definitions'].values()}
installed = {}
for record in r['support_occurrences']:
    name = record['name']
    row = rows[name]
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('EngineSupport' + scope + '.step')
    ...
    for name, one in shapes.items():
        ...
        i, (index, two) = min(enumerate(available), key=lambda pair:
            (pair[1][1].CenterOfMass - one.CenterOfMass).Length + abs(pair[1][1].Volume - one.Volume) / max(one.Volume, 1))
        available.pop(i)
        ...
        row = dict(scope=scope, name=name, imported_solid_index=index,
                   ...
                   centroid_error_mm=(one.CenterOfMass - two.CenterOfMass).Length,
                   passed=one.isValid() and two.isValid() and len(one.Solids) == len(two.Solids) == 1 and
                          ...
                          (one.CenterOfMass - two.CenterOfMass).Length < 1e-5)
        results.append(row)
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='Three rebuilt support definitions and all 61 support occurrences; not a full-tank export.',

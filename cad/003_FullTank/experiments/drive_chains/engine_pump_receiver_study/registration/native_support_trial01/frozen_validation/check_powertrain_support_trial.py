...
def bearing_area(one, two, z):
    # Solid/solid common discards touching boundary faces on this runtime.
    # Intersect the actual coplanar faces, retaining the same area criterion.
    def faces(shape):
        return [f for f in shape.Faces if type(f.Surface).__name__ == 'Plane' and
                abs(f.BoundBox.ZMin - z) < 1e-7 and abs(f.BoundBox.ZMax - z) < 1e-7]
    return sum(left.common(right).Area for left in faces(one) for right in faces(two))
...
for side, sign in [('Left', 1), ('Right', -1)]:
    ...
    area = bearing_area(rail, shape('EngineCase_upper'), axis)
    ck(side + ' actual flange bearing contact', area > 1000 and abs(common.Volume) < 1e-5,
       contact_area_mm2=area, common_mm3=common.Volume)
    lifted = shape('EngineCase_upper').copy()
    lifted.translate(V(0, 0, .01))
    ck(side + ' bearing negative control rejects 0.01mm lift',
       bearing_area(rail, lifted, axis) < 1000 and rail.distToShape(lifted)[0] > .009)

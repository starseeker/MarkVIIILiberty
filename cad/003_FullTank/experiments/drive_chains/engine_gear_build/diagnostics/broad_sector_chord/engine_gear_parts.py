...
def parts(c, parent, original_shaft, original_nut, progress=None):
    ...
    reach = 2*c['hub_radius']
    a = math.radians(c['claw_angle_deg'])
    points = [V(hub_end-.2, 0, 0), V(hub_end-.2, reach, 0),
              V(hub_end-.2, reach*math.cos(a), reach*math.sin(a))]
    wedge = Part.Face(Part.makePolygon(points+points[:1])).extrude(V(c['claw_stock']+.4, 0, 0))
    dog = claw_ring.common(wedge)
    for n in range(c['claw_count']):
        s = dog.copy(); s.rotate(V(), X, n*360/c['claw_count'])
        gear = gear.fuse(s)

...
def case_joint_parts(c, old_case, old_cover):
    ...
    left=mating_faces(case,-gap/2)
    right=mating_faces(cover,gap/2)
    left_stock=Part.makeCompound([f.extrude(App.Vector(gap,0,0)) for f in left.Faces])
    right_stock=Part.makeCompound([f.extrude(App.Vector(-gap,0,0)) for f in right.Faces])
    # Compare the two main mating regions separately; disconnected brake-cap
    # faces belong to a different joint and are not gasket material.
    seals=[]
    for right_solid in right_stock.Solids:
        left_solid=min(left_stock.Solids,key=lambda s:(s.CenterOfMass-right_solid.CenterOfMass).Length)
        result=left_solid.common(right_solid)
        print('Seal intersection',left_solid.Volume,right_solid.Volume,result.Volume,len(result.Solids),flush=True)
        print('Test',left_solid.isInside(App.Vector(0,40,218),1e-7,False),right_solid.isInside(App.Vector(0,40,218),1e-7,False),
              str(left_solid.CenterOfMass),str(right_solid.CenterOfMass),str(left_solid.BoundBox),str(right_solid.BoundBox),flush=True)
        seals.extend(result.Solids)

...
try:
    ...
    for suffix in ['Upper','Lower']:
        ...
        # Probe either side of the sealing sheet; nearly all area must be backed
        # by each casting. Finite 0.001mm probe allows the curved wall to slope.
        for label,s,delta in [('case',case,-.001),('cover',cover,.001)]:
            probe=g.copy();probe.translate(App.Vector(delta,0,0))
            fraction=probe.common(s).Volume/(g.Volume/.3*.001)
            check(suffix+' seal backed by '+label,fraction>.999)
        for y,z in axes:

...
try:
    ...
    for name in preserved:
        ...
        assert now['target'].Name==row['target'],name
        if old_hash[row['target']]!=new_hash[row['target']]:byte_changes.add(row['target'])
        dt,angle=placement_errors(row['placement'],now['shape'].Placement)
        assert dt<1e-7 and angle<1e-8,name
    # Save/recompute can normalize last-bit directions and topology flags.
    # Record every byte change for independent material comparison; never edit
    # archive internals to restore hashes or infer preservation from shape names.
    oil_hash=serialized_shapes(paths['oil'])
    oil_byte_changes=[key for key,name in definition_names.items() if oil_hash['Def_'+key]!=new_hash[name]]
    report=dict(status='development_hierarchy_global_installation_unqualified',native_file=native.name,native_sha256=sha(native),
        ...
        parent_definition_byte_changes=sorted(byte_changes),oil_definition_byte_changes=oil_byte_changes,
        material_preservation_verified=False,
    ...
    write(out/'report.json',report);print('Saved2393occurrences;2152parentframespreserved; independent material checks pending',flush=True)
finally:
    runtime.close()

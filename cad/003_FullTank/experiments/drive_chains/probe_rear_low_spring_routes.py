"""Test provisional short-brake rod cores and M564 spring corridors near M4136.
...
for route in packet['corrected_route_identities']:
    ...
    b = r['controls']['bracket']
    base = V(*r['controls']['stations'][side])
...
assert sha(receiver_native)==r['receiver_native_sha256']==receiver['native_sha256']
receiver_rows=[v for v in receiver['occurrences'] if v['name']!='RearControlChannelStock']
for name,one in envelopes.items():
    for row in receiver_rows:
        two=world(row,receiver)
        if not overlaps(bounds(one),bounds(two)):continue
        common=one.common(two);occupied=sum(abs(v.Volume) for v in common.Solids)
        pairs.append(dict(first=name,second=row['name'],origin='high_spring_prototype',common_mm3=occupied,
                          passed=(common.isNull() or common.isValid()) and occupied < 1e-5))
validate_native_bindings(dict(native_file=str(receiver_native),render_occurrences=[v['name'] for v in receiver_rows],landmarks=[]),receiver)
...
write(out/'route_checks.json', dict(passed=all(v['passed'] for v in pairs), routes=routes, pairs=pairs,
    ...
    standard_manifest_sha256=sha(standard_path), probe_sha256=sha(Path(__file__)),source_packet_sha256=sha(C/'spring_sources01/sources.json'),interface_probe_sha256=sha(parent.parent/'operating_interfaces.json'),receiver_native_sha256=sha(receiver_native),

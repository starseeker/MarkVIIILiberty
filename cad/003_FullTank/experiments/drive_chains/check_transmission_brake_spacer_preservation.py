...
the new spacer and revised spring.
"""
import argparse
...
assert changed == {'Def_BrakeFront_spring'}
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 502 unchanged inherited definitions compared with the brake-stop trial04 parent. The shortened spring and added spacer are checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 502

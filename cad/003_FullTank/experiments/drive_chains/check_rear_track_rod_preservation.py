"""Compare every inherited definition against the saved pre-rod powertrain parent."""
import argparse
...
assert changed == {'Def_BrakeFront_lever', 'Def_RearControlFulcrum_lever_track'}
assert len(source_manifest['definitions']) == 567
assert len(report['new_definitions']) == 1
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All565 unchanged inherited definitions preserved. Two revised receiver definitions and the new rod match the independently tested prototype; seven-definition and24-installed-shape transfer is checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 565

...
try:
    ...
    expected=Counter()
    for rid,count in [('SNL:17:016',1),('SNL:217:024',1),('SNL:67:018',1),('SNL:114:024',4),('SNL:165:001',1),('SNL:67:020',1)]:
        for pid in records[rid]['part_ids']:expected[pid]+=count
    ck('1549 unique physical leaves',len(byid),1549);ck('nine added physical pieces',len(r['new_ids']),9)
    ck('ten affected valid single solids',all(s[n].isValid() and len(s[n].Solids)==1 for n in r['affected_ids']))
    ...
    collar=s['ClutchCollar_collar'];bearing=s['ClutchStack_bearing'];sleeve=s['ClutchStack_sleeve'];support=s['ClutchStack_support'];snap=s['ClutchStack_snap'];thrust=s['ClutchStack_thrust']
    ...
    gap('bearing front seats against thrust collar28',bearing,thrust)
    gap('external snap ring30 seats in receiving groove',snap,collar)
    gap('thrust collar seats on front collar face',thrust,collar)
    gap('external ring remains separate from thrust collar',snap,thrust,1.0)
    zero('unsupported internal snap groove removed',xc(78,998,1000).cut(xc(76.1,997,1001)).cut(collar))
    zero('actual external ring groove exists',xc(89,988,990).cut(xc(86.1,987,991)).common(collar))
    ...
    ck('bearing forward displacement hits thrust collar',moved(bearing,[.2,0,0]).common(thrust).Volume>.01)
    ck('thrust collar aft displacement hits bearing',moved(thrust,[-.2,0,0]).common(bearing).Volume>.01)
    ...
    theta=math.radians(c['snap_gap_angle']);point=App.Vector((c['snap_rear']+c['snap_front'])/2,89*math.cos(theta),89*math.sin(theta))
    ck('snap ring has an actual split',not snap.isInside(point,1e-6,True))
    ck('snap ring is one physical piece',len(snap.Solids),1)
    ck('external ring has independent three millimetre axial stock',snap.BoundBox.XLength,3.0)
    ck('external ring provides a forward support stop',moved(support,[7.2,0,0]).common(snap).Volume>.01)
    ...
    assert sha(path)==r['step_sha256'] and assembly.isValid() and len(assembly.Solids)==10
    ...
    write(out/'assembly_exchange_checks.json',dict(passed=all(x['passed'] for x in placed),checks=placed,native_sha256=sha(native),step_sha256=sha(path),
        scope='10affected placed solids:validity,count,mass and centroid;7definitions checked separately for geometry.',checker_sha256=sha(Path(__file__))))

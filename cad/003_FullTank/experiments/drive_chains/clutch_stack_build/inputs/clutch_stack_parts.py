...
def build(c, parent_controls, parent_collar):
    ...
    snap = xc(c['main_bore_radius'], c['bearing_front'], c['snap_front'])
    snap = snap.fuse(xc(c['snap_ridge_radius'], c['snap_ridge_rear'], c['snap_ridge_front']))
    snap = snap.cut(xc(c['bearing_front_bore_radius'], c['bearing_front']-1, c['snap_front']+1))
    opening = box(c['bearing_front']-1, c['snap_front']+1, 0,
        c['snap_ridge_radius']+1, c['snap_gap_width']/2)
    opening.rotate(App.Vector(), App.Vector(1,0,0), c['snap_gap_angle'])
    snap = snap.cut(opening).removeSplitter()
    collar = collar.cut(xc(c['snap_ridge_radius'], c['snap_ridge_rear'], c['snap_ridge_front'])).removeSplitter()
    parts = dict(bearing=bearing, sleeve=sleeve, support=support.removeSplitter(), key=key, snap=snap)
    for key_name in ['bearing','sleeve','support','snap']:

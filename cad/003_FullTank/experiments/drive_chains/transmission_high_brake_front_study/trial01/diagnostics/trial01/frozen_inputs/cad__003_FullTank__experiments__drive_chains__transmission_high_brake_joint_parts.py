...
def parts(c,bc):
    ...
    for role in ['long','short']:
        sweep=base[role]['sweep_rad'];extra=bc['steel_terminal_margin']/neutral
        start,end=(-extra,sweep+tail) if role=='long' else (-tail,sweep+extra)
        # Make the extended stock as one annular profile. Fusing overlapping
        # short sectors left split coincident supports whose later cone cut
        # generated valid-looking material outside the stock in the retained
        # trial01. This profile retains the intended arc, width and bore pattern.
        s=sector(ri,ro,start,end,width)
        tools=[]
        for hole in base[role]['holes']:
            tool=Part.makeCylinder(bc['steel_hole_radius'],bc['lining_stock']+bc['steel_stock']+4,V(0,0,-2))
            tool.Placement=App.Placement(App.Matrix(*hole['frame']));tools.append(tool)
    ...
    for column,along in enumerate(c['attachment_columns_mm']):
        for row,y in enumerate(c['attachment_rows_mm']):
            ...
            # The cutter is rotationally symmetric. Move its parametric seam
            # around the radial axis without changing its material; the default
            # seam produced an invalid/out-of-stock cut on this thin cylinder.
            # Retained probes compare this construction and direct-axis cutters.
            local=tool.copy();local.Placement=App.Placement(V(),rotate_theta(short)).inverse().multiply(pose).multiply(
                App.Placement(V(),App.Rotation(V(0,0,1),90)))
            short_tools.append(local)
    ...
    shapes['short_band']=shapes['short_band'].cut(Part.makeCompound(short_tools))

...
p=argparse.ArgumentParser();p.add_argument('--candidate',type=Path,default=HERE/'engine_suspension_build');p.add_argument('--worker',action='store_true');p.add_argument('--views',nargs='+',choices=['isometric','suspension','front_mount','rear_mount','bevel_section','front_end','side_view'])
a=p.parse_args();base=a.candidate.resolve();out=base/'source_review';out.mkdir(exist_ok=True)
if not a.worker:
    with (out/'run.log').open('w') as log:
        sys.exit(subprocess.run([sys.executable,__file__,'--candidate',str(base),'--worker']+(['--views']+a.views if a.views else []),env=runtime.environment(out),stdout=log,stderr=subprocess.STDOUT).returncode)
try:
    ...
    previous=read(out/'render_receipt.json') if a.views else None
    if previous:
        assert previous['native_sha256']==sha(native)
        assert all(sha(out/n)==digest for n,digest in previous['images'].items())
    prior_receipt_sha=sha(out/'render_receipt.json') if previous else None
    ...
    write(out/'render_receipt.json',dict(native_sha256=sha(native),prior_render_receipt_sha256=prior_receipt_sha,refreshed_views=a.views or 'all',renderer_sha256=sha(Path(__file__)),images={n+'.png':sha(out/(n+'.png')) for n in names},source_sha256=sha(source),source_crop=[5440,1980,7250,3270],projected_occurrences=projection,

...
def side_gauze(ro,stock,low,high,count,rows,rib,cap):
    # Trim the gauze edge to its conical supporting rim before perforating it.
    # A flat-ended cylinder would penetrate the sloping end frame.
    shell=ring(ro,ro-stock,low,high).cut(cap);tools=[];step=(high-low)/rows
...
def extend(c,p,occ,groups,d,progress=None):
    ...
    for level in ['upper','lower']:
        if level=='upper':
            ...
            mesh_inner=0.
        cap=rev_z(profile);end=cap.copy()
        ...
        p[level+'_side_screen']=side_gauze(inside,mesh_stock,low+stock/2,high-stock/2,c['screen_circumference_cells'],c['screen_height_cells'],c['screen_rib_width'],cap);add(level+'_side_screen',level.title()+'SideScreen')

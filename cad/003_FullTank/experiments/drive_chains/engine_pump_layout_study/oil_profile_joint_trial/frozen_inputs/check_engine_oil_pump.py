...
try:
    ...
    if 'fastener_joints' in d:
        expected_counts={'upper_body_bolt':4,'cover_body_bolt':10,'fastener_nut':14,'fastener_washer':28,'fastener_cotter':13}
        for key,count in expected_counts.items():
            actual=sum(row['key']==key and row['assembly']=='EngineOilPumpFasteners' for row in r['occurrences']);check('hardware_count_'+key,actual==count,expected=count,actual=actual)
        for level,length,grip in [('upper',1.3125*25.4,.875*25.4),('cover',.9375*25.4,.5*25.4)]:
            shape=defs[level+'_body_bolt'];check(level+'_source_bolt_length',abs(shape.BoundBox.XMax-length)<1e-7,source_mm=length,actual_mm=shape.BoundBox.XMax)
            joint=d['fastener_joints'][level]
            for index,(x,y) in enumerate(joint['centers'],1):
                prefix='EngineOilPump_'+level.title()+'Joint'+str(index)
                bolt=doc.getObject(prefix+'Bolt');nut=doc.getObject(prefix+'Nut');span=(bolt.LinkPlacement.Base-nut.LinkPlacement.Base).Length
                check(prefix+'_source_grip',abs(span-grip)<1e-7,source_mm=grip,actual_mm=span)
                if level=='upper':
                    gauge=Part.makeCylinder(c['bolt_diameter']/2+c['bolt_hole_gap'],length,V(x,y,joint['head_bearing_z']-length),V(0,0,1))
                    vols={name:abs(gauge.common(path).Volume) for name,path in passages.items()}
                    check(prefix+'_no_gallery_breakthrough',max(vols.values())<1e-5,bore_intersections_mm3=vols)
            # Cotter cross-hole remains genuinely open through the source-length
            # shank; whole-assembly material checks examine the fitted split pin.
            point=V(joint['cotter_station_from_head_mm'],-10,0);gauge=Part.makeCylinder(.03125*25.4,20,point,V(0,1,0));vol=abs(gauge.common(shape).Volume)
            check(level+'_cotter_cross_hole',vol<1e-5,material_mm3=vol)
        seat=c['lower_body_bottom']+c['bottom_flange_stock'];height=c['washer_stock']+c['fastener_head_stock']+.5
        inner=c['lower_body_radius']-c['lower_body_wall']+.5;outer=inner+2.5
        witness=Part.makeCylinder(outer,height,V(0,0,seat)).cut(Part.makeCylinder(inner,height+2,V(0,0,seat-1)))
        missing=abs(witness.cut(defs['lower_body']).Volume)
        check('cover_head_seats_retain_continuous_wall',missing<1e-5,missing_mm3=missing,witness_radial_stock_mm=2.5)
        upper_cotters=[row['name'] for row in r['occurrences'] if row['name'].startswith('EngineOilPump_UpperJoint') and row['key']=='fastener_cotter']
        check('HB87_three_cotters_one_wire_reservation',len(upper_cotters)==3 and not doc.getObject('EngineOilPump_UpperJoint'+str(c['upper_wire_locked_bolt'])+'Cotter'),cotters=upper_cotters,wire_corner_estimate=c['upper_wire_locked_bolt'])
    ...
    if 'relief_lock' in d:
        from check_oil_pump_lock import check_lock
        check_lock(doc,r,defs,shapes,passages,check)
    if 'mounting' in d:
        from check_oil_pump_mounting import check_mounting
        check_mounting(doc,r,defs,shapes,passages,check)
    check('native_unchanged',sha(native)==r['native_sha256'])
    result=dict(passed=all(x['passed'] for x in checks),complete_pump=False,native_sha256=sha(native),checker_sha256=sha(Path(__file__)),checks=checks,material_pairs=pairs,limitations=['Pump-to-crankcase and standard tank fit not yet checked','Hydraulic behavior and true screen mesh not simulated','External fittings, mounting fasteners and lock wires remain pending'])
    if 'relief_lock' in d:
        result['lock_checker_sha256']=sha(HERE/'check_oil_pump_lock.py')
        result['limitations'][-1]='External fittings, mounting fasteners and two other source lock wires remain pending'
    if 'mounting' in d:
        result['mount_checker_sha256']=sha(HERE/'check_oil_pump_mounting.py')
        result['limitations'][-1]='External fittings, two other wires, receiving-case engagement and mounting fit remain pending'

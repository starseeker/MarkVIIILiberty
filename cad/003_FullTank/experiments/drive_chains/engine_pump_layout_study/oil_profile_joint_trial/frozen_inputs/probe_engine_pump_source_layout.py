...
Select a saved oil-pump candidate and compare the coupled printed levels. The
default retains the original estimated pump for the initial diagnostic. The
...
p.add_argument('--oil-candidate',type=Path,default=HERE/'engine_oil_pump_mounting_study')
p.add_argument('--output',type=Path)
p.add_argument('--render',action='store_true')
a=p.parse_args();study=a.candidate.resolve();out=(a.output or study).resolve();out.mkdir(parents=True,exist_ok=True)
source=read(study/'source_constraints.json')
oil_dir=a.oil_candidate.resolve()
water_dir=HERE/'engine_water_pump_connections_study'
driver_dir=study/'lower_drive_trial'
...
try:
    ...
    result=dict(status='Coupled source layout diagnostic; receiving case and global installation unqualified',
        source_constraints_sha256=sha(study/'source_constraints.json'),checker_sha256=sha(Path(__file__)),
        ...
        limitations=['Oil-pump geometry comes from the explicitly hashed native candidate; this diagnostic does not requalify its internal material or source accuracy.',

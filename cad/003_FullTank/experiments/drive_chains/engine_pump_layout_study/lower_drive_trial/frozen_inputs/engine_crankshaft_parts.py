...
def parts(c,cc,taper,originals,progress=None):
    ...
    # The gear flange can move beyond the bearing endplay envelope; the last
    # journal neck must extend with it to keep one continuous forging.
    journals[-1]=(journals[-1][0],rows[-1]+c['gear_flange_back'])
    ...
    def keyshape(width,extra=0):
        ...
        blank=polygon(pts,V(0,0,width))
        # Rounded milled ends follow LIB12, in the X/Z plan of the keyway.
        lo=ka-extra;hi=kb+extra;r=width/2;y0=blank.BoundBox.YMin-1;y1=blank.BoundBox.YMax+1
        mask=box(lo+r,hi-r,y0,y1,-r,r)
        for x in [lo+r,hi-r]:mask=mask.fuse(Part.makeCylinder(r,y1-y0,V(x,y0,0),Y))
        return blank.common(mask)
    ...
    for label,length in [('long',long),('short',short)]:
        for sign,word in [(1,'upper'),(-1,'lower')]:
            ...
            audit('raw '+label+'_'+word,shell)
            try:
                # OCC unification can mutate its input even when it returns
                # another wrapper. Isolate it from the valid raw solid.
                refined=shell.copy().removeSplitter()
                if refined.isValid() and len(refined.Solids)==1:
                    note='coplanar faces unified on independent copy'
                else:
                    refined=shell;note='unification returned invalid topology; checked original BRep retained'
            except Part.OCCError:
                refined=shell;note='unification raised OCCError; checked original BRep retained'
            d.setdefault('bearing_refinement',{})[label+'_'+word]=note
            audit('selected '+label+'_'+word,refined)
            p[label+'_'+word]=refined
    ...
    # The receiving bore belongs to the finished casting, including inherited
    # nose material. Cutting only the added annulus leaves two old sump lobes
    # intruding into the front sleeve seat.
    receiver=cyl(c['thrust_sleeve_outer_radius']+c['thrust_seat_gap'],sleeveleft,sleeveright)
    upper=upper.cut(receiver);lower=lower.cut(receiver)
    assert c['thrust_seat_radius']-c['thrust_sleeve_outer_radius']-c['thrust_seat_gap']>=cc['case_wall']
    ...
    for i in range(c['thrust_balls_per_row']):
        ...
        # Align the spherical parametrization with the axial trimming planes.
        # Default Z-axis pockets cross poles and fail STEP reopening on OCC7.8;
        # X-axis latitude trims preserve the same spherical clearance geometry.
        cage=cage.cut(Part.makeSphere(ball+c['thrust_cage_ball_gap'],pos,X))

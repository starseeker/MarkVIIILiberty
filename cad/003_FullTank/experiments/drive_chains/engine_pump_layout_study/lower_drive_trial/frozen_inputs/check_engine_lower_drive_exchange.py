"""Compare saved lower-drive definitions and installed STEP solids in both material directions."""
import argparse
import gc
import math
...
p=argparse.ArgumentParser();p.add_argument('--candidate',type=Path,default=HERE/'engine_lower_drive_study');p.add_argument('--worker',action='store_true')
...
try:
    ...
    native=out/'DrivetrainWithLowerDriveStudy.FCStd';nh=sha(native);r=read(out/'report.json');assert r['native_sha256']==nh
    doc=App.openDocument(str(native));byid={i['id']:i for i in leaves(doc.Root)}
    # Retain actual saved-native BReps/poses for all 27 comparisons, then release
    # the other 2153 occurrences before importing STEP and running Booleans.
    # Selection and every strict comparison criterion below remain unchanged.
    expected_sets=[('Definitions',[(k,doc.getObject(k).Shape.copy()) for k in read(out/'definition_order.json')]),
                   ('Installation',[(n,byid[n]['shape'].copy()) for n in r['new_ids']])]
    del byid
    App.closeDocument(doc.Name);del doc;gc.collect()
    mass=calculator(out/'exchange_runtime/mass');exchange=[]
    for label,expected in expected_sets:
        path=out/('LowerDrive'+label+'.step');loaded=Part.Shape();loaded.read(str(path))
    ...
    write(out/'exchange_checks.json',dict(passed=all(x['passed'] for x in exchange),native_sha256=nh,checker_sha256=sha(Path(__file__)),
        artifact_hashes={n:sha(out/n) for n in ['LowerDriveDefinitions.step','LowerDriveInstallation.step']},checks=exchange))

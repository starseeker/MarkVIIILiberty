...
def extend(c,p,occ,groups,d):
    ...
    lo=lo.fuse(cylinder(c['lower_body_radius']-c['lower_body_wall']+.5,c['lower_filter_top'],c['lower_filter_top']+c['lower_filter_ceiling_stock']))
    ...
    pressure=[(12,-space/2,-9),(c['pressure_route_x'],-space/2,-9),(c['pressure_route_x'],c['pressure_route_y'],-9),(c['pressure_turn_x'],c['pressure_route_y'],-9),(c['pressure_port_x'],c['pressure_port_y'],-9),(c['pressure_port_x'],c['pressure_port_y'],1)]
    front_y=c.get('front_route_y',28.)
    front=[(c['return_inlet_port_x'],c['return_inlet_port_y'],1),(c['return_inlet_port_x'],c['return_inlet_port_y'],-22),(-40,front_y,-22),(30,front_y,-22),(30,space/2,-22),(30,space/2,14),(13,space/2,14)]
    supply=[(-30,-space/2,-40),(-30,-space/2,-9),(-13,-space/2,-9)]
    outlet=[(-13,space/2,14),(-30,space/2,14),(-30,space/2,-9),(c['return_route_x'],40,-9),(c['return_route_x'],c['return_route_y'],-9),(0,c['return_route_y'],-9),(0,c['lower_body_radius']+4,-9)]
    ...
    # Start with filled stock, add the boss, then machine the complete dish once.
    # Re-cutting an already hollow revolved dish with its coincident cone left
    # duplicate conical patches and silently lost the subsequent drain bore on
    # OCC 7.8, despite a valid-solid result. The retained cover_trace reproduces it.
    cover=Part.makeCone(0,inner,dish,V(0,0,top-dish-thick)).fuse(cylinder(fr,top-thick,top))
    dx=c['drain_x'];seat_z=top-dish-thick-c['drain_boss_drop'];cover=cover.fuse(cylinder(c['drain_diameter']/2+6,seat_z,top,dx,0))
    cavity=Part.makeCone(0,inner,dish,V(0,0,top-dish)).fuse(cylinder(fr,top,top+5));cover=cover.cut(cavity)

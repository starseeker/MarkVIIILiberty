"""Export and strictly compare rear-anchor definitions and all 330 affected installed components."""
import argparse
import math
...
definitions = {name: definition(name) for name in set(r['changed_definitions']) | set(r['new_definitions'])}
installed = {}
installed_names = set(r['affected_occurrences'])
assert len(definitions)==12 and len(installed_names)==330
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('BrakeAnchors' + scope + '.step')
    ...
    for name, one in shapes.items():
        ...
        row = dict(native_mass=ma, step_mass=mb, adaptive_centroid_error_mm=dc, scope=scope, name=name, imported_solid_index=index,
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    step_hashes=files, mass_provenance=mass.provenance, export_settings={'write.surfacecurve.mode': 1},
    scope='Twelve new/changed definitions and all 330 affected occurrences; adaptive converged mass integration and unchanged strict material/tolerance criteria. Not a full brake installation or full-tank export.',

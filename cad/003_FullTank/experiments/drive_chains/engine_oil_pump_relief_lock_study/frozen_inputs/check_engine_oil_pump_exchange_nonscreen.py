"""Strict STEP comparison of nonscreen parts; dense-screen qualification remains pending."""
...
if not a.worker:
    with (out/'exchange_nonscreen_check.log').open('w') as log:
        sys.exit(subprocess.run([sys.executable,__file__,'--candidate',str(out),'--worker'],env=runtime.environment(out/'exchange_nonscreen_runtime'),stdout=log,stderr=subprocess.STDOUT).returncode)
try:
    ...
    mass=calculator(out/'exchange_nonscreen_runtime/mass');checks=[];deferred=[]
    for scope,filename,expected in [('Definitions','OilPumpDefinitions.step',[(key,doc.getObject('Def_'+key).Shape) for key in r['definition_order']]),('Assembly','OilPumpAssembly.step',installed)]:
        ...
        if not valid:
            write(out/'exchange_nonscreen_failure.json',dict(scope=scope,expected=len(expected),actual=len(loaded.Solids),invalid=[i for i,s in enumerate(loaded.Solids) if not s.isValid()]));raise ValueError(scope+' invalid STEP topology or count')
        ...
        for key,s in expected:
            ...
            deferred_keys={'upper_side_screen','upper_end_screen','lower_side_screen','lower_end_screen'}
            definition_key=key if scope=='Definitions' else doc.getObject(key).DefinitionKey
            if definition_key in deferred_keys:
                deferred.append(dict(scope=scope,part=key,definition_key=definition_key,reason='Dense screen comparison pending; no acceptance inferred from topology or export'))
                continue
            ...
            checks.append(dict(scope=scope,part=key,passed=passed,missing_mm3=missing.Volume,added_mm3=extra.Volume,mass_difference_mm3=dv,centroid_difference_mm=dc,native_max_tolerance_mm=ta,step_max_tolerance_mm=tb));write(out/'exchange_nonscreen_progress.json',checks);print(scope,key,passed,flush=True)
    result=dict(passed=all(x['passed'] for x in checks),complete_exchange=False,deferred=deferred,native_sha256=sha(native),checker_sha256=sha(Path(__file__)),checks=checks,step_hashes={n:sha(out/n) for n in ['OilPumpDefinitions.step','OilPumpAssembly.step']});write(out/'exchange_nonscreen_checks.json',result);assert result['passed']
finally:
    runtime.close()

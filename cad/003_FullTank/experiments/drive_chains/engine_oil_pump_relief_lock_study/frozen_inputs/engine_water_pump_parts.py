...
from engine_water_pump_lock import drain_lock
...
def parts(c,lower,mount_x,progress=None):
    ...
    for sign in [-1,1]:
        ...
        cast_length=c['outlet_cast_length'];transition=c['outlet_transition_length']
        assert abs(length-cast_length-c['outlet_tube_length'])<1e-7
        # HB191 gives the cast-in connection tube's OD, wall and stock length.
        # Its casting attachment station and the short taper remain estimates.
        for profile_kind,root_radius,end_radius in [('outer',c['outlet_cast_radius'],c['outlet_outer_radius']),
                                           ('inner',c['outlet_cast_bore_radius'],c['outlet_inner_radius'])]:
            branch=Part.makeCylinder(root_radius,cast_length-transition,start,direction)
            branch=branch.fuse(Part.makeCone(root_radius,end_radius,transition,start+direction*(cast_length-transition),direction))
            branch=branch.fuse(Part.makeCylinder(end_radius,c['outlet_tube_length']+(1 if profile_kind=='inner' else 0),start+direction*cast_length,direction))
            if profile_kind=='outer':body=body.fuse(branch)
            else:cavity=cavity.fuse(branch)
    ...
    # and open straight hose end. HB191 says mark12081 is cast with cover12435.
    ...
    inner_profile=[(ce,c['chamber_radius']),(ce+3,57),(ce+12,48),(ce+19,31),(neck,c['inlet_od']/2-c['inlet_cast_wall'])]
    curves=[]
    ...
    tubes=[]
    for radius,tube_radius in [(c['inlet_od']/2,c['inlet_od']/2),
                              (c['inlet_od']/2-c['inlet_cast_wall'],c['inlet_od']/2-c['inlet_wall'])]:
        # Same quarter-circle elbow as the retained centreline. Explicit torus
        # parametrization preserves strict STEP comparison in definition and
        # installed frames; the generic arc sweep failed in the local frame.
        elbow=Part.makeTorus(bend,radius,V(),Z,-180,180,90)
        elbow.Placement=App.Placement(V(neck,0,-bend),App.Rotation(Z,X,Y,'ZXY'))
        tubes.append(elbow.fuse(Part.makeCylinder(tube_radius,c['inlet_stub_length'],finish,-Z)))
    for i in range(c['hose_bead_count']):
    ...
    p['lock_wire'],drill,wire_path,d['drain_lock']=drain_lock(c,cover_centers)
    p['plug']=p['plug'].cut(drill)
    add('lock_wire','DrainLockWire',parent='EngineWaterPumpBodyAssembly')

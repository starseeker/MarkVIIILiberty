...
expected=r['expected_new_occurrences'];moved=r['expected_repositioned_occurrences'];newdefs=set(r['new_definitions'])
ck('Exactly six support occurrences repositioned',set(moved)=={hand+role+suffix for hand in ['Port','Starboard'] for role,suffix in [('LowSpeed','Bracket'),('Track','Bracket'),('Track','Stop')]})
...
for name,row in rows.items():
    target=expected.get(name,moved.get(name,prior.get(name)));delta=error(row['frame'],target['frame'])
...
affected=set(expected)|set(moved);all_shapes={n:shape(n) for n in rows}
def bounds(s):

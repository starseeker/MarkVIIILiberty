...
outer=band['track']['inner_radius']+band['lining_stock']+band['steel_stock']
d=dimensions(c,support,upper,interface['center_mm'],outer);shapes,cotter=parts(c,d)
...
locked=sorted(set(loaded+[Path(__file__),a.controls.resolve(),packet,support_path,STAGE/'lib/cad_build.py',STAGE/'lib/evidence.py',
    parent/'report.json',parent/'independent_checks.json',parent/'definition_preservation_checks.json',parent/'exchange_adaptive_checks.json',parent/'../controls.json']))
...
for role,mark,rids in [('link','M337',['SNL:119:023','HB:nomenclature:207:035']),
    ...
    metadata(body,DefinitionKey='brake_link_'+role,SourcePartMark=mark,SourceRecords=rids,SurveyIds=ids,
        ...
        ReconstructionStatus='Four separate pivot pins follow SNL137; earlier two-pin count unresolved. Link clevis, section and lower-eye station estimated; lower attachment pending.')
    defs[role]=body;records[role]=rids
expected={};interfaces={};added_children={}
for hand in ['Port','Starboard']:
    for role in ['LowSpeed','Track']:
        prefix=hand+role;row=rows[prefix+'Bracket'];pose=App.Placement(App.Matrix(*row['frame']))
        owner=doc.getObject(row['owners'][-1]);interfaces[prefix]=dict(bracket=prefix+'Bracket',upper_mm=list(pose.Base),
...
doc.Root.RegistrationStatus='Four M337 links retained at M338 brackets; four separate M339 pins are a source-selection hypothesis. Band anchor brackets and lower fastening pending.'
...
write(out/'report.json',dict(native_file=native.name,native_sha256=sha(native),source_native=str(source.relative_to(ROOT)),
    ...
    expected_new_occurrences=expected,added_children=added_children,affected_occurrences=sorted(expected),

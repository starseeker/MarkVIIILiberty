"""Saved link and support views with source overlays; display changes only."""
import argparse
import base64
...
COLORS.update(Link=(.75,.48,.24),Pin=(.72,.76,.8),Band=(.42,.50,.57),Lining=(.6,.46,.32),Copper=(.83,.48,.27),Frame=(.63,.55,.41),BrakeSupport=(.85,.55,.24),BrakeStop=(.42,.66,.72),Bearing=(.51,.59,.68))
...
for name,row in rows.items():
    if name in r['affected_occurrences']:
        role='Link' if name.endswith('SuspensionLink') else 'Pin' if 'SuspensionPin' in name or 'Cotter' in name else 'BrakeSupport'
...
clip=Part.makeBox(150,460,375,App.Vector(1440,400,860));detail=[];detail_context=[]
...
views=[('isometric',brakes+links,frame,(1,-.8,.55),'Transmission brakes | four inboard suspension links and retained pivots; lower anchors pending'),
       ('link_detail',detail,detail_context,(1,-1,.35),'Suspension detail | links beside the bands; forked upper eyes, pivot pins and split pins')]
...
for role,image_name,size,origin,sx,sz in [
    ...
    width,height=size;name=role.lower()+'_source_overlay';ids=['Port'+role+'Bracket','Port'+role+'SuspensionLink']+(['PortTrackStop'] if role=='Track' else [])
    ...
    svg=[f'<svg xmlns="http://www.w3.org/2000/svg" width="{width+80}" height="{height+210}">',
         ...
         f'<text x="35" y="36" font-family="sans-serif" font-size="24">HB {image_name[:-4]} | saved link/support outlines registered to the retained channels</text>',
         f'<image x="40" y="80" width="{width}" height="{height}" href="data:image/png;base64,{base64.b64encode(source.read_bytes()).decode()}"/>']
    for oid in ids:
        s=item(oid,'BrakeSupport')['shape'];color='#dd7000' if oid.endswith('Bracket') else '#bd2062' if oid.endswith('SuspensionLink') else '#007fa6'
        for edge in s.Edges:
    ...
    svg.extend([f'<text x="35" y="{height+130}" font-family="sans-serif" font-size="21">Orange: M338. Magenta: M337. Blue: M385. Green: shaft axis. Hidden sections and axial placement estimated.</text>',
        f'<text x="35" y="{height+165}" font-family="sans-serif" font-size="20">Original channel registration retained. Lower anchor station fitted to frame; source disagreement remains visible.</text></svg>'])
...
write(out/'render_receipt.json',dict(native_sha256=r['native_sha256'],manifest_sha256=sha(out/'isolated/manifest.json'),
    renderer_sha256=sha(Path(__file__)),images={name+'.png':sha(out/(name+'.png')) for name in ['isometric','link_detail',*overlays]},

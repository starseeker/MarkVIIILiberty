...
actual=lower_bores[0].Surface.Center;actual.y=(lower_bores[0].BoundBox.YMin+lower_bores[0].BoundBox.YMax)/2;near('Lower eye forward station',actual.x,d['lower_eye_relative_mm'][0]);near('Lower eye vertical station',actual.z,d['lower_eye_relative_mm'][2])
# The source side silhouette has an aft-set web between the round eyes.
# A saved curved-web material witness must exist away from the straight chord.
web_mid=V(*d['web_centerline_control_points_mm'][0])*.125+V(*d['web_centerline_control_points_mm'][1])*.375+V(*d['web_centerline_control_points_mm'][2])*.375+V(*d['web_centerline_control_points_mm'][3])*.125
ck('Bowed source-profile web retains middle material',link.isInside(web_mid,1e-7,False))
chord_x=d['lower_eye_relative_mm'][0]*.5
ck('Straight-web negative misses the bowed witness',abs(web_mid.x-chord_x)>c['web_width']/2)

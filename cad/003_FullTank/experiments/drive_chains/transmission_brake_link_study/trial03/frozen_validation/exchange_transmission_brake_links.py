"""Export and strictly compare brake link definitions and all twenty-two affected installed components."""
import argparse
...
assert len(definitions)==3 and len(installed_names)==22
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('BrakeLinks' + scope + '.step')
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='Three new definitions and sixteen installed linkage components and six repositioned supports; not a full brake installation or full-tank export.',

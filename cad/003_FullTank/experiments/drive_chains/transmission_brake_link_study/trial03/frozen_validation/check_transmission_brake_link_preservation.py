"""Compare all inherited brake-link definition material with its saved parent.
...
assert qualification['local_link_checks_passed']
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 471 inherited definitions compared with the qualified brake-band parent. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 471

...
try:
    ...
    from transmission_input_installation_parts import mounting,grease_feed,formed_pin,unspread_pin
    ...
    names=['transmission_input_installation_probe.py','transmission_input_installation_parts.py',
        ...
        'transmission_stud_controls.json','transmission_stud_research.json','transmission_input_controls.json','transmission_input_sources.json',
    ...
    repaired_shapes={'transmission_cap_cotter':formed_pin(c['prior_cap_cotter'])[0],
        'transmission_input_cotter':unspread_pin(c['prior_input_cotter'])}
    repaired_targets=set();repaired_ids=[]
    for i in before:
        if i['definition'] not in repaired_shapes:continue
        changed.append(i['id']);repaired_ids.append(i['id'])
        if i['target'].Name in repaired_targets:continue
        i['target'].Tip.Shape=repaired_shapes[i['definition']];repaired_targets.add(i['target'].Name)
        metadata(i['target'],GeometryRepair='Restore both cotter legs with analytic eye/leg junctions; source size and occurrence placement retained.',
            ParameterUpdate='Regenerate with transmission_input_installation_probe.py and prior cotter controls.')
    ...
    for n,(y,z) in enumerate(md['axes_yz_mm']):
        for key,x in [('mount_stud',0),('mount_nut',c['mount']['nut_seat']),('mount_cotter',md['cotter_station_mm'])]:
            link(mount,'InputInstallation_'+key+str(n),key,App.Placement(App.Vector(x,y,z),App.Rotation(App.Vector(1,0,0),90*n)))
    ...
    for row in prior['interfaces']:
        if row['a'] in changed or row['b'] in changed:gap(row['a'],row['b'],row['expected_mm'])
            ...
            interfaces.append(dict(row,evidence='unchanged geometry; retained from hash-bound parent report'))
            reused_interfaces+=1
    for n in range(4):
    ...
    for name in exchange_ids:
        ...
        # Do not modify stored tolerances. A serialization drift of1.35e-11mm
        # occurred on the input cotter despite exact zero material differences.
        # Declare an absolute1e-10mm reporting-roundoff allowance, separately
        # from the much larger bounded Boolean comparison tolerance.
        checks.append(dict(id=name,native_max_tolerance_mm=ta,step_max_tolerance_mm=tb,comparison_fuzzy_tolerance_mm=fuzzy,
            stored_tolerance_drift_mm=tb-ta,tolerance_reporting_roundoff_mm=1e-10,
            raw_missing_mm3=raw_missing,raw_added_mm3=raw_added,missing_mm3=missing.Volume,added_mm3=extra.Volume,
            passed=a.isValid() and v.isValid() and not missing.Faces and not extra.Faces and raw_missing<1e-5 and raw_added<1e-5 and ta<=.0001 and tb<=max(1e-7,ta)+1e-10))
    ...
    report=dict(complete=True,passed=passed,native_occurrences=len(items),new_occurrences=len(new_ids),new_definitions=len(definitions),
        ...
        repaired_prior_cotter_ids=repaired_ids,
        ...
        reused_prior_interfaces=reused_interfaces,rechecked_interfaces=len(interfaces)-reused_interfaces,

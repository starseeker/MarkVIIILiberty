"""Compare all unchanged brake-stop definitions with the qualified front parent.

This closes only the child-to-parent preservation gate. The parent's separate
direct-source qualification is still required; local reconstruction checks cover
the new stop definitions and two changed bearing castings.
"""
import argparse
...
qualification = read(out / 'mount_checks.json')
assert qualification['passed']
joints = read(out / 'joint_checks.json')
assert joints['passed'] and joints['native_sha256'] == report['native_sha256']
...
assert changed == {'Def_FixedBearing_inner_cap', 'Def_FixedBearing_inner_bracket'}
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              local_rebuild_qualification_sha256=sha(out / 'mount_checks.json'),
              joint_qualification_sha256=sha(out / 'joint_checks.json'),
              scope='All 490 unchanged inherited definitions compared with the qualified front-brake parent. Two bearing castings and four lower-band replacement occurrences checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 490

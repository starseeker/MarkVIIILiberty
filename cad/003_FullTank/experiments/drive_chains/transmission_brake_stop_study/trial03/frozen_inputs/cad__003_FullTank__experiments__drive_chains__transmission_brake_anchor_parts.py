...
def spring(c):
    ...
    width=c['spring_width'];stock=c['spring_stock'];stud=V(c['spring_stud_x'],0,c['spring_stud_z'])
    shape=strip(V(),stud,width,0,stock).multiFuse([
        Part.makeCylinder(c['spring_paddle_radius'],stock,V(),V(0,1,0)),Part.makeCylinder(width/2,stock,stud,V(0,1,0))])
    slot=Part.makeCylinder(c['spring_slot_radius'],stock+2,V(0,-1,0),V(0,1,0)).fuse(Part.makeBox(2*c['spring_slot_radius'],stock+2,30,V(-c['spring_slot_radius'],-1,-30)))
...
def parts(c,band_controls,link_report):
    ...
    for role in ['low','track']:
        ...
        inset=c['track_lug_inset'] if role=='track' else 0.
        lugy=(-1 if role=='low' else 1)*((width-c['lug_stock'])/2-inset);center=V(-anchor,0,0)
        ...
        stud=center+V(c['spring_stud_x'],0,c['spring_stud_z'])
        ...
        end_deg=math.degrees(theta1)
        locations=([(end_deg-offset,y) for offset in [14,10,6,2] for y in [-12.7,12.7]] if role=='low' else
                   [(end_deg-offset,y) for offset in [13,7] for y in [-25.4,0,25.4]]+[(end_deg-2,0)])
        ...
        spring_inner=width/2-inset+spacer
        groove_start=spring_inner-c['spring_groove_side_clearance'];groove_end=spring_inner+c['spring_stock']+c['spring_groove_side_clearance'];tip=groove_end+c['pin_tail_extension']
        ...
        ring=Part.makeCylinder(link['lower_pin_diameter']/2+1,groove_end-groove_start,V(0,groove_start,0),V(0,1,0)).cut(
            Part.makeCylinder(c['pin_groove_root_radius'],groove_end-groove_start+2,V(0,groove_start-1,0),V(0,1,0)))
        pin=pin.cut(ring)

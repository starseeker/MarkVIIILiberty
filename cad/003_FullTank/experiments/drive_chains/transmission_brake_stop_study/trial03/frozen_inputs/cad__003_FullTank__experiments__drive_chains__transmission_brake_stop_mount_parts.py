...
def revise(c,stop,hardware,shapes,rows):
    ...
    cap=old.cut(region)
    addition=plate([(c['split_half_gap'],cut_z),(c['cap_upper_join_x'],cut_z),
        (cap_seat,z+c['lower_boss_height_above_axis']),(cap_seat,floor),(c['split_half_gap'],floor)],-span,span)
    socket=Part.makeCylinder(c['socket_radius'],2*span+2,V(0,-span-1,0),V(0,1,0))
    cap=cap.fuse(addition).cut(socket)
    ...
    addition=plate([(c['lower_web_rear_x'],c['lower_web_rear_bottom_z']),
        ...
        (c['lower_web_knee_x'],floor)],-span,span)
    bracket=bracket.fuse(addition).cut(socket)

...
V=App.Vector
rows={r['name']:r for r in m['occurrences']}
...
assert len(new)==44 and len(revised)==10 and len(groups)==7
...
write(out/'report.json',dict(native_file=native.name,native_sha256=sha(native),source_native=str(source.relative_to(ROOT)),
    ...
    new_definitions=['Def_BrakeStop_'+k for k in shapes],changed_definitions=[],

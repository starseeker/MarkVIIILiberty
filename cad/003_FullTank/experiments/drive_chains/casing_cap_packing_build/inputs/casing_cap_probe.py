...
try:
    ...
    names = ['casing_cap_probe.py','casing_cap_parts.py','casing_cap_controls.json','casing_cap_fastener_sources.json','casing_trim_sources.json',
    ...
    for source in ['casing_cap_fastener_sources.json','casing_trim_sources.json']:
        for name,expected in read(ROOT/source)['inspected_source_hashes'].items():
            assert sha(REPO/name)==expected, 'Changed inspected source: '+name
    ...
    for role,grip in sorted(set((s['length_role'],s['grip']) for s in stations['rivets'])):
        key = role+'_grip'+str(grip).replace('.','_')
        rivet = button_rivet(doc,'Def_CapJoint'+key+'Rivet',a['rivet_diameter'],a[role+'_rivet_length'],grip)
        doc.Definitions.addObject(rivet)
        metadata(rivet,DefinitionId='casing_cap_'+key+'_rivet',Representation='assembly',Coverage='partial',
                 SurveyIds=identities[role],SourceRecord=records[role])
        definitions[key] = rivet
    centers = {}; added_ids = []
    for hand in ['Port','Starboard']:
        ...
        for s in stations['rivets']: link(s['name'],s['length_role']+'_grip'+str(s['grip']).replace('.','_'),s['center'],s['axis'])
    ...
    bores, seats, retention, packing_checks = [], [], [], []
    for hand,cy in centers.items():
        ...
        for s in stations['rivets']:
            ...
            if s.get('packing'):remaining += shape(s['packing']).common(tool).Volume
        ...
        for face in ['Inner','Outer']:
            packing = shape(face+'Packing')
            distances = [packing.distToShape(receiver)[0] for receiver in
                         [shape('Body'+face+'Cleat'),byid[hand+'Casing_Body']['shape']]]
            packing_checks.append(dict(side=hand,face=face,receiver_distances_mm=distances,passed=max(distances)<1e-6))
        for s in stations['bolts']:
    ...
    passed = not overlaps and all(row['passed'] for row in bores+seats+retention+packing_checks)
    report = dict(complete=True,passed=passed,fixture_occurrences=len(items),new_cap_joint_occurrences=len(new),
        ...
        bore_checks=bores,seat_checks=seats,retention_checks=retention,packing_checks=packing_checks,receiver_material_removal=removal_checks,

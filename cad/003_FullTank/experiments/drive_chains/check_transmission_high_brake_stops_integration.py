...
ck('Twenty new occurrences, seven definitions and eight assembly groups',len(rows)==3149 and len(m['definitions'])==539 and len(m['assemblies'])==332 and set(rows)-set(prior)==set(r['expected_new_occurrences']))
...
try:
 for role in ['anchor_bracket','anchor_pin','anchor_cotter','bottom_stop','bottom_lock_plate','mount_screw','stop_screw','stop_nut']:pshapes[role]=doc.getObject('Def_'+role).Shape.copy()
...
for role,one in pshapes.items():
 key='Def_HighBrakeSupport_anchor_bracket' if role=='anchor_bracket' else 'Def_HighBrakeStops_'+role
...
for hand in ['Port','Starboard']:mapping[hand+'HighSpeedBrakeAnchorBracket']=hand+'AnchorBracket'
...
dependencies=[]
for path in [prototype/'report.json',prototype/'independent_checks.json',prototype/'exchange_checks.json']:
...
write(out/'independent_checks.json',dict(passed=all(v['passed'] for v in checks),checks=checks,native_sha256=sha(native),source_native_sha256=sha(source),prototype_native_sha256=sha(pnative),checker_sha256=sha(Path(__file__)),prototype_checks=dependencies,
 scope='Saved counts/hierarchy, inherited occurrence frames, and strict material/placement transfer for the eight tested prototype definitions and22 occurrences. Whole-parent definition preservation and standard-tank context remain separate gates.',

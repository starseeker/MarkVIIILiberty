...
def tapered_bearing(c):
    ...
    for n in range(c['roller_count']):
        tool=Part.makeCone(radii[0]+c['cage_clearance'],radii[1]+c['cage_clearance'],t1-t0,origin,axis)
...
def input_stack(c,b,old_shaft,old_cover):
    ...
    washer=ring(c['washer_radius'],c['thread_radius']+c['spline_gap'],c['washer_start'],c['thread_start'])
...
def retention(c):
    ...
    nut=hex_x(c['nut_af'],c['thread_start'],c['nut_crown_start']).fuse(
        cylinder(c['nut_crown_radius'],c['nut_crown_start'],c['nut_end']))
    nut=nut.cut(cylinder(c['thread_radius']+c['spline_gap'],c['thread_start']-1,c['nut_end']+1))
    for a in [0,60,120]:

...
def build(c,cone_report,retention_report):
    ...
    head=hex_x(c['bolt_head_af'],0,c['bolt_head_height'])
    head.rotate(App.Vector(),App.Vector(1,0,0),c['bolt_head_clock'])
    screw=head.fuse(xc(c['bolt_diameter']/2,-c['bolt_length'],0)).removeSplitter()
    ...
    d.update(hub_rear=rear,spline_front=spline_end,spline_root=root,spline_tip=tip,journal_radius=journal,
        ...
        screw_insertion=c['bolt_length']-c['drum_stock'],thread_engagement=min(c['bolt_length']-c['drum_stock'],c['bolt_thread_length']),
        plain_shank_in_flywheel=max(0,c['bolt_length']-c['bolt_thread_length']-c['drum_stock']),
        blind_back_wall=rim_front-(d['bolt_seat']+c['bolt_length']+c['blind_tip_gap']))

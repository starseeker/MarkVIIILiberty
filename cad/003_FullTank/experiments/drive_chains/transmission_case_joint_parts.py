...
def case_joint_parts(c, old_case, old_cover):
    ...
    case=old_case.copy();cover=old_cover.copy();tools=[]
    ...
    for y,z in axes:
        ...
        hole=cylinder_x(c['bolt_diameter']/2+c['receiver_gap'],-seat-1,seat+1,y,z)
        # Flat lands provide real bearing faces and room for the head/nut.
        # Each connected cutter is a union, never a self-intersecting compound
        # of overlapping cylinders. Separate joint cutters are disjoint.
        tools.append(hole.fuse(cylinder_x(c['relief_radius'],-seat-35,-seat,y,z)).fuse(
            cylinder_x(c['relief_radius'],seat,seat+35,y,z)).removeSplitter())
    assert all(t.isValid() and len(t.Solids)==1 for t in tools)
    cutters=Part.makeCompound(tools)
    ...
    # The land at X=-gap/2 lies slightly inside the radius at X=0. Restore
    # analytic cylindrical seats through the thin seal and add an explicit
    # radial edge clearance to the stationary bush/retainer.
    radial=[Part.makeCylinder(c['bush_radius']+c['gasket_radial_gap'],420,App.Vector(0,-210,0),App.Vector(0,1,0))]
    for span in [c['bush_flange_y'],c['retainer_y']]:
        for sign in [-1,1]:
            lo,hi=sorted(sign*y for y in span)
            lo-=c['gasket_shoulder_gap'];hi+=c['gasket_shoulder_gap']
            radial.append(Part.makeCylinder(c['flange_bore_radius']+c['gasket_radial_gap'],hi-lo,App.Vector(0,lo,0),App.Vector(0,1,0)))
    radial_tool=radial[0].multiFuse(radial[1:]).removeSplitter()
    assert radial_tool.isValid() and len(radial_tool.Solids)==1
    seals=[s.cut(radial_tool).removeSplitter() for s in seals]

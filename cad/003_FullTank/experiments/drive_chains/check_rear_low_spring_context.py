"""Whole-material comparison of the six new low-speed/foot-brake spring support members against retained tank geometry."""
import argparse
import itertools
...
affected=[n for n in rows if n!='RearControlChannelStock'];selected=[];pairs=[]
...
for name,one in targets.items():
    ...
    for n,box in boxes.items():
        if n=='RearControlChannelStock' or not overlap(bb,box):continue
...
receiver_native=ROOT/r['receiver_native'];receiver=read(receiver_native.parent/'isolated/manifest.json')
assert sha(receiver_native)==receiver['native_sha256']==r['receiver_native_sha256']
receiver_rows={v['name']:v for v in receiver['occurrences'] if v['name']!='RearControlChannelStock'}
validate_native_bindings(dict(native_file=str(receiver_native),render_occurrences=list(receiver_rows),landmarks=[]),receiver)
for name,one in targets.items():
    for other,row in receiver_rows.items():
        two=world(row,receiver)
        if not overlap(bounds(one),bounds(two)):continue
        common=one.common(two);volume=sum(abs(s.Volume) for s in common.Solids)
        pairs.append(dict(first=name,second=other,origin='high_spring_prototype',common_mm3=volume,passed=(common.isNull() or common.isValid()) and volume<1e-5))
...
result=dict(passed=all(v['passed'] for v in pairs),pairs=pairs,affected_count=len(affected),tool_envelope_count=0,high_spring_prototype_count=len(receiver_rows),receiver_native_sha256=sha(receiver_native),parent_occurrence_count=len(prior),retained_standard_count=sum(n not in excluded and row['representation']=='assembly' for n,row in tank.items()),native_sha256=sha(native),parent_native_sha256=sha(parent),standard_manifest_sha256=sha(sp),checker_sha256=sha(Path(__file__)),scope='All six new low-speed/foot-brake spring bracket/rivet occurrences against all retained development and standard solids, including floor and existing channel mounts. Receiving channel separately checked for exact holes and no added material; its local contacts included in independent_checks.json.',historical_geometry_qualified=False,installation_qualified=False)

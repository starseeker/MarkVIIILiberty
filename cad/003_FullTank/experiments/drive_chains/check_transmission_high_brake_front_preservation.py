"""Compare every inherited definition against the saved qualified rear-joint parent."""
import argparse
...
assert changed == {'Def_HighBrake_long_band','Def_HighBrake_short_band','Def_HighBrake_anchor_end','Def_TransmissionCore_bevel_case'}
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 507 unchanged inherited definitions compared with the qualified rear-joint parent. Four revised and eight new definitions are checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 507

"""Export and strictly compare ten new definitions and all 30 added installed components."""
import argparse
import math
...
kronrod = KronrodMass(out / 'kronrod_mass_runtime')
...
assert len(definitions)==10 and len(installed_names)==30
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('HighBrakeMechanism' + scope + '.step')
    ...
    doc = App.newDocument('HighBrakeMechanismExchange'+scope)
    try:
        features = []
    ...
    for name, one in shapes.items():
        ...
        initial_mass = None
        if (name == 'Def_HighBrakeMechanism_spring' or name.endswith('AdjustingSpring')) and not (ma['converged'] and mb['converged']):
        ...
        if (name.startswith('Def_HighBrakeMechanism_lever_') or name.endswith(('HighSpeedBrakeLeverLeft','HighSpeedBrakeLeverRight'))) and not (ma['converged'] and mb['converged']):
            # Actual trimmed B-spline lever faces failed Gauss convergence;
            # span-aware GK passed independent exact-geometry controls. Keep
            # the original measurement and all existing acceptance predicates.
            initial_mass = dict(native=ma, step=mb)
            ma, mb = kronrod.measure(one), kronrod.measure(two)
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    step_hashes=files, mass_provenance=mass.provenance, partitioned_mass_provenance=partitioned.provenance, kronrod_mass_provenance=kronrod.provenance, export_settings={'write.surfacecurve.mode': 1,
        'writer': 'Import.export', 'input': 'one named PartDesign::Feature per component'},
    step_representations=representations,
    scope='Ten new definitions and all 30 added occurrences; adaptive converged mass integration and unchanged strict material/tolerance criteria. Not a complete brake mechanism or full-tank export.',

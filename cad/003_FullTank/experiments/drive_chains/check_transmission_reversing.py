...
try:
    ...
    # B-spline control hull bounds are conservative (25.28 x31.63 here), not
    # the spring's actual envelope. Use saved planar ends and material probes.
    ends=sorted(f.CenterOfMass.z for f in s['spring'].Faces if isinstance(f.Surface,Part.Plane) and abs(f.Surface.Axis.z)>.99999)
    check('spring has two ground end faces',len(ends),2)
    check('installed spring height from ground faces',ends[-1]-ends[0],28.0)
    # The swept B-spline approximates the nominal OD within0.0001mm radial.
    # Exact tangent-cylinder Booleans are unstable here (including negative
    # volumes); use an explicitly bounded witness without altering tolerances.
    outside=s['spring'].cut(cz(17.4625/2+.0001,185,215,x,-135))
    check('spring within source OD plus .0001mm radial witness',not outside.Faces)
    samples=[p for edge in s['spring'].Edges for p in edge.discretize(Number=1000)]
    radius=max(math.hypot(p.x-x,p.y+135) for p in samples)
    check('sampled spring OD agrees within .0002mm',2*radius,17.4625,tol=.0002)
    inner_envelope=cz(17.4625/2-.001,185,215,x,-135)
    check('spring reaches printed OD within .002mm diameter',s['spring'].cut(inner_envelope).Volume>.0001)
    ...
    vertices=[App.Vector(x,y,z0) for y,z0 in [(-155,180),(-115,180),(-115,220),(-155,220),(-155,180)]]
    section=s['spring'].section(Part.Face(Part.makePolygon(vertices)))
    check('spring has at least twelve complete meridian intersections',sum(Part.Wire(es).isClosed() for es in Part.sortEdges(section.Edges))>=12)

...
try:
    ...
    for n in range(1,7):
        ...
        near(f'spring{n}/rear seat',spring.BoundBox.XMin,d['ring_front'],2e-5)
        near(f'spring{n}/front seat',spring.BoundBox.XMax,d['spring_front'],2e-5)
    ...
    for key in keys:
        ...
        two=remaining.pop(idx);ta=one.getTolerance(1);tb=two.getTolerance(1);fuzzy=min(.0001,max(1e-7,ta+tb))
    ...
    remaining=list(loaded.Solids);placed=[]
    for name in r['affected_ids']:
        i=byid[name];one=i['shape'].Solids[0];idx=min(range(len(remaining)),key=lambda j:(one.CenterOfMass-remaining[j].CenterOfMass).Length+abs(one.Volume-remaining[j].Volume)/max(one.Area,1))
        two=remaining.pop(idx);mb=mass(two,name+'_placed');ma=native_mass[i['target'].Name]

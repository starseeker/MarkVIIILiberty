...
def build(c,pc,tc):
    ...
    for n in range(tc['plunger_count']):
        ...
        hole=along(cyl(sr,-5,grip+6),base,N)
        seats=along(rivet,base,N)
        tool=hole.fuse(seats).removeSplitter()
    ...
    for row,(count,fraction) in enumerate(zip(c['lining_rivet_rows'],c['lining_rivet_slant_fractions'])):
        ...
        for n in range(count):
            ...
            drill=cyl(lr,-4,total+2)
            counter=Part.makeCone(lr,c['lining_head_radius'],c['lining_head_depth'],App.Vector(0,0,headstart)).fuse(cyl(c['lining_head_radius'],headtop,total+2))
            tools_cone.append(along(drill.fuse(parts['lining_rivet']).removeSplitter(),base,N))
            tools_lining.append(along(drill.fuse(counter).removeSplitter(),base,N))
    ...
    return parts,occ,support,spine,d

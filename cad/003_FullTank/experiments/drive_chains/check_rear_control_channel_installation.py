"""Bind the integrated mounts to the tested prototype and preserve inherited frames and metadata."""
import argparse
import json
...
ck('Exactly29 additions:3202 occurrences,551 used definitions,343 groups',
   len(rows) == 3202 and len(m['definitions']) == 551 and len(m['assemblies']) == 343
   and set(rows) == set(prior) | set(r['expected_new_occurrences'])
   and len(set(rows) - set(prior)) == 29
   and set(m['definitions']) == set(old['definitions']) | set(r['new_definitions']))
ck('All3173 inherited occurrence targets, owners and frames retained',
...
ck('New link frames, definitions and owners match saved prototype',
   all(rows[n]['definition'] == spec['definition'] and rows[n]['owners'] == trial[n]['owners']
...
with tempfile.TemporaryDirectory(prefix='relocated_mounts_', dir=out) as directory:
    ...
    try:
        ck('All3202 installed links reopen with local identity definitions',
        ...
        ck('Four new nuts reuse the same definition as the two prior control nuts',
           sum(v['definition'] == pr['shared_nut_definition'] for v in rows.values()) == 6)
...
mapping = {**{role: 'Def_RearControlMount_' + role for role in ['channel', 'cleat', 'bolt', 'lock', 'rivet']},
           'nut': pr['shared_nut_definition'], 'floor': pr['source_floor_definition']}
for role, key in mapping.items():
    result = same_material(definition('Def_ChannelMount_' + role, pm), definition(key, m))
...
for key, before in op.items():
    ...
    if after is not None and ET.tostring(before) == ET.tostring(after):
        continue
    name, prop = key
    if prop == 'Group' and name in ['Root', 'Definitions'] and after is not None:
    ...
    elif name == r['changed_floor_feature'] and prop == 'Placement' and after is not None:
        loc = after.find('PropertyPlacement')
        passed = all(float(loc.get(k)) == 0 for k in ['Px', 'Py', 'Pz', 'Q0', 'Q1', 'Q2']) and float(loc.get('Q3')) == 1
        reason = 'Floor feature now stores canonical revised shape; actual body and installed material compared above'
    elif prop in ['Geometry', 'Placement'] and after is not None:
...
allowed = {(pr['source_floor_definition'], k) for k in ['RearChannelMountSourceRecords', 'RearChannelMountUpdate']}
ck('Persistent inherited properties preserved except explicit floor/group/status changes',
...
result = dict(passed=all(v['passed'] for v in checks), checks=checks,
              ...
              scope='All links/frames/owners and persistent inherited properties; strict seven-definition and30-installed-shape transfer from the tested prototype. Whole inherited definition preservation is checked separately.',

...
for side in ['Starboard', 'Port']:
    ...
    front = after.multVec(V(*c['track_arm_local_mm']))
...
inputs = [Path(__file__), H/'rear_track_rod_parts.py', H/'transmission_brake_front_parts.py',
          H/'transmission_brake_anchor_parts.py', H.parents[1]/'lib/cad_build.py', front_controls,
...
write(out/'report.json', dict(native_file=saved.name, native_sha256=sha(saved),
      ...
      changed_definitions=[c['lever_definition']], changed_occurrences=sorted(set(specs)-{side+'TrackConnectingRod' for side in ['Starboard', 'Port']}),

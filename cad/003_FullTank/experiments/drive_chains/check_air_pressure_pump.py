...
try:
    ...
    for label,sign in [('Rear',-1),('Front',1)]:
        ...
        ck(label+' shaft is stopped axially by bush after running gap',shift(shaft,App.Vector(sign*(c['shaft_axial_gap']+.05),0,0)).common(bush).Volume>.01)
        zero(label+' shaft remains clear within axial running gap',shift(shaft,App.Vector(sign*c['shaft_axial_gap']/2,0,0)).common(bush))
    ...
    for item in items:
        ...
        ma,mb=mass(one,item['id']+'_placed_native'),mass(two,item['id']+'_placed_step')
        delta=(App.Vector(*ma['center_mm'])-App.Vector(*mb['center_mm'])).Length
        volume=abs(ma['volume_mm3']-mb['volume_mm3'])
        bound=(one.Area+two.Area)/2*(one.getTolerance(1)+two.getTolerance(1))
        placements.append(dict(id=item['id'],passed=delta<max(1e-6,one.getTolerance(1)+two.getTolerance(1)) and volume<bound
            and all(0<=m['estimated_relative_error']<1e-9 for m in [ma,mb]),
            centroid_difference_mm=delta,volume_difference_mm3=volume,surface_tolerance_bound_mm3=bound,native_mass=ma,step_mass=mb))
    write(out/'assembly_exchange_checks.json',dict(passed=all(r['passed'] for r in placements),checks=placements,
        scope='51 placed solids: validity,count,centroid and explicit-accuracy mass; detailed Boolean/tolerance checks are in the 17-definition exchange receipt.',

...
p.add_argument('--output',type=Path,default=HERE/'engine_water_pump_study')
...
try:
    ...
    metadata(pump,QuantityRole='One catalogue pump assembly; nonphysical container',SourceRecord='SNL:160:016',
        Subsystem='Powerplant',Coverage='partial',ReconstructionNotes='Development pump; source profile and case attachment checks pending. Packing quantity and cover elbow identity conflicts retained.')
    ...
    write(out/'report.json',dict(status='unqualified_water_pump_development',native_file=native.name,native_sha256=sha(native),
        ...
        changed_ids=[],native_occurrences=len(byid),new_definitions=len(shapes),unchanged_parent_occurrences=2170,
        ...
        pending=['Independent geometry, source views, gear mesh and STEP checks','Case mounting holes/hardware and gasket/shim stack reconciliation',

...
selected = [n for n in prior if n.startswith('ClutchSupport_') or 'HighSpeedBrakeControl' in n]
...
shaded(items, out/'route_isometric.svg', (-1,-1,.8),
       'Clearance study | local M575 rod witnesses; spring envelopes in outline', context=outlines)
detail = [v for v in items if v['id'].startswith(('Port','ClutchSupport_Aux','ClutchSupport_Left'))]
shaded(detail, out/'route_detail.svg', (-1,-1,.6),
       'Port route witness | bracket, clutch casting and auxiliary pin clearance',
...
for name in ['RearControlChannelStock', 'PortHighSpringBracket', 'PortRodEnvelope', 'PortSpringEnvelope']:
...
write(out/'render_receipt.json', dict(trial_native_sha256=sha(native), parent_native_sha256=sha(parent),
    ...
    scope='Clearance witnesses in saved neighboring geometry; these are not completed M575 rods or spring models.'))

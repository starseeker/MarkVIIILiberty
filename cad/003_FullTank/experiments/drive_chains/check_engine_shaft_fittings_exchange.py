"""Compare saved shaft closure definitions and installed STEP solids in both material directions."""
import argparse
import math
...
p=argparse.ArgumentParser();p.add_argument('--candidate',type=Path,default=HERE/'engine_shaft_fittings_build');p.add_argument('--worker',action='store_true')
...
try:
    ...
    native=out/'DrivetrainWithEngineShaftFittings.FCStd';nh=sha(native);r=read(out/'report.json');assert r['native_sha256']==nh
    ...
    for label,expected in [('Definitions',[(k,doc.getObject(k).Shape) for k in read(out/'definition_order.json')]),
        ...
        path=out/('EngineShaftFittings'+label+'.step');loaded=Part.Shape();loaded.read(str(path))
    ...
    write(out/'exchange_checks.json',dict(passed=all(x['passed'] for x in exchange),native_sha256=nh,checker_sha256=sha(Path(__file__)),
        artifact_hashes={n:sha(out/n) for n in ['EngineShaftFittingsDefinitions.step','EngineShaftFittingsInstallation.step']},checks=exchange))

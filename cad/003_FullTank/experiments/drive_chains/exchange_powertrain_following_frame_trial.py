"""Export and strictly compare the moved frame and its installed casting and MX5 interfaces."""
import argparse
...
installed = {}
installed_names = {name for name,row in rows.items() if
    'TransmissionMountingFrame' in row['owners'] or 'TransmissionCaseMounting' in row['owners']}
installed_names.update(['CenterTransmissionCore_bevel_case'])
installed_names.update(hand+'FixedBearing_'+role+'_bracket'
                       for hand in ['Port','Starboard'] for role in ['inner','outer'])
assert len(installed_names)==36
...
results = []
files = {}
for scope, shapes in [('Installation', installed)]:
    path = out / ('FollowingFrame' + scope + '.step')
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='15 moved frame occurrences, five original castings and 16 retained MX5 constituents in installed coordinates; not a full-tank export.',

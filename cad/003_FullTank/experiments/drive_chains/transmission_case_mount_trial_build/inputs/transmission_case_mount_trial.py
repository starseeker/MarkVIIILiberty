...
REPO=next(p for p in Path(__file__).resolve().parents if (p/'cad/003_FullTank').is_dir())
...
p.add_argument('--output',type=Path,default=ROOT/'transmission_case_mount_trial_build')
...
try:
    ...
    old['CenterTransmissionCore_bevel_case']['target'].Shape.exportBrep(str(out/'inputs/parent_case.brep'))
    ...
    report=dict(status='unqualified_installation_trial',native_sha256=sha(trial),parent_native_sha256=sha(native),input_hashes=hashes,
        parent_case_brep_sha256=sha(out/'inputs/parent_case.brep'),controls=c,pin_controls=pc,dimensions=d,new_ids=new,changed_ids=changed,native_occurrences=len(items),

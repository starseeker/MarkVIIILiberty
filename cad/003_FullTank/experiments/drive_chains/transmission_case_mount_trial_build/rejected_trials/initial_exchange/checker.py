...
p=argparse.ArgumentParser();p.add_argument('--worker',action='store_true');p.add_argument('--candidate',type=Path,default=Path(__file__).parent/'trial01')
...
try:
    ...
    for label,sgn,base,member in [('Upper',1,318.63631878624517,'Top'),('Lower',-1,-262.33445920236954,'Bottom')]:
        ...
        for side,y in [('Port',118),('Starboard',-118)]:
            ...
            check(name+' nut and channel separated by bevel washer',nut.distToShape(channel)[0]>4)
            addition_masks.append(cx(22.001,-227.149,end+4.601,y,z))
            hole_masks.append(cx(9.676,-228.15,end+.301,y,z))
    previous=Part.Shape();previous.read(str(Path(__file__).parent/'Def_TransmissionCore_bevel_case.brep'))

"""Compare all inherited brake-band definition material with its saved parent.
...
the five new definitions and two revised friction lands.
"""
import argparse
...
assert qualification['local_brake_checks_passed']
...
assert changed == {'Def_TransmissionCore_brake_case', 'Def_Output_drum'}
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 464 unchanged inherited definitions compared with the qualified brake-suspension parent. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 464

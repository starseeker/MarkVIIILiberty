...
for name in expected:
    ...
    near(name+' full friction contact',inner_area,sum(f.Area for f in inner))
    near(name+' full steel backing contact',outer_area,sum(f.Area for f in outer))
    contacts.append(dict(name=name,friction_area_mm2=inner_area,backing_area_mm2=outer_area))

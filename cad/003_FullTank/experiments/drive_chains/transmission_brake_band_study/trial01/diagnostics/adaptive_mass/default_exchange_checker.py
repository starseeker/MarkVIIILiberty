"""Export and strictly compare brake-band definitions and every affected installed occurrence."""
import argparse
...
assert len(definitions)==7 and len(installed_names)==252
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('BrakeBands' + scope + '.step')
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='Five new brake definitions, two revised drum definitions and all 252 affected installed components; not a full-tank export.',

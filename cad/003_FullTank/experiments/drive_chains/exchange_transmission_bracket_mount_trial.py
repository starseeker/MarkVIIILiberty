"""Export and strictly compare bracket joint definitions and all affected installed components."""
import argparse
...
definitions = {name: definition(name) for name in set(r['changed_definitions']) | set(r['hardware_definitions'].values())}
installed = {}
installed_names = set(r['affected_occurrences'])
assert len(definitions)==8 and len(installed_names)==86
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('BracketMounts' + scope + '.step')
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='Four receiving definitions, four hardware definitions and 86 affected installed components; not a full-tank export.',

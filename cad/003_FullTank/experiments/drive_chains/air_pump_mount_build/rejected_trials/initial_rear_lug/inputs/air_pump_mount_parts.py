...
def build(c,pc,old_cover,old_house):
    ...
    for sign,label,key in [(1,'Port','bracket_left'),(-1,'Starboard','bracket_right')]:
        ...
        for n,x in enumerate([c['pump_x']-pc['foot_hole_x'],c['pump_x']+pc['foot_hole_x']],1):
            ...
            add(f'Base{n}_Washer','washer',(x,y,bottom-c['washer_stock']),'BaseFasteners')
            add(f'Base{n}_Nut','small_nut',(x,y,bottom-c['washer_stock']-c['small_nut_height']),'BaseFasteners')
    ...
    revised=dict(cover=cover.removeSplitter(),housing=house.removeSplitter())

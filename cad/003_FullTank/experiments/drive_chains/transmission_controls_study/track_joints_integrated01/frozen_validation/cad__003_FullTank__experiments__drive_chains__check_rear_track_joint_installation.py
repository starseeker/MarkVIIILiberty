"""Bind the integrated rear track end joints to the tested prototype and preserve inherited frames and metadata."""
import argparse
import json
...
ck('Exactly16 additions:3262 occurrences,567 used definitions,359 groups',
   len(rows) == 3262 and len(m['definitions']) == 567 and len(m['assemblies']) == 359
   and set(rows) == set(prior) | set(r['expected_new_occurrences'])
   and len(set(rows) - set(prior)) == 16
   and set(m['definitions']) == set(old['definitions']) | set(r['new_definitions']))
ck('All3246 inherited occurrence targets, owners and frames retained',
...
with tempfile.TemporaryDirectory(prefix='relocated_mounts_', dir=out) as directory:
    ...
    try:
        ck('All3262 installed links reopen with local identity definitions',
        ...
        ck('Four forks share one definition and all four nuts reuse the existing definition',
           sum(v['definition'] == r['role_definitions']['fork'] for v in rows.values()) == 4
           and r['role_definitions']['nut'] in old['definitions'])
...
for role, key in mapping.items():
    result = same_material(definition('Def_TrackJoint_' + role, pm), definition(key, m))
...
result = dict(passed=all(v['passed'] for v in checks), checks=checks,
              ...
              scope='All links/frames/owners and persistent inherited properties; strict four-definition and16-installed-shape transfer from the tested prototype. Whole inherited definition preservation is checked separately.',

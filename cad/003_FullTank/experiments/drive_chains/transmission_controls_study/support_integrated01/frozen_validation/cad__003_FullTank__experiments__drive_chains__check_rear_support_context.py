"""Whole-material comparison of the combined rear support family against retained tank geometry."""
import argparse
import itertools
...
affected=[n for n in rows if n not in {'RearControlChannelStock','hull_floor_7'}];selected=[];pairs=[]
...
for name,one in targets.items():
    ...
    for n,box in boxes.items():
        if n in {'RearControlChannelStock','hull_floor_7'} or not overlap(bb,box):continue

"""Strict per-definition and installed STEP comparison for the saved combined rear support family."""
import argparse
import math
...
for scope,shapes in [('Definitions',definitions),('Installed',installed)]:
    path=out/('RearSupportFamily'+scope+'.step');assert not path.exists();doc=App.newDocument('RearSupportExchange'+scope)
    try:
        objects=[]

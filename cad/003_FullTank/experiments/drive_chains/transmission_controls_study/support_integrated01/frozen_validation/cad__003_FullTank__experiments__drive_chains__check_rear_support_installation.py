"""Bind the integrated rear support family to the tested prototype and preserve inherited frames and metadata."""
import argparse
import json
...
ck('Exactly20 additions:3246 occurrences,564 used definitions,353 groups',
   len(rows) == 3246 and len(m['definitions']) == 564 and len(m['assemblies']) == 353
   and set(rows) == set(prior) | set(r['expected_new_occurrences'])
   and len(set(rows) - set(prior)) == 20
   and set(m['definitions']) == set(old['definitions']) | set(r['new_definitions']))
ck('All3226 inherited occurrence targets, owners and frames retained',
...
with tempfile.TemporaryDirectory(prefix='relocated_mounts_', dir=out) as directory:
    ...
    try:
        ck('All3246 installed links reopen with local identity definitions',
        ...
        ck('Two right cleats share one definition and existing nut/washer definitions',
           sum(v['definition'] == r['role_definitions']['cleat'] for v in rows.values()) == 2
           and all(r['role_definitions'][role] in old['definitions'] for role in ['lock', 'nut']))
...
mapping = r['role_definitions']
for role, key in mapping.items():
    result = same_material(definition('Def_RearSupport_' + role, pm), definition(key, m))
...
ck('Persistent inherited properties preserved except explicit receiver/group/status changes',
...
result = dict(passed=all(v['passed'] for v in checks), checks=checks,
              ...
              scope='All links/frames/owners and persistent inherited properties; strict ten-definition and22-installed-shape transfer from the tested prototype. Whole inherited definition preservation is checked separately.',

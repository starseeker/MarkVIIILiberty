"""Whole-material comparison of the24 new fulcrum and lever members against retained tank geometry."""
import argparse
import itertools
...
affected=[n for n in rows if n!=r['controls']['channel_occurrence']];selected=[];pairs=[]
...
for name,one in targets.items():
    ...
    for n,box in boxes.items():
        if n==r['controls']['channel_occurrence'] or not overlap(bb,box):continue
...
result=dict(passed=all(v['passed'] for v in pairs),pairs=pairs,affected_count=len(affected),tool_envelope_count=0,parent_occurrence_count=len(prior),retained_standard_count=sum(n not in excluded and row['representation']=='assembly' for n,row in tank.items()),native_sha256=sha(native),parent_native_sha256=sha(parent),standard_manifest_sha256=sha(sp),checker_sha256=sha(Path(__file__)),scope='All24 new fulcrum/lever/washer/cotter/rivet occurrences against all retained development and standard solids, including floor and existing channel mounts. Receiving channel separately checked for exact holes and no added material; its local contacts included in independent_checks.json.',historical_geometry_qualified=False,installation_qualified=False)

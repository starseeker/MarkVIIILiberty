...
assert changed == set()
assert len(source_manifest['definitions']) == 542
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All542 inherited definitions compared with the qualified brake-support parent. Three new definitions and reused nut are separately compared against the tested rear-control joint prototype. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 542

...
try:
 for role in ['fork','pin','cotter','nut']:
  key=r['shared_definitions'].get(role,'Def_ControlJoint_'+role);body=doc.getObject(key)
  assert body.Placement.isIdentity();definitions[role]=body.Shape.copy()
 for name,spec in r['expected_new_occurrences'].items():
  link=doc.getObject(name);pose=doc.getObject(spec['owners'][-1]).getGlobalPlacement().multiply(link.LinkPlacement)
  assert max(abs(x-y) for x,y in zip(pose.toMatrix().A,spec['frame']))<1e-7
  s=link.LinkedObject.Shape.copy();s.Placement=pose;installed[name]=s
finally:App.closeDocument(doc.Name)
assert len(definitions)==4 and len(installed)==8
checks=[];files={}

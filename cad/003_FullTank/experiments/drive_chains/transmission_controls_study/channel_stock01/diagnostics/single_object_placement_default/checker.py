...
Part.setStaticValue('write.surfacecurve.mode',1);mass=AdaptiveMass(out/'adaptive_mass_runtime');exchange=[]
...
write(out/'exchange_checks.json',dict(passed=all(v['passed'] for v in exchange),checks=exchange,native_sha256=sha(native),checker_sha256=sha(Path(__file__)),mass_provenance=mass.provenance))

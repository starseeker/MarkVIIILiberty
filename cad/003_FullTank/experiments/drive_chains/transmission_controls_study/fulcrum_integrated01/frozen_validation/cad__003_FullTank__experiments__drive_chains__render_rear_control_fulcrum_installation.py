"""Render actual saved rear fulcrums and compare them through the unchanged local source registration."""
import argparse
...
parent=ROOT/r['source_native'];old=read(parent.parent/'isolated/manifest.json');assert sha(parent)==old['native_sha256']==r['source_native_sha256']
rows={v['name']:v for v in m['occurrences'] if v['name'] in r['expected_new_occurrences'] or v['name']==r['controls']['channel_occurrence']};prior={v['name']:v for v in old['occurrences']}
context_names=[n for n in prior if n.startswith('RearChannelLeftCleat') or n=='hull_floor_7']
...
COLORS.update(Channel=(.69,.48,.28),Bracket=(.57,.59,.47),Lever=(.39,.54,.61),Hardware=(.73,.67,.50),Cleat=(.44,.53,.56),Floor=(.48,.54,.52))
...
for name,row in rows.items():
    role=r['expected_new_occurrences'].get(name,{}).get('role','channel');system='Channel' if role=='channel' else 'Bracket' if role=='bracket' else 'Lever' if role.startswith('lever') else 'Hardware'
    items.append(item(name,row,m,system))
for name in context_names:
    v=item(name,prior[name],old,'Floor' if name=='hull_floor_7' else 'Cleat')
    (floor if name=='hull_floor_7' else items).append(v)
shaded(items,out/'fulcrum_isometric.svg',(-1,-1,.8),'Rear controls | four retained fulcrums and horizontal levers',context=floor)
shaded([v for v in items if v['id'].startswith('PortLow')],out/'fulcrum_detail.svg',(-1,-1,.5),'M4131/M4133 | cast foot, horizontal lever, spring washer and split pin')
...
for name in rows:
    ...
    role=r['expected_new_occurrences'].get(name,{}).get('role','channel');color='#d26e19' if role=='channel' else '#1d7ea3' if role.startswith('lever') else '#74794d'
...
draw.text((1130,690),'Fixed side registration; port fulcrums/levers only. No plan calibration.',fill='#111111')
im.save(out/'source_overlay.png');im.crop((1130,685,1680,970)).resize((1650,855)).save(out/'source_detail.png')
write(out/'render_receipt.json',dict(native_sha256=sha(native),manifest_sha256=sha(out/'isolated/manifest.json'),renderer_sha256=sha(Path(__file__)),parent_native_sha256=sha(parent),source_registration_sha256=sha(regpath),camera=camera,source_camera_refitted=False,images={n:sha(out/n) for n in ['fulcrum_isometric.png','fulcrum_detail.png','source_overlay.png','source_detail.png']},scope='Selected25 installed occurrences in the full development native, plus retained M4130 mounts and floor context. Source side projection diagnoses the conditional layout; no historical perspective or plan fit.',historical_geometry_qualified=False))
print('Rendered fulcrums and fixed-registration source overlay.',flush=True)

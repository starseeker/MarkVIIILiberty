"""Strict per-definition and installed STEP comparison for the saved fulcrum assembly."""
import argparse
import math
...
for scope,shapes in [('Definitions',definitions),('Installed',installed)]:
    path=out/('RearControlFulcrums'+scope+'.step');assert not path.exists();doc=App.newDocument('FulcrumExchange'+scope)
    try:
        objects=[]

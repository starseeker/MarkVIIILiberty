...
for name,station in c['stations'].items():
    ...
    base=V(*station['pivot']);pose=App.Placement(App.Matrix(*rows[name+'FulcrumBracket']['frame']));seat=base.z+b.get('seat_height',b['foot_stock'])
    ...
    ck(name+' journal cap remains beyond cotter bore',b.get('seat_height',b['foot_stock'])+b['journal_height']-r['details']['cotter_center_z_mm']-b['cotter_hole_diameter']/2>4)
    for i,y in enumerate(b['rivet_y'],1):
        ...
        fa=bearing(rv,bracket,base.z+b['foot_stock']);ba=bearing(rv,channel,base.z-c['channel_stock'])
    ...
    for role,xy in c['lever_profiles'][station['lever_profile']].items():
        q=lp.multVec(V(*xy,l.get('arm_bottom',0)));tool=Part.makeCylinder(l['end_bore_diameter']/2,l['stock']+2,q-V(0,0,1))

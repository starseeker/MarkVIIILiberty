...
for name in affected:
    one=world(rows[name],m);bb=bounds(one)
...
result=dict(passed=all(v['passed'] for v in pairs),pairs=pairs,affected_count=len(affected),parent_occurrence_count=len(prior),retained_standard_count=sum(n not in excluded and row['representation']=='assembly' for n,row in tank.items()),native_sha256=sha(native),parent_native_sha256=sha(parent),standard_manifest_sha256=sha(sp),checker_sha256=sha(Path(__file__)),scope='All29 new channel/cleat/hardware occurrences against all retained development and standard solids. Revised floor separately checked for exact holes and no added material; its local contacts included in independent_checks.json.',historical_geometry_qualified=False,installation_qualified=False)

...
def parts(c, channel, channel_pose):
    ...
    rear = c.get('mount_face') == 'rear_flange'
    if rear:
        points = [(-b['foot_width']/2, b['bottom']), (b['foot_width']/2, b['bottom']),
                  (b['foot_width']/2, b['shoulder_bottom']), (radius, b['shoulder_top']),
                  (radius, z), (-radius, z), (-radius, b['shoulder_top']),
                  (-b['foot_width']/2, b['shoulder_bottom'])]
        vertices = [V(0, y, height) for y, height in points]
        plate = Part.Face(Part.makePolygon(vertices+[vertices[0]])).extrude(V(t, 0, 0))
        bracket = plate.fuse(crown)
        local_holes = [Part.makeCylinder(c['rivet']['hole_diameter']/2, t+2,
                                        V(-1, y, b['rivet_height']), V(1, 0, 0))
                       for y in b['rivet_y']]
        guide_length = t+2
    else:
        ...
        guide_length = t+bend+2
    bracket = bracket.cut(Part.makeCylinder(b['guide_diameter']/2, guide_length,
                                           V(-1, 0, z), V(1, 0, 0)))
    ...
    for name, point in c['stations'].items():
        for y in b['rivet_y']:
            if rear:
                top = V(*point) + V(t, y, b['rivet_height'])

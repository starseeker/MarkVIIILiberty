...
assert changed == {'Def_ClutchSupport_floor_revision'}
assert len(source_manifest['definitions']) == 546
assert len(report['new_definitions']) == 5
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All545 unchanged inherited definitions preserved. The revised floor and five new definitions match the independently tested mounting prototype; seven-definition and30-installed-shape transfer is checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 545

"""Strict per-definition and installed STEP comparison for the saved channel mount."""
import argparse
import math
...
for scope,shapes in [('Definitions',definitions),('Installed',installed)]:
    path=out/('RearControlChannelMount'+scope+'.step');assert not path.exists();doc=App.newDocument('ChannelExchange'+scope)
    try:
        objects=[]

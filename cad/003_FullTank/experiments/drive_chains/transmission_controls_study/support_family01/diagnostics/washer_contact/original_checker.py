...
for side, xyz in c['stations'].items():
    ...
    ck(side+' complete lock washer underside bears on nut',
       all(supported(f, washer_contact) for f in faces(lock, lower)), bearing_area_mm2=washer_contact.Area)
    projection = nut.BoundBox.ZMin-screw.BoundBox.ZMin
    ck(side+' bolt spans full nut height with an exposed tip', projection > 0,

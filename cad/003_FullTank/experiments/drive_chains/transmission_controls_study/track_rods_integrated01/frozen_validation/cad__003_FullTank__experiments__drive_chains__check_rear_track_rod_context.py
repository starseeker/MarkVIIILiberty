"""Whole-material comparison of the revised receivers, end joints and straight rods against retained tank geometry."""
import argparse
import itertools
...
result=dict(passed=all(v['passed'] for v in pairs),pairs=pairs,affected_count=len(affected),tool_envelope_count=0,parent_occurrence_count=len(prior),retained_standard_count=sum(n not in excluded and row['representation']=='assembly' for n,row in tank.items()),native_sha256=sha(native),parent_native_sha256=sha(parent),standard_manifest_sha256=sha(sp),checker_sha256=sha(Path(__file__)),replaced_occurrences=sorted(set(rows)&set(prior)), scope='All24 trial components compared against each other and retained development/standard solids. Only replaced old versions of these same named occurrences are omitted; replacement material is included in local pairs.',historical_geometry_qualified=False,installation_qualified=False)

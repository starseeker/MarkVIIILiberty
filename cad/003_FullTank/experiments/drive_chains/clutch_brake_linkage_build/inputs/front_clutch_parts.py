...
def spring(c):
    ...
    # Clock the inferred ground patch across the split collar. Its approximate
    # rear bearing arc is 2*pi*ground_depth/end_pitch, so centre that on Z=0.
    phase=-math.pi*(r-offset)/end_pitch
    def integral(u):
        if u<=end:return 0
        if u<end+blend:
    ...
    for n in range(count+1):
        u=n/c['spring_samples_per_turn'];angle=2*math.pi*u+phase
    ...
    # Split the same interpolated centreline into one-turn edges before sweeping.
    # One seven-turn surface can silently lose intersections in OCC Booleans.
    edges=[]
    for n in range(turns):
        curve=path.copy()
        curve.segment(path.parameter(pts[n*c['spring_samples_per_turn']]),
                      path.parameter(pts[(n+1)*c['spring_samples_per_turn']]))
    ...
    detail=dict(turns=turns,free_turns=turns-2*end,end_turns_each=end,
        ...
        inferred_clocking_degrees=math.degrees(phase),sweep_spans=len(edges))

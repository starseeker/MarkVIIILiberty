"""Compare saved brake definitions and installed STEP solids in both material directions."""
import argparse
...
try:
    ...
    native=out/'TransmissionWithClutchBrake.FCStd';nh=sha(native);r=read(out/'report.json');assert r['native_sha256']==nh
    ...
    for label,expected in [('Definitions',[(k,doc.getObject(k).Shape) for k in read(out/'definition_order.json')]),
        ...
        path=out/('ClutchBrake'+label+'.step');loaded=Part.Shape();loaded.read(str(path))
    ...
    write(out/'exchange_checks.json',dict(passed=all(x['passed'] for x in exchange),native_sha256=nh,checker_sha256=sha(Path(__file__)),
        artifact_hashes={n:sha(out/n) for n in ['ClutchBrakeDefinitions.step','ClutchBrakeInstallation.step']},checks=exchange))

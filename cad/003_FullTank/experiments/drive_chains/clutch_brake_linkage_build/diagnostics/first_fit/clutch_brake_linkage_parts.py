...
def cotter(diameter, length, radius):
    controls=dict(cotter_center_spacing=diameter*.52,cotter_diameter=diameter,
        crown_radius=radius,cotter_head_gap=.8,cotter_exit_gap=.8,
...
def parts(c, old, band_controls, bd, support_controls):
    ...
    root=at(outer+stock/2,finish-.04)
    tip=V(x0,c['mount_head_y']+c['mount_grip'],c['mount_z'])
    direction=tip-root;direction.normalize();normal=V(0,-direction.z,direction.y)
    web=polygon([root+normal*stock/2,tip+normal*stock/2,
                 tip-normal*stock/2,root-normal*stock/2],X*width)
    ...
    bell=bell.fuse(Part.makeBox(arm,22,ws,V(-arm,-11,-ws/2)))
    bell=bell.fuse(Part.makeBox(22,input_offset,ws,V(-11,0,-ws/2)))
    ...
    fork=fork.cut(Part.makeBox(34,gap,2*fr+2,V(-18,-gap/2,-fr-1)))
    ...
    carrier=Part.makeBox(xb-xa,c['carrier_stock'],40,V(xa,my+stock,mz-20))
    ...
    # A plate strip in YZ joins the mounting plate to the pivot boss.
    a=V(pivot.x-20,my+stock+c['carrier_stock']/2,mz)
    b=pivot+normal*(bearing_low+5);b.x=a.x
    tangent=b-a;tangent.normalize();across=V(0,-tangent.z,tangent.y)
    connector=polygon([a+across*c['carrier_stock']/2,b+across*c['carrier_stock']/2,
        b-across*c['carrier_stock']/2,a-across*c['carrier_stock']/2],X*40)
    carrier=carrier.fuse(connector).fuse(boss)
    ...
    p['bell_cotter'],ccd=cotter(cd,c['bell_cotter_length'],cr)

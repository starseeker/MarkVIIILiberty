...
for joint in r['mount_datums']['mounts']:
    ...
    cylinders = [f.Surface for f in local.Faces if type(f.Surface).__name__=='Cylinder'
                 and abs(abs(f.Surface.Axis.x)-1)<1e-10]
    ck(name+' analytic printed stud diameter', bool(cylinders) and
       all(abs(2*s.Radius-19.05)<1e-6 for s in cylinders),
       radii_mm=[s.Radius for s in cylinders], conservative_bbox_y_mm=local.BoundBox.YLength)
    stock=Part.makeCylinder(19.05/2,142.875,V(),V(1,0,0))
    near(name+' all stud material inside printed stock',local.cut(stock).Volume)
    y,z=joint['y'],joint['z']

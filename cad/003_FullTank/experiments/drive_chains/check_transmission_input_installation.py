...
try:
    ...
    for row in report['exchange_roundtrip_checks']:
        check(row['id']+' stored STEP tolerance within declared reporting roundoff',
            row['step_max_tolerance_mm']<=max(1e-7,row['native_max_tolerance_mm'])+1e-10)
    check('negative:1e-7mm stored-tolerance inflation rejected',not (1.01e-5<=1e-5+1e-10))
    ...
    for n in range(4):
        ...
        check('MX25 '+str(n)+' has two distinct bent leg surfaces',sum(isinstance(f.Surface,Part.Toroid) and abs(f.Surface.MajorRadius-1.5)<1e-6 for f in cotter.Faces),2)
        for sign in [-1,1]:
    ...
    # The two branch gauges pass below the cage small-end lips, through the
    # existing gap above the cone ribs. They have diameter1mm; the main feed
    # gauge has diameter2mm. Neither gauge cuts or changes model geometry.
    branch=port+u*47.8
    bridge=segment(branch,low+u*.5,radius=.5)
    axial=segment([254,branch.y,branch.z],[300,branch.y,branch.z],radius=.5)
    witness=radial.fuse(elbow_gauge).fuse(vertical).fuse(bridge).fuse(axial).removeSplitter()
    ...
    for name in report['changed_ids']:
        if name in report['repaired_prior_cotter_ids']:continue
    ...
    for name in report['repaired_prior_cotter_ids']:
        target=byid[name]['target'].Shape.Solids[0]
        cap=byid[name]['definition']=='transmission_cap_cotter'
        wire,half,head,length,station=(.74375,.84375,-14.,34.925,0.) if cap else (1.18125,1.2,-27.5,63.5,480.)
        check(name+' preserves source leg stock after fusion',target.Volume>=2*math.pi*wire**2*length-1e-5)
        straight=28.5 if cap else length
        for sign in [-1,1]:
            witness_leg=cyl(wire*.8,[station,head+.1,sign*half],[0,1,0],straight-.2)
            zero(name+' retains straight leg '+str(sign),witness_leg.cut(target))
        if cap:check(name+' retains both bent tails',sum(isinstance(f.Surface,Part.Toroid) and abs(f.Surface.MajorRadius-2)<1e-6 for f in target.Faces),2)
    ...
    result=dict(passed=all(r['passed'] for r in checks),checks=checks,check_count=len(checks),native_sha256=sha(native),
        ...
        scope='Saved nominal threads/retention, connected2mm main-feed gauge with1mm branches beneath the cage lips, and protected material. No flow-rate, sealing, load or historical-fit qualification.')

"""Compare every inherited definition against the saved qualified spacer parent."""
import argparse
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 504 inherited definitions compared with the qualified brake-spacer parent. Seven new high-speed brake definitions are checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 504

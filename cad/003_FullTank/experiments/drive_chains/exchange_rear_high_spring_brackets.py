"""Strict per-definition and installed STEP comparison for the saved high-speed spring guide pair."""
import argparse
import math
...
for scope,shapes in [('Definitions',definitions),('Installed',installed)]:
    path=out/('RearHighSpringBrackets'+scope+'.step');assert not path.exists();doc=App.newDocument('HighSpringExchange'+scope)
    try:
        objects=[]

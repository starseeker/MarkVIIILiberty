"""Render the saved rear high-speed brake joints without camera fitting."""
...
COLORS.update(Anchor=(.71,.47,.29),Rivet=(.62,.56,.45),Screw=(.69,.72,.74),HighBand=(.38,.54,.61),HighLining=(.70,.47,.25),Steel=(.58,.62,.68),Frame=(.62,.53,.40))
cache={};items={}
for row in m['occurrences']:
    ...
    role='Rivet' if 'AnchorEndRivet' in name else 'Anchor' if name.endswith('AnchorEnd') else 'Screw' if 'CouplingScrew' in name else 'HighLining' if 'HighSpeedBrake' in name and name.endswith('Lining') else 'HighBand' if 'HighSpeedBrake' in name and name.endswith('Band') else 'Frame' if 'TransmissionMountingFrame' in owners else 'Steel'
...
shaded([v for v in items.values() if v['system']!='Frame'],out/'isometric.svg',(1,-.8,.55),'High-speed brake development | rear anchor ends and six-screw couplings; forward fittings pending',context=context)
focus=[v for k,v in items.items() if k.startswith('PortHighSpeedBrake') and ('AnchorEnd' in k or 'CouplingScrew' in k)]
...
shaded(focus,out/'high_brake_detail.svg',(1,-1,.3),'Rear band joints | display-cut backing and lining; external anchor support pending')
...
bores=[f.Surface for f in anchor.Faces if isinstance(f.Surface,Part.Cylinder) and abs(f.Surface.Radius-r['controls']['anchor_eye_bore_radius'])<1e-6 and abs(abs(f.Surface.Axis.y)-1)<1e-6]
assert len(bores)==1

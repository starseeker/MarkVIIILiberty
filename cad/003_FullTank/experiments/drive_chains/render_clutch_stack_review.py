...
prior=HERE/'clutch_collar_build/previews/section.png'
...
crop=[320,370,385,375]
source_detail=f'<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1558" viewBox="{" ".join(map(str,crop))}">{embed(snl,0,0,1447,1055)}</svg>'
...
svg=['<svg xmlns="http://www.w3.org/2000/svg" width="1800" height="2190">','<rect width="1800" height="2190" fill="#f6f4ed"/>',
    ...
    text(915,115,'SNL21 core detail: engine left, transmission right',21),embed(out/'snl21_detail.png',905,130,875,630),
    text(30,800,'New native: relieved bearing, nested sleeve, keys and snap ring',21),
    text(920,800,'Previous checkpoint: main collar bore still vacant',21),
...
notes=[
    ...
    'SH999A now receives the larger bearing, four key beds and snap-ring groove. Eight physical pieces bring the combined native to1548 leaves.',
    ...
    'The split snap ring has an outer ridge captured in an actual receiving groove; its exact historical section, split and elastic installation are unknown.',
    'Sleeve-to-collar attachment is a nominal fitted surface, not a qualified torque connection. Thrust collar/balls, cones, plungers and engine interfaces remain ahead.'
]
...
(out/'index.html').write_text('<!doctype html><html lang="en"><meta charset="utf-8"><title>Main clutch stack source comparison</title><style>body{font:18px system-ui;background:#f6f4ed;margin:24px}img{max-width:100%}p{max-width:100ch}</style><h1>Main clutch bearing, sleeve and keyed support</h1><p>The catalogue shows a relieved bearing with a nested sleeve and an outer keyed cone support. The new and formerly empty sections are shown with independent scales. HB/SNL identities differ and literal key dimensions make thicker bands than the drawing. Cones, thrust/ball mechanism and engine interfaces remain incomplete.</p><img src="comparison.png" alt="HB and SNL sources, populated and formerly empty sections, bearing and positive sleeve"><p><a href="render_receipt.json">Source hashes, crop coordinates and limitations</a></p></html>')

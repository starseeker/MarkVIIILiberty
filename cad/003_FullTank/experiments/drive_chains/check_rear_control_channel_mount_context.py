"""Whole-material comparison of all new mounts against retained tank geometry."""
import argparse
import itertools
...
affected=[n for n in rows if n!='hull_floor_7'];selected=[];pairs=[]
...
for i,y in enumerate(r['controls']['cleat']['stations_y'],1):
    ch=r['controls']['channel'];cl=r['controls']['cleat'];q=App.Vector(ch['placement'][0]-ch['width']/2+cl['bolt_x'],y+cl['bolt_y'],ch['placement'][2]+cl['stock'])
    targets['Cleat'+str(i)+'SocketEnvelope']=Part.makeCylinder(25,60,q)
for name,one in targets.items():
    bb=bounds(one)
    for n,box in boxes.items():
        if n=='hull_floor_7' or not overlap(bb,box):continue
...
result=dict(passed=all(v['passed'] for v in pairs),pairs=pairs,affected_count=len(affected),tool_envelope_count=4,parent_occurrence_count=len(prior),retained_standard_count=sum(n not in excluded and row['representation']=='assembly' for n,row in tank.items()),native_sha256=sha(native),parent_native_sha256=sha(parent),standard_manifest_sha256=sha(sp),checker_sha256=sha(Path(__file__)),scope='All29 new channel/cleat/hardware occurrences and four conditional socket envelopes against all retained development and standard solids. Revised floor separately checked for exact holes and no added material; its local contacts included in independent_checks.json.',historical_geometry_qualified=False,installation_qualified=False)

...
p.add_argument('--worker',action='store_true');p.add_argument('--local-only',action='store_true');args=p.parse_args();out=args.candidate.resolve()
if not args.worker:
    command=[sys.executable,__file__,'--candidate',str(out),'--worker']+(['--local-only'] if args.local_only else [])
    with (out/'interface_check.log').open('w') as log:sys.exit(subprocess.run(command,env=runtime.environment(out/'check_runtime'),stdout=log,stderr=subprocess.STDOUT).returncode)
try:
    ...
    for i,(a,b) in enumerate(seats):
        ...
        for sign,word in [(1,'upper'),(-1,'lower')]:
            ...
            for fx in ([a+length*.25,a+length*.75] if i==0 else [(a+b)/2]):
                ...
                radius=inner+.3
                inside(name+' localized groove retains angular running land '+str(fx),shell,[fx,radius/math.sqrt(2),sign*radius/math.sqrt(2)])
            ...
            tip=dowel.optimalBoundingBox(False).ZMin if sign>0 else -dowel.optimalBoundingBox(False).ZMax
            near(name+' dowel tip radius',tip,c['dowel_radial_start'],1e-5)
            clearance=dowel.distToShape(shaft)[0]
            ck(name+' dowel recessed from shaft',clearance>=c['dowel_radial_start']-jr-1e-5,dict(actual_mm=clearance,minimum_mm=c['dowel_radial_start']-jr))
    ...
    hs=d['thrust_housing_span'];mid=sum(hs)/2;seat_radius=c['thrust_sleeve_outer_radius']+c['thrust_seat_gap']
    stock=Part.makeCylinder(seat_radius+cc['case_wall'],1,V(mid-.5,0,0),X).cut(Part.makeCylinder(seat_radius,3,V(mid-1.5,0,0),X))
    near('thrust housing retains selected case stock',abs(stock.cut(upper).cut(lower).Volume),0,1e-5)
    ...
    # Receiving edits must not silently alter the established shell, sump,
    # ribs, supports or other features outside their explicit machining regions.
    zones=[]
    for a,b in seats:
        zones.append(Part.makeCylinder(cc['bearing_boss_radius']+1,b-a+.2,V(a-.1,0,0),X))
        for fx in ([a+(b-a)*.25,a+(b-a)*.75] if a==seats[0][0] else [(a+b)/2]):
            zones.append(Part.makeCylinder(c['bearing_oil_feed_radius']+.1,71,V(fx,0,0),-Z))
    hs=d['thrust_housing_span'];zones.append(Part.makeCylinder(c['thrust_seat_radius']+1,hs[1]-hs[0]+.2,V(hs[0]-.1,0,0),X))
    wall=cc['case_wall'];width=cc['body_half_width']-wall;last=rows[-1]
    for height in [cc['gear_chamber_floor'],c['gear_floor_height']]:
        zones.append(Part.makeBox(cc['gear_end_after_last_row']-2*wall+.2,2*width+.2,wall+.2,V(last+wall-.1,-width-.1,height-wall-.1)))
    # The two floor bores also cut the upper end wall if an aperture intersects
    # it; these bounded receiving operations are retained explicitly.
    for x,y,rad in [(last+78,0,34),(last+25,55,9)]:
        zones.append(Part.makeCylinder(rad+.1,c['gear_floor_height']+2.2,V(x,y,-1.1)))
    region=Part.makeCompound(zones)
    for key,newcase in [('upper',upper),('lower',lower)]:
        original=parent.getObject('Def_EngineCase_'+key).Shape
        missing=original.cut(newcase).cut(region);added=newcase.cut(original).cut(region)
        near(key+' original material preserved outside receiving edits',abs(missing.Volume),0,1e-5)
        near(key+' no extra material outside receiving edits',abs(added.Volume),0,1e-5)
    ...
    write(out/'material_checks.json',dict(passed=not failures,native_sha256=sha(native),pairs=pairs,overlaps=failures,standard_context_checked=not args.local_only))

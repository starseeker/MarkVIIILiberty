...
try:
    ...
    for pair in delta['removed']:
        ...
        axial_gap = max(first.BoundBox.XMin - second.BoundBox.XMax, second.BoundBox.XMin - first.BoundBox.XMax)
        print(a, b, 'distance', gap, 'axial bbox gap', axial_gap, flush=True)
        first_trimmed, second_trimmed = first.optimalBoundingBox(False), second.optimalBoundingBox(False)
        trimmed_gap = max(first_trimmed.XMin - second_trimmed.XMax, second_trimmed.XMin - first_trimmed.XMax)
        assert axial_gap > 0 and abs(trimmed_gap - expected_gap) < 1e-5
        assert gap >= expected_gap - 1e-6 and material < 1e-5
        ...
        results.append(dict(a=a, b=b, gap_mm=gap, axial_bbox_gap_mm=axial_gap, trimmed_axial_gap_mm=trimmed_gap, minimum_gap_mm=expected_gap,
    ...
    receipt = dict(passed=True, previous_native_sha256=hashes[0], native_sha256=hashes[1],
                   ...
                   initial_probe_correction='Sleeve/cage surfaces are also radially separated; axial gap is a lower bound on Euclidean distance, not an equality. Default BRep bounds are conservative; trimmed bounds recover the design planes.',

...
p.add_argument('--projection-only',action='store_true',help='Reuse hash-verified native views and refresh the fixed source projection')
...
if not a.worker:
    ...
    if a.projection_only:args.append('--projection-only')
...
try:
    ...
    if a.projection_only:
        prior=read(out/'render_receipt.json');assert prior['native_sha256']==sha(native)
        for name in ['isometric','crossmembers','underside','front_joint','rear_joint','side_view']:
            assert sha(out/(name+'.png'))==prior['images'][name+'.png']
    ...
    overlay_ids=frame+floors+['ClutchDrive_drum','ClutchDrum_flywheel']
    ...
    write(out/'render_receipt.json',dict(native_sha256=sha(native),renderer_sha256=sha(Path(__file__)),
        reused_native_views_from_renderer_sha256=prior['renderer_sha256'] if prior else None,

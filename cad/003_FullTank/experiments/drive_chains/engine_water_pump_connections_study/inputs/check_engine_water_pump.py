...
p=argparse.ArgumentParser();p.add_argument('--candidate',type=Path,default=HERE/'engine_water_pump_connections_study')
...
try:
    ...
    from check_water_pump_additions import check_additions
    ...
    for sign in [-1,1]:
        rotation=App.Rotation(X,c['outlet_clock_deg'])
        start=V(c['scroll_center_x'],0,0)+rotation.multVec(V(0,sign*c['outlet_start_y'],sign*c['outlet_offset_z']))
        fluid=fluid.fuse(Part.makeCylinder(c['outlet_cast_bore_radius'],c['outlet_cast_length']-c['outlet_transition_length'],start,rotation.multVec(Y*sign)))

...
p.add_argument('--output',type=Path,default=HERE/'engine_water_pump_connections_study')
...
try:
    ...
    from engine_water_pump_lock import route
    ...
    for path in [Path(__file__),a.controls.resolve(),HERE/'engine_water_pump_parts.py',HERE/'engine_water_pump_sources.json',
                 HERE/'engine_water_pump_mounting.py',HERE/'engine_water_pump_lock.py',HERE/'transmission_bevel_tooth.py',HERE/'engine_crankshaft_parts.py',HERE/'engine_crossmember_parts.py',
    ...
    metadata(pump,QuantityRole='One catalogue pump assembly; nonphysical container',SourceRecord='SNL:160:016',
        Subsystem='Powerplant',Coverage='partial',ReconstructionNotes='Development pump with mounting hardware, source-dimensioned integral hose connections and full-stock drain lock wire. HB191 supports two packing pieces and inlet cast with cover; later SNL quantity and exact cast profiles remain documented.')
    ...
    ownership=dict(shaft=('LQ138A','SNL:216:004'),key=('LQ105A','SNL:114:022'),impeller_nut=('LQ147A','SNL:130:020'),
        ...
        lock_wire=('LQ167A / L209','SNL:161:006'))
    ...
    for key,s in shapes.items():
        ...
        metadata(body,DefinitionId='engine_water_pump_'+key,OriginalMark=mark,SourceRecord=rid,
            ...
            ReconstructionNotes='Source-led development: unprinted dimensions and cast profiles estimated; HB191 integral connection mapping selected; packing stock and fastener alternatives documented; threads smooth envelopes.')
        definitions[key]=body.Name
        if key in ['body','cover']:
            metadata(body,IntegralConnectionIdentity='12075 (two cast-with-body outlet tubes)' if key=='body' else '12081 (one cast-with-cover inlet tube)',
                IntegralConnectionSource='HB191 nomenclature; separate connection records represented as integral features, not additional overlapping mass',
                IntegralConnectionStock=dict(outer_diameter_mm=2*c['outlet_outer_radius'] if key=='body' else c['inlet_od'],
                    wall_mm=c['outlet_outer_radius']-c['outlet_inner_radius'] if key=='body' else c['inlet_wall'],
                    length_mm=c['outlet_tube_length'] if key=='body' else c['inlet_stub_length']),
                MaterialInterfaceStatus='Cast-with ownership selected from HB191; internal material interface not established')
        elif key=='lock_wire':
            metadata(body,MaterialEvidence='Soft iron: SNL276:001 for plug LQ145A',StockLength_mm=c['lock_wire_stock_length'],
                CenterlineLength_mm=d['drain_lock']['centerline_length_mm'],RouteEvidence='HB139 shows drain-to-cover-fastening connection; exact route, anchor loop and drilling estimated')
    reference=doc.addObject('Part::Feature','WaterPumpDrainWirePath');doc.Definitions.addObject(reference)
    reference.Shape=route(c,d['cover_centers'])[0]
    metadata(reference,QuantityRole='Nonphysical construction centerline; excluded from physical BOM and STEP',
        SourceRecord='SNL:161:006',ParameterUpdate='Regenerate engine_water_pump_build.py',GeometryInputs=c)
    ...
    write(out/'report.json',dict(status='unqualified_water_pump_development',native_file=native.name,native_sha256=sha(native),
        ...
        mounting_datums=mounting_datums,wire_centerline_object=reference_name,
        ...
        pending=['Independent geometry, source views, gear mesh and STEP checks','Qualify source hose connections and estimated wire route, case mounting, hardware stack and clocking',
                 'Retain packing stock/quantity alternatives and source stud-length discrepancies; confirm cast profiles','Oil pump and remaining engine systems','Standard tank integration'],

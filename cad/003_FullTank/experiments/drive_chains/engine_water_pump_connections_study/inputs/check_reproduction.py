...
root=Path('/home/cyapp/MarkVIIILiberty');a=root/'cad/003_FullTank/experiments/drive_chains/engine_water_pump_connections_study';b=root/'.work/engine-water-pump-lock/fresh_nominal'
read=lambda p:json.loads(p.read_text())
sha=lambda b:hashlib.sha256(b).hexdigest()
properties={'Placement','LinkPlacement','LinkTransform','Scale','ScaleVector','LinkedObject','Group','Tip','Label','Label2','OccurrenceId','DefinitionId','SurveyIds','SourceRecord','Coverage','Representation','Subsystem','QuantityRole','Visibility','GeometryInputs','IntegralConnectionIdentity','IntegralConnectionSource','IntegralConnectionStock','MaterialInterfaceStatus','MaterialEvidence','StockLength_mm','CenterlineLength_mm','RouteEvidence'}
def data(path):
...
result=dict(passed=all(checks.values()),checks=checks,candidate_native_sha256=ra['native_sha256'],fresh_native_sha256=rb['native_sha256'],serialized_shapes=len(da[0]),checked_properties=len(da[1]),scope='Independent empty-output rebuild; exact serialized BRep, all object types, placement/link/hierarchy/identity metadata and frozen inputs compared. Fresh builder also saved/reopened2247 occurrences; nominal native acceptance applies through these identical geometry/assembly data.')

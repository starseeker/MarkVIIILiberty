...
p=argparse.ArgumentParser();p.add_argument('--candidate',type=Path,default=HERE/'engine_water_pump_connections_study');p.add_argument('--worker',action='store_true')
...
try:
    ...
    COLORS.update(PumpCasting=(.63,.69,.65),PumpSteel=(.57,.65,.71),PumpBearing=(.72,.75,.77),PumpPacking=(.35,.38,.34),PumpGasket=(.65,.45,.31),PumpBronze=(.76,.62,.35),PumpWire=(.28,.32,.35))
    clip=Part.makeBox(400,150,300,V(-60,0,-150));names=[]
    def draw(name,title,direction,section=False,context=False,explode=False,detail=False):
        rendered=[]
        for ident in r['new_ids']+(['EngineLowerDrive_IntegralDriver','EngineCase_lower'] if context else []):
            ...
            if detail and key not in ['body','cover','plug','plug_gasket','lock_wire'] and ident not in ['EngineWaterPump_'+n+'6' for n in ['CoverStud','CoverNut','CoverWasher','CoverCotter']]:continue
            ...
            if key=='lock_wire':color='PumpWire'
            ...
            if explode:
                shift=0
                if key in ['body','joint_gasket','plug','plug_gasket','lock_wire']:shift=80
            ...
            if detail:
                shape=shape.common(Part.makeBox(85,95,65,V(130,-65,-103)))
                # The crop can leave detached hose-bead slivers from the cover.
                # Retain its main context piece; this changes display only.
                if key=='cover' and len(shape.Solids)>1:
                    shape=max(shape.Solids,key=lambda solid:solid.Volume)
    ...
    draw('drain_lock','Drain lock wire | source stock; estimated formed route and attachment',(.7,-1,-.4),detail=True)
    source=read(HERE/'engine_water_pump_sources.json');pairs=[]
    for suffix,view in [('figure_079_original.png','section_iso'),('p289-geometry.png','exploded'),('MarkVIII043.jpg','isometric'),('plate139.png','drain_lock'),('MarkVIII096.jpg','isometric')]:
    ...
    (out/'index.html').write_text('<!doctype html><meta charset="utf-8"><title>Water-pump development review</title><style>body{font:18px system-ui;margin:24px;background:#f6f4ed}img{max-width:100%}.pair{display:grid;grid-template-columns:1fr 1fr;gap:16px;align-items:center}</style><h1>Water-pump development candidate</h1><p>HB191 supplies the hose-end stock, integral connection ownership and selected two packing pieces. Cast profiles, commercial bearing internals and the exact wire route remain estimates; later catalogue packing and fastener alternatives are retained. Source cameras are not registered. Sections, detail cropping and exploded offsets affect display only; the separated wire is unhooked for the display, not a service pose.</p>'+''.join(pairs)+''.join(f'<h2>{n}</h2><img src="{n}.png">' for n in names))

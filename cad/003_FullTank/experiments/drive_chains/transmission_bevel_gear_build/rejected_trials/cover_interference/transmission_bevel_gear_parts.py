...
import Part
from transmission_core_parts import cylinder,box,revolve,spline_envelope
...
def formed_rivet(radius,low,high,length,height):
    ...
    for face,sign in [(low,-1),(high,1)]:
        sphere=Part.makeSphere(sphere_radius,App.Vector(0,face-sign*(sphere_radius-height),0))
        caps.append(sphere.common(cylinder(tail_radius+1,min(face,face+sign*height),max(face,face+sign*height))))
...
def thrust_bearing(c,station):
    ...
    for n in range(c['ball_count']):
        ...
        balls.append(Part.makeSphere(r,pos));cage=cage.cut(Part.makeSphere(r+c['cage_ball_gap'],pos))

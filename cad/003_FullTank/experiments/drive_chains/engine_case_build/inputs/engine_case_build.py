...
try:
    ...
    for path in [Path(__file__),HERE/'engine_case_parts.py',HERE/'engine_case_supports.py',HERE/'engine_case_sources.json',HERE/'engine_suspension_parts.py',HERE/'engine_crossmember_parts.py',a.controls.resolve()]:
    ...
    support['front_bracket'],datums['front_yoke_profile']=front_yoke(c,sc,cc,origin.z)
    ...
    for n in changed:metadata(old[n]['object'],ReconstructionNotes='Provisional engine casing integration: rail extension, lower hub and dropped yoke arms. Source support hardware retained; historical mounting unqualified.')

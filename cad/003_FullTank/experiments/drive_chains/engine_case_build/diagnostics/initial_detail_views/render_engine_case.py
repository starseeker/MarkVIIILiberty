...
p=argparse.ArgumentParser();p.add_argument('--candidate',type=Path,default=HERE/'engine_case_build');p.add_argument('--worker',action='store_true')
...
try:
    ...
    draw(['EngineCase_upper'],'upper_underside','Upper casting underside | eight webs, seven bearing seats and dry gear chamber',(-1,1,-.65))
    draw(['EngineCase_lower'],'lower_casting','LQ180A lower casting | integral saddles, oil trough, wells and pump receiver',(1,1,.8))
    ...
    names=['isometric','upper_casting','upper_underside','lower_casting','longitudinal_section','transverse_section','side_view','SNL2_engine_context','source_overlay']
    write(out/'render_receipt.json',dict(native_sha256=sha(native),renderer_sha256=sha(Path(__file__)),source_sha256=sha(source),source_crop=[5440,1980,7250,3270],projected_occurrences=projected,images={n+'.png':sha(out/(n+'.png')) for n in names}))

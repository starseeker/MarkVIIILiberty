...
try:
    ...
    from engine_oil_pump_lock import route as relief_lock_route
    ...
    for key,shape in definitions.items():
        ...
        special={'mount_stud':'HB:nomenclature:196:052','mount_gasket':'HB:nomenclature:199:004'}
        rid=special[key] if key in special else (f'HB:nomenclature:203:{hb_rows[key]:03}' if key in hb_rows else 'SNL:160:005')
        row=indexed[rid];raw=json.loads(row['raw_json']);mark=raw.get('mark','LQ65A')
        refs=[rid]
        if key=='relief_lock_wire':refs+=['SNL:160:014','HB:printed87:relief cage to uncottered bolt118']
        if key=='mount_stud':refs.append('SNL:237:025')
    ...
    for row in occurrences:
        ...
        if row.get('source_records'):prop(link,'SourceRecords',row['source_records'],'App::PropertyStringList')
        s=objects[row['key']].Shape.copy();s.Placement=link.LinkPlacement.multiply(s.Placement);shapes.append(s)
    path,_=relief_lock_route(c,datums)
    reference=doc.addObject('PartDesign::Feature','ReliefLockCenterline');library.addObject(reference);reference.Shape=path
    prop(reference,'QuantityRole','Nonphysical construction reference; excluded from BOM and STEP solids')
    ...
    inputs += [HERE/'engine_oil_pump_lock.py',HERE/'engine_water_pump_lock.py']
    inputs += [HERE/'engine_oil_pump_mounting.py']

"""Strict per-definition and installed STEP comparison for the saved low-speed/foot-brake spring anchor pair."""
import argparse
import math
...
for scope,shapes in [('Definitions',definitions),('Installed',installed)]:
    path=out/('RearLowSpringBrackets'+scope+'.step');assert not path.exists();doc=App.newDocument('LowSpringExchange'+scope)
    try:
        objects=[]

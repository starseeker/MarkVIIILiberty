...
def parts(c,progress=None):
    ...
    lower=ring(ro,ri,bottom,0).fuse(cylinder(fr,-c['mount_flange_stock'],0))
    lower=lower.fuse(ring(fr,ri,bottom,bottom+c['bottom_flange_stock']))
    nose=Part.Face(Part.makePolygon([V(-fr+c['mount_nose_overlap'],-c['mount_nose_half_width'],0),V(c['mount_nose_x'],0,0),V(-fr+c['mount_nose_overlap'],c['mount_nose_half_width'],0),V(-fr+c['mount_nose_overlap'],-c['mount_nose_half_width'],0)]))
    nose=nose.extrude(V(0,0,-c['mount_flange_stock']));lower=lower.fuse(nose)

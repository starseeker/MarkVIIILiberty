"""Export and strictly compare spacer/spring definitions and all eight affected installed components."""
import argparse
import math
...
assert len(definitions)==2 and len(installed_names)==8
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('BrakeSpacers' + scope + '.step')
    ...
    doc = App.newDocument('BrakeSpacersExchange'+scope)
    try:
        features = []
    ...
    for name, one in shapes.items():
        ...
        initial_mass = None
        if (name == 'Def_BrakeFront_spring' or name.endswith('AdjustingSpring')) and not (ma['converged'] and mb['converged']):
            initial_mass = dict(native=ma, step=mb)
            measurement_frame = App.Placement() if scope == 'Definitions' else App.Placement(App.Matrix(*rows[name]['frame']))
            ma, mb = partitioned.measure(one, measurement_frame), partitioned.measure(two, measurement_frame)
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='Two new/changed definitions and all eight affected occurrences; adaptive converged mass integration and unchanged strict material/tolerance criteria. Not a complete brake mechanism or full-tank export.',

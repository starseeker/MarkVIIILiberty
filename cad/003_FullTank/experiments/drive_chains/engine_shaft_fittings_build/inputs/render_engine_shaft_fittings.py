...
try:
    ...
    for name in ['Pin6', 'Main2', 'Gear']:
        ...
        length_label = 'derived-length' if name == 'Gear' else 'source-length'
        draw(selection, name.lower()+'_section', name+' retaining set | cap seats, gaskets and '+length_label+' stud',
             tuple(rotation.multVec(V(.06, -1, .3))), clip)
    pd = r['parent_datums']
    tip = pd['taper']['rear']-r['parent_controls']['output_thread_length']
    clip = Part.makeBox(35-tip, 200, 80, origin+V(tip-5, -100, 0))
    draw(['EngineCrank_Forging', 'ShaftClosure_NosePlug', 'EngineCrank_FlywheelKey',
         ...
         'nose_section', 'Nose section | plug clears cotter; reduced cavity leaves a blind key-screw receiver', (0, -.15, -1), clip)

"""Export and strictly compare rebuilt casing definitions and all affected installed components."""
import argparse
...
definitions = {name: definition(name) for name in r['replaced_definitions']}
installed = {}
installed_names = set(r['affected_occurrences'])
assert len(definitions)==12 and len(installed_names)==157
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('ChainCasings' + scope + '.step')
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='12 rebuilt definitions and 157 affected installed components; not a full-tank export.',

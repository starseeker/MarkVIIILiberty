...
def coverage(fs,gs):
    ...
    for f in fs:
        matching=[g for g in gs if abs(f.Surface.Radius-g.Surface.Radius)<1e-7 and f.Surface.Axis.cross(g.Surface.Axis).Length<1e-7
            and (f.Surface.Center-g.Surface.Center).cross(g.Surface.Axis).Length<1e-7 and g.BoundBox.YMin<=f.BoundBox.YMin+1e-7 and g.BoundBox.YMax>=f.BoundBox.YMax-1e-7]

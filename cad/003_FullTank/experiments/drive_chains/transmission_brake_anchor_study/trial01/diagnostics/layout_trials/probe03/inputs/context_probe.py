...
read=lambda p:json.loads(p.read_text());m=read(H/'transmission_brake_link_study/trial03/isolated/manifest.json');r=read(ROOT/'.work/transmission-brake-anchors/probe03/report.json');d=r['dimensions'];c=read(H/'transmission_brake_anchor_study/controls.json')['controls']
...
def fresh(role,pose):
 if role not in new_cache:
  s=Part.Shape();s.read(str(ROOT/'.work/transmission-brake-anchors/probe03'/(role+'.brep')));assert s.Placement.isIdentity();new_cache[role]=s

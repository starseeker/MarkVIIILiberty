"""Compare all inherited rear-anchor definition material with its saved parent.
...
the three newly added definitions.
"""
import argparse
...
assert qualification['local_anchor_checks_passed']
...
assert changed == {'Def_BrakeBand_low_band', 'Def_BrakeBand_track_band'}
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 472 unchanged inherited definitions compared with the qualified brake-link parent. Two backing revisions checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 472

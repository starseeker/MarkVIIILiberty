"""Compare every inherited definition against the saved pre-joint powertrain parent."""
import argparse
...
assert not changed
assert len(source_manifest['definitions']) == 564
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All564 inherited definitions preserved. Three new definitions and the existing shared nut match the independently tested track-joint prototype; four-definition and16-installed-shape transfer is checked separately. Does not qualify placements or historical interpretation.',

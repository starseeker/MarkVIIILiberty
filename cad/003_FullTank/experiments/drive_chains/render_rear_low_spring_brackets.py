"""Render actual saved low-speed/foot-brake spring supports and compare them through the unchanged local source registration."""
import argparse
...
context_names=[n for n in prior if n.startswith(('RearChannelLeftCleat', 'ClutchSupport_')) or n=='hull_floor_7' or any(v.endswith('ControlFulcrum') for v in prior[n].get('owners', []))]
...
shaded(items,out/'low_spring_isometric.svg',(-1,-1,.8),'Rear controls | two low-speed/foot-brake spring supports; clearance study',context=floor)
shaded([v for v in items if v['id'].startswith('PortLowSpring')],out/'low_spring_detail.svg',(-1,-1,.5),'M4136 | shared spring anchor and two source-listed rivets')
...
draw.text((1130,690),'Fixed side registration; port low-speed/foot-brake spring support only. No plan calibration.',fill='#111111')
im.save(out/'source_overlay.png');im.crop((1130,685,1680,970)).resize((1650,855)).save(out/'source_detail.png')
write(out/'render_receipt.json',dict(native_sha256=sha(native),manifest_sha256=sha(out/'isolated/manifest.json'),renderer_sha256=sha(Path(__file__)),parent_native_sha256=sha(parent),source_registration_sha256=sha(regpath),camera=camera,source_camera_refitted=False,images={n:sha(out/n) for n in ['low_spring_isometric.png','low_spring_detail.png','source_overlay.png','source_detail.png']},scope='Saved7-occurrence trial plus retained channel mounts, fulcrums, clutch support and floor context. Source side projection diagnoses the conditional layout; no historical perspective or plan fit.',historical_geometry_qualified=False))

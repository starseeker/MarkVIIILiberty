...
ck('Exactly four inherited definitions intentionally revised',set(r['changed_definitions'])=={'Def_HighBrake_long_band','Def_HighBrake_short_band','Def_HighBrake_anchor_end','Def_TransmissionCore_bevel_case'})
...
for role in ['long','short']:
 key='Def_HighBrake_'+role+'_lining';before=definition(key,True);after=definition(key)
 differences=[before.cut(after),after.cut(before)]
 ck(role+' printed lining material and metadata unchanged',all(d.isValid() and not d.Faces and abs(d.Volume)<1e-5 for d in differences) and m['definitions'][key]['properties']==old['definitions'][key]['properties'],exact_brep_bytes=m['definitions'][key]['brep_sha256']==old['definitions'][key]['brep_sha256'],difference_faces=[len(d.Faces) for d in differences])

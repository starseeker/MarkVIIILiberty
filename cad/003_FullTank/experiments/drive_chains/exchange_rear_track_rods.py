"""Strict per-definition and installed STEP comparison for the saved straight rods and revised receiving assembly."""
import argparse
import math
...
mass=AdaptiveMass(out/'adaptive_mass_runtime');definitions={};installed={}
...
for scope,shapes in [('Definitions',definitions),('Installed',installed)]:
    path=out/('RearTrackConnectingRods'+scope+'.step');assert not path.exists();doc=App.newDocument('TrackRodExchange'+scope)
    try:
        objects=[]
...
write(out/'exchange_checks.json',dict(passed=all(v['passed'] for v in checks),checks=checks,native_sha256=sha(native),checker_sha256=sha(Path(__file__)),mass_provenance=mass.provenance,step_hashes=files,export_settings={'write.surfacecurve.mode':1,'Mod/Import/ExportKeepPlacement':True},historical_geometry_qualified=False,installation_qualified=False))

...
def formed_pin(c):
    ...
    for sign in [-1,1]:
        ...
        pieces += [axial(wire,start,App.Vector(0,1,0),straight),torus,axial(wire,bend_end,tangent,tail)]
    ...
    points += [App.Vector(0,top-eye*math.sin(a),eye*math.cos(a)) for a in [math.pi-i*math.pi/16 for i in range(1,17)]]
    points.append(App.Vector(0,head,half))
    for a,b in zip(points,points[1:]):pieces.append(axial(wire,a,b-a,(b-a).Length))
    ...
    return shape,dict(straight_length_mm=straight,bend_arc_length_mm=arc,tail_length_mm=tail,
        ...
        construction='Analytic cylinder/torus segments; eye approximated with cylindrical segments and sphere junctions.')

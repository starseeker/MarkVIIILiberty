...
def lower_wire(x,c,scale=1,inside=False):
    ...
    segments=[
      [[-w,-24*scale],[-w,-116*scale+t],[-135*scale+t,-150*scale+t],[-55*scale+t,-depth]],
      [[-55*scale+t,-depth],[-45*scale+t,-depth-1],[-50*scale+t,-trough],[-38*scale+t,-trough]],
      [[38*scale-t,-trough],[50*scale-t,-trough],[45*scale-t,-depth-1],[55*scale-t,-depth]],
      [[55*scale-t,-depth],[135*scale-t,-150*scale+t],[w,-116*scale+t],[w,-24*scale]]]
    top=1 if inside else 0;edges=[Part.makeLine(V(x,-w,top),V(x,-w,-24*scale))]
    for i,pts in enumerate(segments):
        if i==2:edges.append(Part.makeLine(V(x,-38*scale+t,-trough),V(x,38*scale-t,-trough)))
...
def parts(c,progress=None):
    ...
    gx=last+58;ports=[]
    for sign in [-1,1]:
        axis=V(0,sign*math.sin(angle),math.cos(angle));port_origin=V(gx,0,0)+axis*120;upper=upper.cut(Part.makeCylinder(c['gear_inclined_bore_radius'],280,port_origin,axis))
        ports.append(dict(name='inclined_'+str(sign),origin=list(port_origin),axis=list(axis),radius=c['gear_inclined_bore_radius']))
    for name,x,y,r in [('vertical',last+78,0,c['gear_vertical_bore_radius']),('breather',last+25,0,15.0)]:
        ...
        port_z=60 if name=='breather' else 0
        upper=upper.cut(Part.makeCylinder(r,320,V(x,y,port_z)));ports.append(dict(name=name,origin=[x,y,port_z],axis=[0,0,1],radius=r))

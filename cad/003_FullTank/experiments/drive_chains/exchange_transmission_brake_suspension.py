"""Export and strictly compare brake suspension definitions and all six installed supports."""
import argparse
...
assert len(definitions)==2 and len(installed_names)==6
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('BrakeSuspension' + scope + '.step')
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='Two new support definitions and six installed components; not a full-tank export.',

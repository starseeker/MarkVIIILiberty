...
assert changed == {'Def_HighBrakeSupport_anchor_bracket'}
assert len(source_manifest['definitions']) == 532
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 531 unchanged inherited definitions compared with the receiving-web development parent. The revised M362 bracket and seven new definitions are checked separately against the bottom-stop prototype. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 531

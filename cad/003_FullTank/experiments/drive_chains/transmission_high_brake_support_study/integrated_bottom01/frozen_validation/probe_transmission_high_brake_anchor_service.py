...
import Part
V=A.Vector
p=argparse.ArgumentParser();p.add_argument('--candidate',type=Path,required=True);p.add_argument('--withdrawal-mm',type=float,default=80.);p.add_argument('--drift-mm',type=float,default=100.);p.add_argument('--output',type=Path);a=p.parse_args();out=a.candidate.resolve();resultdir=a.output.resolve() if a.output else out
if resultdir!=out:assert not resultdir.exists();resultdir.mkdir(parents=True)
r=read(out/'report.json');parent=H/'transmission_high_brake_support_study/integrated01';m=read(parent/'isolated/manifest.json');pr=read(parent/'report.json')
...
c=r['controls'];d=r['details'];seat=d['pin_head_seat_y_mm'];tip=d['pin_tip_y_mm'];distance=a.withdrawal_mm;tools={};assert distance>=c['pin_under_head_length']
for hand,pose in frames.items():
 ...
 for role,s in [('withdrawal',shaft.fuse(head)),('drift_access',Part.makeCylinder(5,a.drift_mm,V(0,tip,0),V(0,1,0)))]:
  s.Placement=pose;tools[hand+'_'+role]=s;s.exportBrep(str(resultdir/(hand+'_'+role+'_envelope.brep')))
records=[]
...
write(resultdir/'pin_service_probe.json',dict(passed=all(x['passed'] for x in records),pairs=records,neighbor_count=len(neighbors),native_sha256=r['prototype_native_sha256'],parent_native_sha256=r['parent_native_sha256'],probe_sha256=sha(Path(__file__)),withdrawal_mm=distance,drift_radius_mm=5,drift_access_length_mm=a.drift_mm,minimum_pin_withdrawal_mm=c['pin_under_head_length'],scope='Conservative straight pin withdrawal and a stated-length drift approach after cotter/bottom-stop removal, against selected retained epicyclic/frame/brake material. Gear extraction from tank, cotter unbending/tool handling and complete service sequence are not qualified.',installation_qualified=False))

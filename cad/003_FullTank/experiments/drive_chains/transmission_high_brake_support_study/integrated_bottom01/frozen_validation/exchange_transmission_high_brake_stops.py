"""Strict STEP round trips for saved bottom-stop definitions and installed poses."""
...
assert len(definitions)==8 and len(installed)==22
checks=[];files={}
for scope,shapes in [('Definitions',definitions),('Installed',installed)]:
 path=out/('HighBrakeBottomStop'+scope+'.step');assert not path.exists();doc=App.newDocument('BottomStopExchange'+scope)
 try:
  features=[]
 ...
 for name,one in shapes.items():
  ...
  ma,mb=mass.measure(one),mass.measure(two);dc=math.dist(ma['centroid_mm'],mb['centroid_mm'])
  ...
  checks.append(dict(scope=scope,name=name,passed=passed,imported_solid_index=i,native_tolerance_mm=ta,step_tolerance_mm=tb,missing_mm3=missing.Volume,added_mm3=added.Volume,fuzzy_missing_faces=fm,fuzzy_added_faces=fa,native_mass=ma,step_mass=mb,centroid_error_mm=dc))

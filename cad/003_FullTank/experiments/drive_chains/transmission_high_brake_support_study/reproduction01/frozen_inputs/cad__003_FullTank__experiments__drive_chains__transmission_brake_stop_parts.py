...
def parts(c, anchor, saved_bands, stations, mount_xz):
    ...
    # A circular upper eye fits the existing machined nut seat. The initial
    # rectangular tab clipped the cap outside that seat; retain trial01.
    tab = Part.makeCylinder(c['mount_tab_radius'], t, inverse.multVec(V(mx,0,mz)),
                            inverse.multVec(V(1,0,0)))

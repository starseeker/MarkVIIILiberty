...
ck('Eight new occurrences, three definitions and two assembly groups',len(rows)==3129 and len(m['definitions'])==532 and len(m['assemblies'])==324 and set(rows)-set(prior)==set(r['expected_new_occurrences']))
...
prototype=ROOT/r['prototype'];pnative=prototype/'ReceivingWebStudy.FCStd';assert sha(pnative)==r['prototype_native_sha256']
doc=App.openDocument(str(pnative));pshapes={};pframes={}
try:
 for role in ['case','anchor_bracket','anchor_lock_plate','mount_screw']:pshapes[role]=doc.getObject('Def_'+role).Shape.copy()
 for link in doc.Root.Group:pframes[link.Name]=list(doc.Root.getGlobalPlacement().multiply(link.LinkPlacement).toMatrix().A)
finally:App.closeDocument(doc.Name)
for role,one in pshapes.items():
 key='Def_TransmissionCore_bevel_case' if role=='case' else 'Def_HighBrakeSupport_'+role
...
mapping={'CenterTransmissionCore_bevel_case':'Case'}
for hand in ['Port','Starboard']:
 for suffix in ['AnchorBracket','AnchorLockPlate','AnchorMountScrew1','AnchorMountScrew2']:mapping[hand+'HighSpeedBrake'+suffix]=hand+suffix
...
dependencies=[]
for path in [prototype/'report.json',prototype/'receiver_interface_checks.json',prototype/'exchange_refined02/exchange_checks.json']:
...
write(out/'independent_checks.json',dict(passed=all(v['passed'] for v in checks),checks=checks,native_sha256=sha(native),source_native_sha256=sha(source),prototype_native_sha256=sha(pnative),checker_sha256=sha(Path(__file__)),prototype_checks=dependencies,
 scope='Saved counts/hierarchy, inherited occurrence frames, and strict material/placement transfer for the four tested prototype definitions and nine occurrences. Whole-parent definition preservation and standard-tank context remain separate gates.',

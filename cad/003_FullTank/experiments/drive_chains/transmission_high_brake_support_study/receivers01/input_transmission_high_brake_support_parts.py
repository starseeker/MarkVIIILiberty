...
def parts(c,old_case):
    ...
    for role in ['top','bottom']:
        ...
        for station in c[role+'_bolt_stations']:
            ...
            pose=App.Placement(base,App.Rotation(V(0,0,1),n))
        ...
        if role=='bottom':
            tools=[]
            for f in poses:
                drill=Part.makeCylinder(c['mount_bore_radius'],stock+2,V(0,0,-1));drill.Placement=App.Placement(App.Matrix(*f));tools.append(drill)
    ...
    return new,dict(mounts=mounts,anchor_center_brake_mm=c['anchor_center'],
        ...
        limitations=['Unprinted receiver and bracket sections are conditional estimates.',
            ...
            'This initial scope precedes the pin, locking plates and stop members.']),dict(addition_envelope=addition,receiving_holes=Part.makeCompound(holes))

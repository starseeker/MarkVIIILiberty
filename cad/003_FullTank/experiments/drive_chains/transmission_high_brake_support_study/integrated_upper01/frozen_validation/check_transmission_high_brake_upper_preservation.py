"""Compare every inherited definition against the saved receiving-web development parent."""
import argparse
...
assert len(source_manifest['definitions']) == 539
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 538 unchanged inherited definitions compared with the bottom-stop development parent. The revised receiving case and three new definitions are checked separately against the upper-stop prototype. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 538

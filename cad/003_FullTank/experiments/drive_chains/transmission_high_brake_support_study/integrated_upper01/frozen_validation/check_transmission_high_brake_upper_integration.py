...
ck('Sixteen new occurrences, three definitions and four assembly groups',len(rows)==3165 and len(m['definitions'])==542 and len(m['assemblies'])==336 and set(rows)-set(prior)==set(r['expected_new_occurrences']))
...
try:
 for role in ['case','top_stop','back_stop','clip','lock_plate','mount_screw','stop_screw','stop_nut']:pshapes[role]=doc.getObject('Def_'+role).Shape.copy()
...
for role,one in pshapes.items():
 key='Def_TransmissionCore_bevel_case' if role=='case' else r['shared_definitions'].get(role,'Def_HighBrakeUpper_'+role)
...
write(out/'independent_checks.json',dict(passed=all(v['passed'] for v in checks),checks=checks,native_sha256=sha(native),source_native_sha256=sha(source),prototype_native_sha256=sha(pnative),checker_sha256=sha(Path(__file__)),prototype_checks=dependencies,
 scope='Saved counts/hierarchy, inherited occurrence frames, and strict material/placement transfer for the eight tested prototype definitions and17 occurrences. Whole-parent definition preservation and standard-tank context remain separate gates.',

...
r=read(out/'report.json');native=out/'HighBrakeUpperStopStudy.FCStd';assert sha(native)==r['prototype_native_sha256']
...
svg=['<svg xmlns="http://www.w3.org/2000/svg" width="1445" height="1850">','<rect width="100%" height="100%" fill="white"/>',
 '<text x="30" y="34" font-family="sans-serif" font-size="22">HB133 | M399/M398 upper stops and M365 clip in retained registration</text>',
...
svg+=['<text x="30" y="1745" font-family="sans-serif" font-size="18">Red: saved top stop, back stop, clip and shared hardware. Fixed source registration.</text>',
 '<text x="30" y="1780" font-family="sans-serif" font-size="18">Strap stock, bends, clip wrap and stop setting are estimates. Shared MX61 plate and screw definitions retained.</text>',
...
# Keep the earlier source overlay: the fixed diagnostic source registration is retained.
if (out/'upper_stop_source.svg').exists():assert (out/'upper_stop_source.svg').read_text()=='\n'.join(svg)
(out/'upper_stop_source.svg').write_text('\n'.join(svg))
with fitz.open(stream=(out/'upper_stop_source.svg').read_bytes(),filetype='svg') as f:f[0].get_pixmap().save(str(out/'upper_stop_source.png'))
...
for name,s in shapes.items():
 ...
 role=roles[name];system='Bracket' if role=='Def_top_stop' else 'Stop' if role in ['Def_back_stop','Def_clip'] else 'Hardware'
...
parent=H/'transmission_high_brake_support_study/integrated_bottom01';m=read(parent/'isolated/manifest.json');context=[]
for row in m['occurrences']:
 if row['name'] not in [hand+'HighSpeedBrake'+part for hand in ['Port','Starboard'] for part in ['LongBand','ShortBand','AnchorEnd','LongFrontEnd','ShortFrontEnd']]:continue
...
shaded(items,out/'upper_stop_isometric.svg',(1,-1,.45),'High-speed upper stops and clips | both installed sides',context=context)
shaded([v for v in items if v['id'].startswith('Port')],out/'upper_stop_detail.svg',(.5,-1,.7),'Port top stop, back stop and clip | source-led reconstruction',context=[v for v in context if v['id'].startswith('Port')])
write(out/'review_render_receipt.json',dict(native_sha256=sha(native),source_registration_sha256=sha(regpath),source_sha256=sha(source),
 renderer_sha256=sha(Path(__file__)),image_sha256=sha(out/'upper_stop_source.png'),additional_images={n:sha(out/n) for n in ['upper_stop_isometric.png','upper_stop_detail.png']},sections=sections,camera_refit=False,historical_geometry_qualified=False))
print('Rendered saved upper-stop prototype in fixed source registration',flush=True)

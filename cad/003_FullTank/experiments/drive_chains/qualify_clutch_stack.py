"""Bind the reviewed main-clutch bearing, sleeve and keyed support checkpoint."""
import argparse
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--candidate', type=Path, default=HERE / 'clutch_stack_build')
    ...
    parent = HERE / 'clutch_collar_build'
    ...
    assert sha(parent / 'TransmissionWithClutchCollar.FCStd') == report['parent_native_sha256']
    ...
    dossier = read(HERE / 'clutch_stack_sources.json')
    ...
    artifacts = {
        'ClutchStackInstallation.step': 'step_sha256',
        'ClutchStackDefinitions.step': 'definition_step_sha256',
        'inputs/parent_collar.brep': 'parent_collar_sha256',
    }
    ...
    assert variants['checker_sha256'] == sha(HERE / 'check_clutch_stack_variants.py')
    assert variants['parts_sha256'] == sha(HERE / 'clutch_stack_parts.py')
    assert variants['standard_native_hashes'] == report['standard_native_hashes']
    assert len(variants['scenarios']) == 2
    for name, summary in zip(['smaller', 'larger'], variants['scenarios']):
    ...
    assert len(expected_images) == 8 and set(visual['image_hashes']) == expected_images
    ...
    result = dict(
        ...
        source_dossier_sha256=sha(HERE / 'clutch_stack_sources.json'), input_hashes=report['input_hashes'],

...
for hand,sign in [('Port',1),('Starboard',-1)]:
    for role,drum in [('LowSpeed',hand+'TransmissionCore_brake_case'),('Track',hand+'TransmissionOutput_drum')]:
        ...
        assert c['longitudinal_datum']=='shaft'
        pin_x=interface['axis_center_mm'][0]-(c['source_pin_px'][0]-c['source_axis_px'][0])*d['scale_x_mm_px']
        interface['bracket_x_shift_from_channel_registration_mm']=pin_x-(top[3]+d['pin_x_relative_channel_mm'])
        placements[key+'Bracket']=dict(role='bracket',group=key+'BrakeSupport',
            frame=list(App.Placement(V(pin_x,y,top[11]-d['pin_drop_mm']),App.Rotation()).toMatrix().A))
        if role=='Track':

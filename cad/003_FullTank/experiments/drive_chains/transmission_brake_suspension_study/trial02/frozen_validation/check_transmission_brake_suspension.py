...
for name in expected:
    ...
    if is_bracket:
        source_dx=(c['source_pin_px'][0]-c['source_axis_px'][0])*c['channel_depth']/(c['source_channel_x_px'][1]-c['source_channel_x_px'][0])
        near(name+' pin longitudinal station follows source shaft separation',rows[name]['frame'][3],face.Surface.Center.x-source_dx)
        near(name+' pin drop follows retained upper channel',rows[name]['frame'][11],rows['TransmissionFrame_TopChannel']['frame'][11]-d['pin_drop_mm'])

...
for name,row in rows.items():
    ...
    for i,(dx,dy) in enumerate(c['rivet_points_from_rear']):
        ...
        joints.append(dict(name=name+'Rivet'+str(i+1),kind='gusset_channel',group=name+'Joints',gusset=name,channel=channel,
            local_xy_mm=[point.x,point.y],frame=list(world.toMatrix().A)))
for side,sign in [('Left',1),('Right',-1)]:
    plate_name='TransmissionFrame_MiddleDiaphragm';angle_name='TransmissionFrame_'+side+'InnerAngle'
    pp=pose(plate_name);ap=pose(angle_name);angle_key=rows[angle_name]['definition']
    changed[angle_key]=load(angle_key)
    for i,fraction in enumerate(c['diaphragm_rivet_fractions']):
        point=V(rear+c['angle_stock']+c['diaphragm_stock'],sign*(inner_y-c['diaphragm_rivet_inset']),
                old.BoundBox.ZMin+fraction*old.BoundBox.ZLength)
        world=pp.multiply(App.Placement(point,App.Rotation(V(0,1,0),-90)))
        tool=Part.makeCylinder(c['rivet_diameter']/2+c['hole_radial_clearance'],s+t+2,
                              V(point.x+1,point.y,point.z),V(-1,0,0))
        changed[diaphragm]=changed[diaphragm].cut(tool)
        tool.Placement=ap.inverse().multiply(pp);changed[angle_key]=changed[angle_key].cut(tool)
        joints.append(dict(name='FrameDiaphragm'+side+'Rivet'+str(i+1),kind='diaphragm_upright',
            group='FrameDiaphragm'+side+'Joints',gusset=angle_name,channel=plate_name,
            local_height_fraction=fraction,frame=list(world.toMatrix().A)))
...
for j in joints:
    key=j['group']
    if key not in groups:groups[key]=assembly(key,root)
...
new_assemblies=[root.Name]+[v.Name for v in groups.values()]
...
write(out/'report.json',dict(native_file=native.name,native_sha256=sha(native),source_native=str(source.relative_to(ROOT)),
    ...
    new_assemblies=new_assemblies,affected_occurrences=sorted(affected),
    expected_physical_occurrences=len(rows)+32,expected_definition_count=len(m['definitions'])+1,
    expected_assembly_count=len(m['assemblies'])+len(new_assemblies),diaphragm_half_width_mm=half,

"""Compare every inherited definition against the saved qualified forward-fitting parent."""
import argparse
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 519 unchanged inherited definitions compared with the qualified forward-fitting parent. Ten new definitions are checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 519

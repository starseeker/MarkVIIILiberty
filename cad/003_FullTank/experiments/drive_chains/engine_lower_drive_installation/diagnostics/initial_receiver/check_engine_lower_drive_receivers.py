...
try:
    ...
    # Source oil entries sit above the lug: the case must not plug those ports.
    oil_z=r['datums']['bearing_span'][1]-uc['dowel_end_offset']
    for sign in [-1,1]:
        route=Part.makeCylinder(uc['oil_port_diameter']/2,12,V(apex+sign*(uc['housing_radius']-1),0,oil_z),V(sign,0,0))
        zero('oil entry remains exposed '+str(sign),route.common(case))

...
root=Path('/home/cyapp/MarkVIIILiberty');a=root/'cad/003_FullTank/experiments/drive_chains/engine_lower_drive_installation';b=root/'.work/engine-distribution/receiver_fresh_reproduction'
...
result=dict(passed=all(checks.values()),checks=checks,candidate_native_sha256=ra['native_sha256'],fresh_native_sha256=rb['native_sha256'],serialized_shapes=len(da[0]),checked_properties=len(da[1]),scope='Independent empty-output rebuild; exact serialized BRep, all object types, placement/link/hierarchy/identity metadata and frozen inputs compared. Fresh builder also saved/reopened2170 occurrences; nominal native acceptance applies through these identical geometry/assembly data.')

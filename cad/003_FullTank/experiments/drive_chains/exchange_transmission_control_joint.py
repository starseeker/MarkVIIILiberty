...
try:
 ...
 for name,spec in r['specs'].items():
  link=doc.getObject(name);pose=doc.getObject(spec['owner']).getGlobalPlacement().multiply(link.LinkPlacement);assert max(abs(x-y) for x,y in zip(pose.toMatrix().A,r['specs'][link.Name]['frame']))<1e-7

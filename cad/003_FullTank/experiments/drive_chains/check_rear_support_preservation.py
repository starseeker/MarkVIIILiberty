"""Compare every inherited definition against the saved pre-support powertrain parent."""
import argparse
...
assert changed == {'Def_RearControlMount_channel', 'Def_ClutchSupport_floor_revision'}
assert len(source_manifest['definitions']) == 558
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All556 unchanged inherited definitions preserved. The revised channel/floor and six new definitions match the independently tested support-family prototype; ten-definition and22-installed-shape transfer is checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 556

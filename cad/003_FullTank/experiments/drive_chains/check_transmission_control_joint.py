...
for hand in ['Port','Starboard']:
    ...
    cylinders=[f for f in eye.Faces if isinstance(f.Surface,Part.Cylinder) and f.Surface.Axis.cross(Y).Length<1e-7 and f.Surface.Center.cross(Y).Length<1e-6 and abs(f.Surface.Radius-8.0875)<1e-6]
    ...
    forkbores=[f for f in fork.Faces if isinstance(f.Surface,Part.Cylinder) and f.Surface.Axis.cross(Y).Length<1e-7 and f.Surface.Center.cross(Y).Length<1e-6 and abs(f.Surface.Radius-8.0875)<1e-6]

...
def parts(c,main_phase=90.,progress=None):
    ...
    # HB101 distinguishes the cupped end receiving the wider upper bevel.
    # This conical recess is estimated, while the unequal-end treatment is sourced.
    h=h.cut(Part.makeCone(c['bush_radius']+c['bush_housing_gap'],c['upper_cup_radius'],
                          c['upper_cup_depth'],V(0,0,hhi-c['upper_cup_depth']),Z))
    ...
    for y in [-c['clamp_offset'],c['clamp_offset']]:
        ...
        for sign in [-1,1]:
            ...
            relief=c['washer_radius']+c['clamp_relief_margin']
            h=h.cut(box(xa,xb,y-relief,y+relief,mid-relief,mid+relief))

"""Compare every inherited definition against the saved pre-reconciliation powertrain parent."""
import argparse
...
assert changed == {'Def_ControlJoint_pin', 'Def_ControlJoint_fork', 'Def_HighBrakeMechanism_lever_left', 'Def_HighBrakeMechanism_lever_right'}
assert len(source_manifest['definitions']) == 568
assert len(report['new_definitions']) == 0
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All564 unchanged inherited definitions preserved. Four revised family definitions match the independently tested prototype; nine-definition and16-installed-shape transfer is checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 564

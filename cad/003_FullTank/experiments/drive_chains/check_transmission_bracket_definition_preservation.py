"""Compare unchanged bracket-mount-trial definition material with its saved parent.
...
the four deliberately rebuilt definitions.
"""
import argparse
...
assert qualification['local_joint_checks_passed']
...
assert len(changed) == 4 and changed <= target_manifest['definitions'].keys()
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 457 unchanged child definitions compared with the immediate casing parent. Does not close the parent direct-source gate or qualify placements and historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 457

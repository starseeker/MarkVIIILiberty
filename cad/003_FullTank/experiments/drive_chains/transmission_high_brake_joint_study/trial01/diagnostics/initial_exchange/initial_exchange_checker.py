"""Export and strictly compare seven high-speed brake definitions and all 34 installed components."""
import argparse
import math
...
assert len(definitions)==7 and len(installed_names)==34
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('HighBrakeJoints' + scope + '.step')
    ...
    doc = App.newDocument('HighBrakeJointsExchange'+scope)
    try:
        features = []
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='Seven new definitions and all 34 added occurrences; adaptive converged mass integration and unchanged strict material/tolerance criteria. Not a complete brake mechanism or full-tank export.',

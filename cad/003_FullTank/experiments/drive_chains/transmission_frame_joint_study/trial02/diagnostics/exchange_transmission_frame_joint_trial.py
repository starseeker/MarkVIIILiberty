"""Export and strictly compare frame joint definitions and all affected installed components."""
import argparse
...
assert len(definitions)==8 and len(installed_names)==43
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('FrameJoints' + scope + '.step')
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='Seven receiving definitions, one rivet definition and 43 affected installed components; not a full-tank export.',

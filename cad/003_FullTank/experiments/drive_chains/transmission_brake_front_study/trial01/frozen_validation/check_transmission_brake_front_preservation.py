"""Compare all inherited front-brake definition material with its saved parent.
...
the eight newly added definitions.
"""
import argparse
...
assert qualification['local_front_checks_passed']
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 482 unchanged inherited definitions compared with the qualified rear-anchor parent. Two backing revisions checked separately. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 482

"""Export and strictly compare front-brake definitions and all 132 affected installed components."""
import argparse
import math
...
assert len(definitions)==10 and len(installed_names)==132
...
for scope, shapes in [('Definitions', definitions), ('Installation', installed)]:
    path = out / ('BrakeFront' + scope + '.step')
...
write(out / 'exchange_checks.json', dict(passed=all(v['passed'] for v in results),
    ...
    scope='Ten new/changed definitions and all 132 affected occurrences; adaptive converged mass integration and unchanged strict material/tolerance criteria. Not a full brake installation or full-tank export.',

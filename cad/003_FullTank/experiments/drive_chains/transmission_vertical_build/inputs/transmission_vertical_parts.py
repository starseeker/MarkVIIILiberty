...
def rounded_finger(c,reach):
    """Revolve a single neck/ball profile to preserve analytic STEP surfaces."""
    r=c['finger_radius'];neck=c['finger_neck_radius']
    start=reach-c['finger_neck_setback']-c['finger_overlap']
    join=reach-math.sqrt(r*r-neck*neck)
    a=App.Vector(start,0,0);b=App.Vector(start,0,neck)
    d=App.Vector(join,0,neck);end=App.Vector(reach+r,0,0)
    profile=Part.Wire([Part.makeLine(a,b),Part.makeLine(b,d),
        Part.Arc(d,App.Vector(reach,0,r),end).toShape(),Part.makeLine(end,a)])
    return Part.Face(profile).revolve(App.Vector(),App.Vector(1,0,0))
...
def vertical_parts(c,old_case,old_rod):
    ...
    finger=rounded_finger(c,reach);finger.translate(App.Vector(0,0,rod_z))
    upper=upper.fuse(finger)
    ...
    for dy in [-c['fastener_offset'],c['fastener_offset']]:
        tool=xc(c['fastener_radius']+c['fastener_gap'],-c['bearing_radius']-1,seat+1,dy,zmid)
        tool=tool.fuse(xc(c['nut_relief_radius'],-c['bearing_radius']-1,back,dy,zmid))
        bearing=bearing.cut(tool)

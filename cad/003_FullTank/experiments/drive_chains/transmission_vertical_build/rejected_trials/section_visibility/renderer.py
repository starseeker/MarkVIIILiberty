...
try:
    ...
    def role(n):
        if n.startswith(('ReversingControl','VerticalControl','UpperVertical','LowerVertical')):return 'Control'
    ...
    draw(report['new_ids']+['ReversingControl_rod','ReversingControl_fork','ReversingControl_plunger','ReversingControl_spring','ReversingControl_cap'],
        'vertical_mechanism','Reversing linkage | separate shaft, keys, levers, blind bearings and bolt/stud sets',(-1,-1,1))
    # Reveal the shaft, key and blind journal by cutting the back half of the
    # bearing and lever only. Keep the actual shaft/key geometry intact.
    clip=Part.makeBox(200,200,110,origin+App.Vector(-295,-279,130))
    selected=[]
    ...
    shaded(selected,out/'vertical_bearing_section.svg',(-1,-1,.6),'Upper bearing section | blind journal, shaft shoulder and Woodruff key (size estimated)')
    ...
    assert sha(native)==report['native_sha256'];print('Rendered six vertical-control views; native unchanged.',flush=True)

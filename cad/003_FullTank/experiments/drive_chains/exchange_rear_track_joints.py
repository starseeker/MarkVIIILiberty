"""Strict per-definition and installed STEP comparison for the saved four track-rod end joints."""
import argparse
import math
...
for scope,shapes in [('Definitions',definitions),('Installed',installed)]:
    path=out/('RearTrackRodJoints'+scope+'.step');assert not path.exists();doc=App.newDocument('TrackJointExchange'+scope)
    try:
        objects=[]

...
def parts(c, old, band_controls, bd, support_controls):
    ...
    tip=V(sum(c['mount_x'])/2,c['mount_head_y']+c['mount_grip']+stock/2,c['mount_z']+21)
    ...
    # Finish the web toe at the bracket's flat mounting plane below the
    # plate's upper edge. The loft otherwise projects through that mating
    # face; the trim is defined by the joint planes, not a neighboring solid.
    bb=web.BoundBox
    web=web.cut(Part.makeBox(bb.XLength+2,my-bb.YMin+1,mz+20-bb.ZMin+1,
                           V(bb.XMin-1,bb.YMin-1,bb.ZMin-1)))
    ...
    return_x=xb+c['carrier_outboard_offset']
    carrier=Part.makeBox(return_x+10-xa,c['carrier_stock'],40,V(xa,my+stock,mz-20))
    ...
    # Rise beyond the anchor plate's end, then pass below the rod to the
    # pivot. The estimated return offset keeps the sloping web off that end.
    a=V(return_x,my+stock+c['carrier_stock']/2,mz+15)
    upright=Part.makeBox(16,c['carrier_stock'],25,V(return_x-8,my+stock,mz-10))
    ...
    for k,x in enumerate(c['mount_x'],1):
        ...
        add('mount_bolt','AnchorBolt%d'%k,origin,App.Rotation(Z,Y))
        add('mount_lock','AnchorLock%d'%k,V(x,seat,mz),App.Rotation(Z,Y))
        add('mount_nut','AnchorNut%d'%k,V(x,seat+c['mount_lock_stock'],mz),App.Rotation(Z,Y))

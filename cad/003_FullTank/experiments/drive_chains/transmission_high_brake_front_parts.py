...
def parts(c,parent,bands):
    ...
    for role,sign,z in [('long',1,c['upper_pin_z']),('short',-1,c['lower_pin_z'])]:
        ...
        for station,axial in zip(c['steel_stations'],c['steel_axial']):
            ...
            # Keep the default cutter seam for the fitting. Rotating it 90deg
            # left a spurious cylinder-face patch across the third long hole
            # despite an empty solid/void intersection. The thin backing needs
            # the separately qualified 90deg seam; material is identical.
            rotation=App.Placement(V(),App.Rotation(V(0,0,1),90));tool.Placement=pose
            foot=foot.cut(tool);local=tool.copy();local.Placement=inv.multiply(pose).multiply(rotation);tools.append(local)

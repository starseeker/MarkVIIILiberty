...
def rounded_triangle(points,radius,height):
    """Convex circular offset of a CCW triangle; analytic straight/arc outline."""
    v=[App.Vector(x,y,0) for x,y in points];edges=[]
    normals=[]
    for a,b in zip(v,v[1:]+v[:1]):
        d=b-a;n=App.Vector(d.y,-d.x,0);n.normalize();normals.append(n)
    for i in range(3):
        j=(i+1)%3;n0,n1=normals[i],normals[j]
        a=v[i]+n0*radius;b=v[j]+n0*radius;end=v[j]+n1*radius
        angle=math.atan2(n0.y,n0.x)
        turn=(math.atan2(n1.y,n1.x)-angle)%(2*math.pi)
        mid=v[j]+App.Vector(math.cos(angle+turn/2),math.sin(angle+turn/2),0)*radius
        edges.extend([Part.makeLine(a,b),Part.Arc(b,mid,end).toShape()])
...
def build(c):
    ...
    axes=c['bearing_bolt_axes_yz']
    plate=rounded_triangle(axes,c['bearing_corner_radius'],c['cover_stock'])
    ...
    shoulder=h+c['cover_stock']-c['bush_length']-c['shaft_axial_gap']
    shaft=shaft.fuse(xc(c['shaft_body_radius'],-shoulder,shoulder))
    ...
    for station,x in enumerate(c['station_x']):
        ...
        for sign,label in [(1,'Port'),(-1,'Starboard')]:
            ...
            tools.append(moved(cyl(c['cylinder_bore_radius'],20,seat+1),(x,0,0),rot))
    ...
    tools.append(cyl(1.5,c['shaft_cavity_radius']-1,roof-6))
    ...
    notes.update(banks=banks,roof_height=roof,base_mounting_plane=floor,
        shaft_body_shoulder_x=[-shoulder,shoulder],shaft_axial_endplay=2*c['shaft_axial_gap'],

...
for name,row in rows.items():
    ...
    elif name.endswith(('TransmissionCore_bevel_case','TransmissionCore_brake_case','TransmissionCore_plain_case')):role='TrialTransmission'
    ...
    if role:
        ...
        if ('FixedTransmissionBearings' in owners or 'TransmissionMountingFrame' in owners
            or 'TransmissionCaseMounting' in owners or name.startswith(('PortTransmissionOutput_','StarboardTransmissionOutput_'))
            or name.endswith(('TransmissionCore_bevel_case','TransmissionCore_brake_case','TransmissionCore_plain_case'))):
...
views=[
    ...
    ('transmission_photo_view',transmission,[],(1,-.08,1.3),'Transmission comparison view | source-counted joints with inferred pad planes and hole distribution'),
]
...
write(out/'render_receipt.json',dict(native_sha256=r['native_sha256'],manifest_sha256=sha(out/'isolated/manifest.json'),
    ...
    views={name:dict(direction=direction,solid_ids=[v['id'] for v in selected],context_ids=[v['id'] for v in outlined])
           for name,selected,outlined,direction,title in views},display_only_channel_crop=True,
    source_comparison='Qualitative photograph; no metric calibration or source/CAD fit.',

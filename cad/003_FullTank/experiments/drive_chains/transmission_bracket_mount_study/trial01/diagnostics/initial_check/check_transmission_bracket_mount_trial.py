...
for name,before in old['assemblies'].items():
    ...
    okay=okay and (len(added)==1 if allowed_additions else not added)
...
for role in ['inner','outer']:
    ...
    witness=box(-150,300,-300,300,-180,180)
    near(role+' bearing and cap-stud material retained',before.common(witness).cut(after).Volume)
    near(role+' bearing and cap-stud voids retained',after.common(witness).cut(before).Volume)

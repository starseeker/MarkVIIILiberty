...
for role in ['inner','outer']:
    ...
    witness=box(-radius-8,300,-300,300,-200,200)
...
for role in ['inner','outer']:
    name='Def_FixedBearing_'+role+'_bracket';before=definition(name,'old');after=definition(name)
    added=after.cut(before);removed=before.cut(after)
    if role=='inner':
        allowed_added=base['inner_revised_base'].cut(base['inner_original_base'])
        allowed_removed=base['inner_original_base'].cut(base['inner_revised_base'])
        if added.Solids and allowed_added.Solids:added=added.cut(allowed_added)
        if removed.Solids and allowed_removed.Solids:removed=removed.cut(allowed_removed)
    for joint in [j for j in r['details']['joints'] if j['role']==role]:
        y,z=joint['y_local_mm'],joint['z_local_mm']
        tools=[cylinder_x(r['controls']['head_spotface_radius'],r['details']['head_seat_x_mm'],0,y,z),
    ...
    near(role+' no material added outside declared casting revision',added.Volume)
    near(role+' no material lost outside revision and joint tools',removed.Volume)

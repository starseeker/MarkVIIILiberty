"""Compare saved suspension definitions and installed STEP solids in both material directions."""
import argparse
import math
...
try:
    ...
    native=out/'DrivetrainWithEngineSuspension.FCStd';nh=sha(native);r=read(out/'report.json');assert r['native_sha256']==nh
    ...
    for label,expected in [('Definitions',[(k,doc.getObject(k).Shape) for k in read(out/'definition_order.json')]),
        ...
        path=out/('EngineSuspension'+label+'.step');loaded=Part.Shape();loaded.read(str(path))
        ...
        for key,shape in expected:
            ...
            analytic=None
            if key=='Def_EngineSuspension_bevel_washer' or (key.startswith('EngineSuspension_') and 'Bevel' in key):
                c=r['controls'];side=c['bevel_washer_side'];thick=c['bevel_washer_center_stock'];radius=c['rear_bolt_diameter']/2+c['hole_radial_allowance']
                slope=math.tan(math.radians(c['rear_channel_upper_taper_deg']));area2=side*side-math.pi*radius**2;inertia=side**4/12-math.pi*radius**4/4
                volume=area2*thick;local=App.Vector(-slope*inertia/volume,0,(thick*thick*area2+slope*slope*inertia)/(2*volume))
                if label=='Installation':
                    occurrence=next(x for x in r['occurrences'] if x['name']==key)
                    local=App.Placement(App.Vector(*occurrence['xyz']),App.Rotation(*occurrence['rotation'])).multVec(local)
                analytic=dict(volume_mm3=volume,center_mm=list(local),native_volume_error_mm3=abs(ma['volume_mm3']-volume),step_volume_error_mm3=abs(mb['volume_mm3']-volume),native_center_error_mm=(App.Vector(*ma['center_mm'])-local).Length,step_center_error_mm=(App.Vector(*mb['center_mm'])-local).Length)
                passed=passed and max(analytic['native_volume_error_mm3'],analytic['step_volume_error_mm3'])<1e-5 and max(analytic['native_center_error_mm'],analytic['step_center_error_mm'])<1e-6
            exchange.append(dict(scope=label,part=key,passed=passed,analytic_bevel=analytic,missing_mm3=missing,added_mm3=added,mass_difference_mm3=dv,centroid_difference_mm=dc))
    ...
    write(out/'exchange_checks.json',dict(passed=all(x['passed'] for x in exchange),native_sha256=nh,checker_sha256=sha(Path(__file__)),
        artifact_hashes={n:sha(out/n) for n in ['EngineSuspensionDefinitions.step','EngineSuspensionInstallation.step']},checks=exchange))

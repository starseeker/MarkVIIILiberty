"""Compare every inherited definition against the saved qualified operating-mechanism parent."""
import argparse
...
assert changed == {'Def_TransmissionCore_bevel_case'}
assert len(source_manifest['definitions']) == 529
assert len(report['new_definitions']) == 3
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 528 unchanged inherited definitions compared with the qualified operating-mechanism parent. The revised case and three new definitions are checked separately against the tested receiving prototype. Does not qualify placements or historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 528

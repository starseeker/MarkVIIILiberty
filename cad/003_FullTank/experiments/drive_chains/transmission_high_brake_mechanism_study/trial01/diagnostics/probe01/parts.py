...
def parts(c,front):
    ...
    outer=f['fork_gap']+2*f['lug_stock'];head=-outer/2-c['pin_axial_clearance'];cotter=outer/2+c['cotter_station_overhang'];tip=cotter+c['pin_end_beyond_cotter']
    ...
    boss=Part.makeCylinder(c['lever_bearing_radius'],c['lever_upper_seat_distance']-seat,V(0,0,seat));boss.Placement=screw_frame;lever=lever.fuse(boss)
    ...
    for name,s in shapes.items():assert s.isValid() and len(s.Solids)==1 and s.Solids[0].isClosed(),(name,s.isValid(),len(s.Solids))
    detail=dict(upper_pin_mm=list(upper),lower_pin_mm=list(lower),pin_head_seat_y_mm=head,pin_tip_y_mm=tip,cotter_station_y_mm=cotter,fork_outer_width_mm=outer,
        ...
        limits=['Paired lever member section and integral bearing bosses are inferred; B-spline curves approximate the illustrated silhouette.', 'Free-end pin and cotter stock, spring dimensions, nut form and screw shoulders are estimates.', 'Washer bore dimensions and joining-rivet stock are printed; washer outside diameters and thickness are estimates.', 'Thread interfaces use nominal cylindrical envelopes, without helices.', 'Static arrangement only; anchor supports, stops, control rods and complete service paths remain pending.'])

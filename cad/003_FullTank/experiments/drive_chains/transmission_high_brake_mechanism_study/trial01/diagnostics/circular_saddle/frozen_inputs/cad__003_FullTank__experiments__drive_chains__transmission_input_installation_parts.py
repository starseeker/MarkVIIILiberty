...
def eye_join(wire,half,head,eye,rise,join,station=0):
    top=head-rise
    # Trim a full torus. A directly parameter-trimmed half torus can lose the
    # lead solids during fusion at these small clearances despite isValid().
    shape=Part.makeTorus(eye,wire,App.Vector(station,top,0),App.Vector(1,0,0))
    extent=2*(eye+wire+1)
    shape=shape.cut(Part.makeBox(2*extent,extent,2*extent,App.Vector(station-extent,top,-extent)))
    for sign in [-1,1]:
        a=App.Vector(station,top,sign*eye);b=App.Vector(station,head+join,sign*half);delta=b-a;axis=delta/delta.Length
        shape=shape.fuse(axial(wire,a-axis*join,axis,delta.Length+2*join))
    return shape


def unspread_pin(c):
    half=c['spacing']/2;wire=(c['diameter']-c['spacing'])/2
    shape=eye_join(wire,half,c['head'],c['eye_radius'],c['eye_rise'],c['join_overlap'],c['station'])
    legs=[axial(wire,App.Vector(c['station'],c['head'],sign*half),App.Vector(0,1,0),c['length']) for sign in [-1,1]]
    shape=shape.fuse(legs[0]).fuse(legs[1]).removeSplitter()
    assert shape.isValid() and len(shape.Solids)==1 and all(leg.cut(shape).Volume<1e-5 for leg in legs)
    return shape


def formed_pin(c):
    """Analytic cylinder/torus legs avoid swept-torus STEP ambiguity."""
    half=c['cotter_center_spacing']/2;wire=(c['cotter_diameter']-c['cotter_center_spacing'])/2
    head=-c['crown_radius']-c['cotter_head_gap'];bend=c['crown_radius']+c['cotter_exit_gap']
    r=c['cotter_bend_radius'];angle=math.radians(c['cotter_bend_angle'])
    straight=bend-head;arc=r*angle;tail=c['cotter_length']-straight-arc
    assert tail>0 and wire>0
    legs=[];ends=[]
    for sign in [-1,1]:
        start=App.Vector(0,head,sign*half)
        bend_end=App.Vector(0,bend+r*math.sin(angle),sign*(half+r*(1-math.cos(angle))))
        tangent=App.Vector(0,math.cos(angle),sign*math.sin(angle))
        torus=Part.makeTorus(r,wire,App.Vector(),App.Vector(0,0,1),-180,180,c['cotter_bend_angle'])
        torus.rotate(App.Vector(),App.Vector(0,1,0),sign*90);torus.translate(App.Vector(0,bend,sign*(half+r)))
        legs.append(axial(wire,start,App.Vector(0,1,0),straight).fuse(torus).fuse(axial(wire,bend_end,tangent,tail)).removeSplitter())
        ends.append(list(bend_end+tangent*tail))
    eye_shape=eye_join(wire,half,head,c['cotter_eye_radius'],c['cotter_eye_rise'],c['cotter_eye_join_overlap'])
    shape=eye_shape.fuse(legs[0]).fuse(legs[1]).removeSplitter()
    assert len(shape.Solids)==1 and all(leg.cut(shape).Volume<1e-5 for leg in legs)
    return shape,dict(straight_length_mm=straight,bend_arc_length_mm=arc,tail_length_mm=tail,
        total_leg_centerline_mm=straight+arc+tail,tail_endpoints=ends,wire_radius_mm=wire,
        under_eye_y_mm=head,bend_start_y_mm=bend,exact_stock_volume_conserved=False,
        construction='Analytic cylinder/torus legs and semicircular eye; eye leads overlap the leg junctions by the stated construction allowance.')
...
def mounting(c, originals):
    ...
    for y,z in axes:
        relief=cylinder(c['nut_relief_radius'],c['nut_seat'],c['nut_relief_end'])
        relief.translate(App.Vector(0,y,z));shapes['housing']=shapes['housing'].cut(relief)
...
def grease_feed(c, housing, spacer):
    ...
    def sweep(radius):
        angle=math.radians(c['angle']);r=c['bend_radius'];straight=c['center_to_face']-r*math.tan(angle/2)
        torus=Part.makeTorus(r,radius,App.Vector(),App.Vector(0,0,1),-180,180,c['angle'])
        torus.rotate(App.Vector(),App.Vector(1,1,1),120);torus.translate(App.Vector(0,-r,straight))
        return axial(radius,App.Vector(),App.Vector(0,0,1),straight).fuse(torus).fuse(
            axial(radius,end-direction*straight,direction,straight)).removeSplitter()

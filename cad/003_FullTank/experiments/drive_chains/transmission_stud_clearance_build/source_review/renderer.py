"""Render the fastened bearing cap beside its handbook illustration."""
import argparse
import base64
...
try:
    ...
    native=candidate/'TransmissionStudCandidate.FCStd';assert sha(native)==r['native_sha256']
    doc=App.openDocument(str(native));doc.recompute()
    prefix='PortFixedBearing_inner_'
    selected=[i for i in leaves(doc.Root) if i['id'].startswith(prefix) and (i['id'].endswith(('_cap','_lid','_cover_pin')) or '_Stud' in i['id'])];assert len(selected)==15
    shaded(selected,out/'fastened_cap_detail.svg',(1,.8,.8),'M294 cap with MX10/MX36 studs, castle nuts and split pins')
    shaded(selected,out/'cap_rear_detail.svg',(-1,.7,.8),'Cap from rear | two source stud lengths and inferred upper/lower allocation')
    ...
    native_data=base64.b64encode((out/'fastened_cap_detail.png').read_bytes()).decode()
    scale=590/510
    svg=('<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="900">'
         ...
         '<text x="30" y="36" font-family="sans-serif" font-size="24">Bearing cap fasteners | handbook illustration and native candidate</text>'
         f'<image x="635" y="135" width="940" height="529" href="data:image/png;base64,{native_data}"/>'
         '<text x="30" y="779" font-family="sans-serif" font-size="19">HB20 Plate11: cap nuts, rear hinge and open lid. Native: fastened closed cap. Views are not registered.</text>'
         '<text x="30" y="811" font-family="sans-serif" font-size="19">SNL241 establishes stud sizes and nut/split-pin identities. Nut dimensions and cast profiles remain inferred.</text>'
         '<text x="30" y="843" font-family="sans-serif" font-size="19">Visible differences: rounded cup/cast transitions, oil inlet, oil fittings and wool remain incomplete.</text>'
         '<text x="30" y="875" font-family="sans-serif" font-size="19">Thread flanks, tightening and historical allocation of the two inner stud lengths remain unqualified.</text></svg>')
    (out/'source_fastener_detail.svg').write_text(svg)
    with fitz.open(stream=svg.encode(),filetype='svg') as drawing:drawing[0].get_pixmap().save(str(out/'source_fastener_detail.png'))
    ...
    print('Saved fastened-cap review views.',flush=True)

"""Compare unchanged casing-trial definition material with its saved parent.
...
the twelve deliberately rebuilt definitions.
"""
import argparse
...
assert qualification['local_casing_checks_passed']
...
assert target_manifest['definitions'].keys() == source_manifest['definitions'].keys()
changed = set(report['replaced_definitions'])
assert len(changed) == 12 and changed <= target_manifest['definitions'].keys()
...
result = dict(passed=all(v['passed'] for v in records), native_sha256=report['native_sha256'],
              ...
              scope='All 449 unchanged child definitions compared with the immediate following-frame parent. Does not close the parent direct-source gate or qualify placements and historical interpretation.',
              installation_qualified=False)
write(out / 'definition_preservation_checks.json', result)
assert result['passed'] and len(records) == 449

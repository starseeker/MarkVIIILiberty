...
if selected['overlap_mm3']>1e-5 or selected['minimum_gap_mm']<.1:
    for offset in [.25,.5,1.,2.]:
        ...
        # Match one roller center to the nearest existing cylindrical relief.
        # This picks a static reference phase only; the offset remains an
        # explicit reconstruction assumption, never a source calibration edit.
        data=load();v=data['values']
        groove_radius=v['drive_root_radius'].value+v['drive_groove_radius'].value
        count=int(v['drive_teeth'].value)
        grooves=[App.Vector(groove_radius*math.sin(2*math.pi*n/count),0,
                            groove_radius*math.cos(2*math.pi*n/count)) for n in range(count)]
        groove=min(grooves,key=lambda g:(g-center).Length)
        delta=center-groove;projection=unit.dot(delta)
        discriminant=projection**2-(delta.dot(delta)-a['roller_circle']**2)
        assert discriminant>0
        offset=min([-projection-math.sqrt(discriminant),-projection+math.sqrt(discriminant)],key=abs)
        assert abs(offset)<=2.,offset
        direction=groove-(center+unit*offset)
        angle=round(math.degrees(math.atan2(direction.x,direction.z))%40,4)
        selected=measure(angle,offset)
        assert selected['overlap_mm3']<1e-5 and selected['minimum_gap_mm']>=.1,selected
...
write(OUT/'report.json',dict(status='candidate_static_pinion_fit',passed=True,selected=selected,samples=rows,
    ...
    placement_method='Small explicit radial station correction and static phase put one roller center at an existing cylindrical driving-ring relief. Other rollers are checked natively; no continuous engagement is established.',

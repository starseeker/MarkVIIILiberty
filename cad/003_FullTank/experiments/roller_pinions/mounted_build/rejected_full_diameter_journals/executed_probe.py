...
p=dict(shaft_length=25.25*25.4,shaft_diameter=4.434*25.4,
       ...
       bearing_running_gap=.1,rear_panel_seam_pixel_x=1642.,keyway_end_overrun=.1)
...
write(OUT/'hypotheses.json',dict(status='unaccepted_mount_and_receiver_fixture',values_mm=p,
    ...
    note='Common source-defined parts retain their native geometry. M1546 casting, oil drilling, 3/4-inch nominal pipe-thread envelope, plug head and counterbore are inferred. Inner plug is cut flush as HB132 specifies. The keyway is open through the shaft end to avoid a 0.0625 mm inferred lip. The wing/end panel seam is independent of the unchanged fuel-compartment backplate and provisionally moved from source pixel1630 to1642. No motion, seal, thread or retention strength is qualified.'))
...
# Constant-diameter fixed shaft with through lubrication and two radial leads.
shape=cylinder_y(p['shaft_diameter']/2,p['shaft_length'])
...
shape=annulus(a['barrel_radius'],p['shaft_diameter']/2+p['bearing_running_gap'],a['shoulder'],p['inner_face'])
shape=shape.fuse(annulus(a['flange_radius'],p['shaft_diameter']/2+p['bearing_running_gap'],a['outside'],p['inner_face']))

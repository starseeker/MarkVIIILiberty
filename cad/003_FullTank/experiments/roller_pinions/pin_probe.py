...
p=dict(pin_length=5.593*25.4,pin_diameter=1.75*25.4,head_radius=27.5,head_stock=8.,
       ...
       cotter_center_spacing=4.04,cotter_head_gap=.5,cotter_from_inner_end=10.,cotter_hole_clearance=.2,
...
wire=p['cotter_wire_radius'];half=p['cotter_center_spacing']/2;stem=p['cotter_nominal_length'];top=r+p['cotter_head_gap'];bottom=top-stem
...
for side,label in [(1,'A'),(-1,'B')]:
 for n in range(9):
  ...
  # about Z to put both factory heads outward while keeping cotter eyes radial.
  rotation=App.Rotation(App.Vector(0,1,0),n*40).multiply(App.Rotation(App.Vector(0,0,1),0 if side==1 else 180))
...
with database() as c:
 for role,rid in role_rows.items():
  ...
  assert c.execute('SELECT 1 FROM part_evidence WHERE part_id=? AND record_id=?',(source_rows[rid]['part_ids'][0],rid)).fetchone()

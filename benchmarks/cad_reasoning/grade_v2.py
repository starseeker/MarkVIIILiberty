...
def compile_candidate(code, entry, App, Part):
    ...
    builtins = {k:v for k,v in vars(__import__('builtins')).items() if k in
        ['range','len','min','max','abs','sum','all','any','float','int','bool','enumerate','zip','list','dict','tuple','set',
...
def main():
    ...
    args=p.parse_args();out=(args.output or HERE/'results/self_test').resolve();out.mkdir(parents=True,exist_ok=True)
    if not args.worker:
